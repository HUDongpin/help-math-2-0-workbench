...
test('private registry supports independent lesson calibrations without widening authority', async () => {
  ...
  const g4L9P5 = document.calibrations.find(
    (calibration: {calibrationId: string}) =>
      calibration.calibrationId === 'g4-l9-p5-f08-occurrence-32-stress-v1',
  );
  assert.deepEqual(g4L9P5.entries, [{
    key: 'course-g04-l09-ti-007',
    module: './modules/course-g04-l09-ti-007',
    maturity: 'private-current-js',
    complexityLane: 'behavior-heavy',
  }]);
  assert.deepEqual(animationModuleRegistration('course-g04-l09-ti-007'), {
    maturity: 'private-current-js',
    scope: 'private-engineering',
