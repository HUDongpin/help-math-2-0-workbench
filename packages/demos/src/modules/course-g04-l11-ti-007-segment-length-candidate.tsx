...
import {COURSE_G04_L11_TI_007_INTERACTION_AUTHORITY,
  ...
  getCourseG04L11Ti007SelectedGlossary, reduceCourseG04L11Ti007State,
  type CourseG04L11Ti007Event, type CourseG04L11Ti007GlossaryId,
  type CourseG04L11Ti007State} from
  "../timelines/course-g04-l11-ti-007-segment-length-interaction";
...
const SOURCE_DOMAIN = "sprite-258";
const UI_FONT = '"Arial Rounded MT Bold", "Trebuchet MS", ui-rounded, system-ui, sans-serif';
const ES_TERMS: Readonly<Record<CourseG04L11Ti007GlossaryId, string>> = Object.freeze({
  number: "número", unit: "unidad", length: "longitud",
  "line-segment": "segmento de línea", horizontal: "horizontal",
  subtract: "restar", "x-coordinate": "coordenada x",
  "coordinate-grid": "cuadrícula de coordenadas",
});
const ES_TERM_PROMPTS: Readonly<Record<CourseG04L11Ti007GlossaryId, string>> =
Object.freeze({
  ...
  horizontal: "Un segmento horizontal va recto de izquierda a derecha.",
  subtract: "Resta para hallar la diferencia entre dos valores.",
  "x-coordinate": "La coordenada x indica la posición horizontal de un punto.",
...
function SegmentLengthControls({state, dispatch, lang, replayLocally}: {
  state: CourseG04L11Ti007State;
  dispatch: React.Dispatch<CourseG04L11Ti007Event>;
  lang: "en" | "es"; replayLocally: () => void;
}) {
  const isSpanish = lang === "es";
  const selectedGlossary = getCourseG04L11Ti007SelectedGlossary(state);
  const modal = state.phase === "wrong-feedback" ||
    state.phase === "correct-feedback" || state.phase === "help" ||
    state.phase === "glossary";
  return <section className="course-g04-l11-ti007-practice"
    aria-label={isSpanish ? "Práctica de longitud horizontal" :
      "Horizontal line-segment length practice"}
    data-animation-internal-pedagogical-control-count="12"
    data-source-answer-control-count="3" data-source-help-control-count="1"
    data-source-glossary-control-count="8" data-source-quiz-stop-frame="119"
    ...
    <header><span>HELP Math 2.0</span>
      <h2>{isSpanish ? "Longitud de un segmento horizontal" :
        "Length of a Horizontal Line Segment"}</h2>
      <p>{isSpanish ? "Haz clic en el número de unidades de longitud." :
        COURSE_G04_L11_TI_007_INTERACTION_SOURCE.prompt}</p>
    </header>
    <div className="course-g04-l11-ti007-facts">
      <div><span>{isSpanish ? "Extremos" : "Endpoints"}</span>
        <strong>(9,1) → (4,1)</strong>
        <p>{isSpanish ? "Los puntos comparten y = 1; compara sus valores de x." :
          "The points share y = 1, so compare their x-values."}</p></div>
    ...
    <div className="course-g04-l11-ti007-choices" role="group"
      aria-label={isSpanish ? "Opciones de respuesta" : "Answer choices"}>
      {COURSE_G04_L11_TI_007_CHOICES.map((choice) => <button
    ...
    <div className="course-g04-l11-ti007-support">
    ...
    <div className="course-g04-l11-ti007-terms" role="group"
      aria-label={isSpanish ? "Términos matemáticos" : "Math terms"}>
      {COURSE_G04_L11_TI_007_GLOSSARY.map((term) => <button
    ...
    {state.phase === "wrong-feedback" || state.phase === "correct-feedback"
      ? <div aria-live="assertive"
        className={`course-g04-l11-ti007-panel ${state.phase}`}
    ...
    {state.phase === "help" ? <div aria-live="polite"
      className="course-g04-l11-ti007-panel help" role="status">
      <strong>{isSpanish ? "Cómo hallar la longitud" : "How to find the length"}</strong>
      <p>{isSpanish ? "Resta las coordenadas x: 9 menos 4. La diferencia es la longitud." :
        COURSE_G04_L11_TI_007_INTERACTION_SOURCE.visibleCoaching}</p>
    ...
    {selectedGlossary ? <div aria-live="polite"
      className="course-g04-l11-ti007-panel glossary" role="status">
    ...
    {state.completed ? <div aria-live="polite"
      className="course-g04-l11-ti007-complete">
      <strong>9 − 4 = 5</strong>
      <p>{isSpanish ? "La longitud es 5 unidades." :
        "The length is 5 units."}</p>
    </div> : null}
    <div className="course-g04-l11-ti007-playback">
...
export function createCourseG04L11Ti007SegmentLengthCandidate<
...
>(candidate: SourceCandidate<SourceContract>) {
  ...
  function Renderer(props: AnimationRendererProps) {
    ...
    const [state, dispatch] = useReducer(reduceCourseG04L11Ti007State,
      {frame: props.frame, seed: props.seed}, ({frame, seed}) =>
        createCourseG04L11Ti007State(frame, seed));
    ...
    return <div className="course-g04-l11-ti007-candidate"
      data-authoritative-original-runtime-evidence="false"
      data-current-js-functional-candidate="true"
      data-current-js-functional-scope="143-source-canvas-frames-twelve-teaching-controls-and-modern-feedback"
      ...
      <style>{`
        .course-g04-l11-ti007-candidate [data-source-replay-parity="unvalidated"]{display:none}
        .course-g04-l11-ti007-source-stage{position:relative}.course-g04-l11-ti007-source-answer-mask{background:#b8d8f7;height:42%;left:61%;pointer-events:none;position:absolute;top:27%;width:18%;z-index:2}
        .course-g04-l11-ti007-practice{background:linear-gradient(145deg,#f5fbff,#e4f6ff);border:2px solid #2168a7;border-radius:18px;box-sizing:border-box;color:#17395f;display:grid;font-family:${UI_FONT};gap:14px;margin-top:12px;padding:16px;width:100%}
        .course-g04-l11-ti007-practice header span,.course-g04-l11-ti007-facts span{color:#0758ba;display:block;font-size:12px;font-weight:900;letter-spacing:.08em;text-transform:uppercase}.course-g04-l11-ti007-practice header h2{font-size:clamp(23px,4vw,32px);line-height:1.1;margin:3px 0 0}.course-g04-l11-ti007-practice p{font-family:system-ui,sans-serif;line-height:1.45;margin:5px 0 0}
        .course-g04-l11-ti007-facts{display:grid;gap:12px;grid-template-columns:1fr 1fr}.course-g04-l11-ti007-facts>div{background:#fff;border:2px solid #75a6d5;border-radius:14px;padding:13px}.course-g04-l11-ti007-facts strong{color:#8b3100;display:block;font-size:clamp(21px,4vw,32px);margin-top:4px}
        .course-g04-l11-ti007-choices{display:grid;gap:10px;grid-template-columns:repeat(3,1fr)}.course-g04-l11-ti007-support{display:flex}.course-g04-l11-ti007-terms{display:grid;gap:8px;grid-template-columns:repeat(4,1fr)}
        .course-g04-l11-ti007-practice button{background:linear-gradient(#fff8ad,#ffc72f);border:2px solid #a95000;border-radius:11px;color:#102b70;cursor:pointer;font:800 15px ${UI_FONT};min-height:48px;padding:9px 11px}.course-g04-l11-ti007-practice button[aria-pressed="true"]{background:#0a61bc;border-color:#063a73;color:#fff}.course-g04-l11-ti007-practice button:disabled{cursor:not-allowed;opacity:.62}.course-g04-l11-ti007-practice button:focus-visible{outline:4px solid #0758ba;outline-offset:3px}
        .course-g04-l11-ti007-panel,.course-g04-l11-ti007-complete{background:#fff;border:3px solid #1c75bc;border-radius:14px;padding:14px}.course-g04-l11-ti007-panel.wrong-feedback{border-color:#c65100}.course-g04-l11-ti007-panel strong,.course-g04-l11-ti007-complete strong{display:block;font-size:24px}.course-g04-l11-ti007-panel.glossary{align-items:center;display:flex;gap:14px;justify-content:space-between}.course-g04-l11-ti007-playback{display:flex;justify-content:flex-end}
        @media(max-width:700px){.course-g04-l11-ti007-terms{grid-template-columns:repeat(2,1fr)}}
        @media(max-width:600px){.course-g04-l11-ti007-facts,.course-g04-l11-ti007-choices{grid-template-columns:1fr}.course-g04-l11-ti007-panel.glossary{align-items:stretch;flex-direction:column}.course-g04-l11-ti007-playback button,.course-g04-l11-ti007-support button{width:100%}}
      `}</style>
    </div>;
  }
  const sourceContract = Object.freeze({...candidate.sourceContract,
    currentJavascriptInteractionStatus:
      "143-source-canvas-frames-twelve-teaching-controls-and-modern-feedback-functional-candidate",
    currentJavascriptFunctionalEntry: Object.freeze({frameDomain: SOURCE_DOMAIN,
      frame: 119, scenario: SOURCE_SCENARIO, language: "en",
      deterministicCaptureOverlayEnabled: false}),
    interaction: COURSE_G04_L11_TI_007_INTERACTION_SOURCE,
    authority: COURSE_G04_L11_TI_007_INTERACTION_AUTHORITY,
