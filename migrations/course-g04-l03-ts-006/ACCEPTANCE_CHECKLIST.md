# course-g04-l03-ts-006 Acceptance Checklist
...
- [ ] An authorized original-runtime baseline is identified, versioned, and hash-bound; Ruffle is only a version-bound forensic reference and Adobe Animate test-movie evidence remains subordinate to the shipped original runtime.
...
- [ ] Accepted owner review points to an immutable `{ path, bytes, sha256 }` record that binds the exact human, audio, behavior, product, and exception evidence; `not-required` includes an explicit reason.
...
- [ ] Owner review is hash-bound and accepted, or explicitly marked not required with a reason.
