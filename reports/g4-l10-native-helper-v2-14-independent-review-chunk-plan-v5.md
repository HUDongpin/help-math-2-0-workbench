# G4 L10 Native Helper v2.14 Independent-Review Chunk Plan v5
...
Frozen at: `2026-08-06T23:43:03Z`

This is a new no-clobber successor to `g4-l10-native-helper-v2-14-independent-review-chunk-plan-v4.md`. It does not revive, reuse, repair, validate, cure, or incorporate any prior task ID, HMG4RB4, command, output, finding, or review result. The v4 plan's 21-chunk transport succeeded without observed contract-content truncation, but its batch failed and is permanently non-reusable. V5 retains the verified chunk boundaries while correcting the v4 snapshot label that conflated the 20-byte xattr name with the 11-byte xattr value.
...
xattr-name-count=1
xattr-name-literal=com.apple.provenance
xattr-name-UTF-8-bytes=20
xattr-value-hex=01 02 00 22 02 11 CD 1E 32 E5 B0
xattr-value-bytes=11
ACL-entry-count=0
```

This root-authored freeze snapshot is transport metadata, not reviewer evidence. It uses the original absolute path and macOS-native `shasum`, `wc`, `iconv`, `stat`, `xattr`, `ls`, `sed`, and `tr`. `com.apple.provenance` is ambient metadata with no authority effect. Each reviewer must independently establish a before snapshot and an immediate-final after snapshot. Drift, an unexpected xattr, quarantine, ACL entry, symlink, wrong type, unavailable observation, or ambiguous output fails that unit.
...
Any different command body, missing tool, nonzero exit, extra stdout/stderr, ambiguity, or diagnostic correction permanently invalidates that review unit before evidence begins. It must be disclosed in the final output. Python filesystem or xattr inspection is forbidden, including `os.listxattr`. Heredocs are forbidden for byte, LF, identity, chunk, preimage, output, or HMG4RB4 accounting.

## 3. Predeclared terminal-output non-truncation contract

The 50,310-byte contract is partitioned at existing LF boundaries into exactly 21 contiguous chunks. Every target byte occurs exactly once in order. Each chunk is at most 3,072 bytes; the observed maximum is 3,037 bytes. No reviewer may discover, merge, split, reorder, or replace these boundaries dynamically.
...
3. Use one separate tool call for each display and set that call's output allowance to at least 12,000 tokens. Metadata-only commands must emit at most 120 lines and 8,000 bytes.
...
5. Extract to a reviewer-unique `/tmp` file first; verify bytes/LF/SHA there without displaying file content; then display exactly the same single predeclared native `sed` range. Never print the reconstructed contract.
...
Each reviewer creates one reviewer-unique directory with `/usr/bin/mktemp -d` under `/tmp` and writes nothing in the workspace. Before each evidence-bearing command it states the command label and exact claims tested. Labels describe claims only; they are not parsed as data and must not contain asserted byte counts or line ranges that differ from the command. A failed, ambiguous, corrected, or truncated evidence command permanently invalidates that unit.

1. Observe the original target before identity using native `shasum -a 256`, `wc -c`, `tr -cd '\n' | wc -c`, `iconv -f UTF-8 -t UTF-8` to the unique temporary directory followed by byte/hash comparison, `stat -f`, `xattr`, `xattr -px`, and `ls -ldeO@`. The xattr name and xattr value are separate fields: the literal name is 20 UTF-8 bytes; the native value is 11 bytes. No Python, Git blob, copy, attachment, memory, prior task, or inherited output may substitute.
2. Process chunks 001 through 021 sequentially. For chunk NNN, extract only `/usr/bin/sed -n '<first>,<last>p' "$target" > "$review_tmp/chunk-NNN.bin"`; verify only that file with native `wc` and `shasum`; then display only the same one `sed` range in a separate tool call with at least 12,000 output tokens.
...
Each task and HMG4RB4 is one-use. This batch cannot be replayed, mixed, repaired, or reused for any later review or authority transition.
