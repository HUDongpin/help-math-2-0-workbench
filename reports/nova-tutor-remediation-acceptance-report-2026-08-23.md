...
- 当前总状态：`IMPLEMENTED_AND_LOCALLY_VERIFIED_WITH_UNDEPLOYED_STRUCTURED_LOG_CANVAS_AND_GITLINK_GUARD_PATCH / LATEST_PR_CI_CHECKOUT_BLOCKED_BY_EXTERNAL_GITLINK / G4_L3_TEXT_ONLY_PROTECTED_PREVIEW_PASS_WITH_TOOLBAR_CSP_WARNING / NOVA_OFF_STAGED_PRODUCTION_PASS / NOT_RELEASED`
...
本文件用于在修复后逐层填写验收证据，不覆盖原始问题报告，也不把代码完成、自动测试、人审、Owner/法律批准、Preview、staged Production 或正式生产混写成一个“完成”。截至本次收口，修复已形成远端 draft PR；早期代码 artifact `5fce7424a54f6f11b39c82f7b588d800b298681d` 已完成本地门、Site workspace CI、空 rollout Preview 和八课 master-off Preview 验收。当时的 draft PR head `22ff85fd7eb6f69e981c8c91692e8ea4d60b5907` 又从 exact detached clean worktree 部署为第三个受保护 Preview `dpl_E9pgBqTLgFvRFuKxQzKSxHVHmEac`：只选择 G4 L3、只开放文字、frame/speech 均关闭，并完成恰好一次真实 OpenRouter canary。另已创建并验收一个使用 Production scope、Nova master-off、未绑定 HELP Math custom domain 的 staged rollback candidate。没有向 `www.helpmath.ai` 推广，没有修改 Production 环境、Production alias、Firewall 或 drain；Vercel 曾自动把项目 system alias 指向 staged candidate，alias 解析已恢复到原 production deployment，但 staged inspect 仍保留该 alias 关联显示，严格 metadata reconciliation 尚未关闭。所有真实 canary 只保存无内容 receipt，未保存 prompt/reply 正文或 provider response ID。

受保护 Preview 的截图随后暴露出一个非阻断 P3：用户 speaker label 与正文显示为 `YouIn…`。2026-08-24 已在共享工作树中加入显式空格及 EN/ES 浏览器断言，同时修正 full-stack 测试助手在 React hydration 前点击服务器渲染控件的竞态，并补齐语音状态 telemetry：浏览器只记录固定状态与 locale，明确禁止 transcript、问题、课程上下文或身份。当前本地 web 419/419、server-only 68/68、FULL_STACK_FAKE_UPSTREAM 77/77、原始 modern-wide 门 55/55、typecheck、lint、production build 与秘密扫描全部通过。full-stack 当前还覆盖 current-frame 的 Remove、关闭重开、翻页 identity 重绑定、成功后清除及 Provider 失败后的保留/明确重试；语音草稿清空/关闭保持零请求。客户端 mock 层另证明 busy 双击只发一次，以及 Vercel Firewall 风格的非 JSON 429 正确显示 `NOVA_BUSY`。此前本地默认自动扩到五 worker，而 CI 固定为两个，导致两轮分别 50/54、51/54；失败用例隔离后分别 4/4、3/3 通过，trace 显示约 6–8.6 秒首批 chunk 加载及一次明确 `ECONNRESET`，没有对应产品 console/page error。现在 `test:e2e:modern-wide` 已显式固定 `--workers=2`，与 CI 一致；静态契约防止再次漂移，新增用例后的不带额外参数原始命令 55/55 通过。该批 patch 在对应收据生成时尚未提交、推送或重新部署，因此已验收 Preview 的历史截图和 receipt 仍如实保留 `fixedInThisDeployment=false`；不得把本地修复写成 Preview 或 Production 已包含。

同一批当时未提交的本地 patch 还加入 K–12 输出防御纵深：EN/ES 敏感信息索取会在展示前 fail-closed，并覆盖零宽字符、分隔符、leet 与大小写西里尔同形字符；assessment 页面上的明确答案句、选择项指令与 `\\boxed{}` 会作为 `unsafe-output` 拒绝，而普通非 assessment 教学解释继续允许。该自动规则由 fake/stub provider 测试证明，不是数学教育者、安全专家、Owner 或法律人审，也尚未进入上述 Preview artifact。

本次最终本地收口期间，共享 checkout 的 HEAD 被 Owner-authored 外部提交推进并 push 为 `eee37e3cc4b25edf0fe330d85116ddb081f24f5f`，其中包含此前本地 P3/telemetry/K–12 patch 以及大量独立课程迁移文件；Git reflog 明确记为 `update by push`，GitHub draft PR #4 当前 `OPEN`/draft head 也为该 SHA。Codex 没有创建、stage、commit 或 push 该提交。所有既有 Preview/Production receipt 仍绑定各自的历史 artifact，尤其 G4 L3 live Preview 仍是 `22ff85fd7…`，不能因 PR head 前移而冒充重新部署。当前仅余未提交的 Nova 精确 hunk：API 完成日志增加 content-free `capability=text|image|voice|null`；浏览器只在学习者审阅语音草稿并显式 Send 时附带 strict `speech-to-draft` 来源标记，服务端把它作为不可信 observability metadata 映射为 `voice`，不用于授权、不进入 Provider prompt 且不携带 raw audio；课程帧可见像素探针改用 Nova 私有且首个 context 即带 `willReadFrequently:true` 的 canvas，避免修改 renderer-owned context 并关闭 Chromium 重复读像素 warning 的代码根因。
...
Vercel 浏览器登录完成后，2026-08-24 09:01 UTC 又执行一次只读 `vercel inspect https://www.helpmath.ai --json`：公开别名仍精确解析到 `dpl_6pj5sVFj7P1r7M7Cg12DJngSXhhj`，状态 `READY`、target=`production`，deployment URL 仍为 `helpmath-mdcvbv23q-peter-dongpin-hu-s-projects.vercel.app`。该复核没有修改任何 Vercel 环境变量、deployment、alias、Firewall 或 drain，也没有发送 Provider 请求；因此它只证明 Production 未被本轮 Preview 工作改变，不证明旧 Production 上的 Nova 已修复。
...
| 实现 | `IMPLEMENTED_LOCAL_WITH_UNDEPLOYED_LOG_AND_CANVAS_READBACK_PATCH` | 八课策略、canonical resolver、rollout/capability、frame、speech confirmation、错误、EN/ES 文档与先前 P3/K–12 patch 均存在于当前本地/PR head `eee37e3cc…`；其上另有未提交/未部署的结构化 capability 日志与私有 readback canvas patch；八课 426 placements 的 production asset profile 已形成 | 最后的本地 patch 尚未进入 draft PR 或已验收 Preview；人工/Owner/法律、合并或正式发布 |
| 自动测试 | `PASS_LOCAL / EARLIER_SITE_CI_PASS / LATEST_PR_CI_CHECKOUT_FAIL` | 当前本地 web 419/419、server-only 68/68、显式 CI-aligned 两 worker modern-wide 55/55、full-stack 77/77、typecheck、lint、build、asset profiles、diff-check 和秘密扫描已通过；worker/fake 守护测试 8/8；早期已提交 artifact 的 Site workspace CI 对应门此前通过 414/414 | 最新 PR head 的 Site/Workbench 均在任何 install/test 前因外部提交的无 `.gitmodules` gitlink checkout 失败；最后的未提交日志/readback patch 也不在远端 CI；历史五 worker dev lane、人工评审或生产行为 |
...
| speech-to-draft | flag off 时控件不存在；flag on 时识别结果只进入可编辑草稿，显式 Send 前网络请求数为 0 | `useNovaSpeech` 不再有 `onFinal` 网络动作；Focus/Classroom 只写 draft；15 个语音负面/确认浏览器 case 已在真实 Next route + fake upstream 层执行通过，含编辑草稿、清空草稿及关闭面板零请求；浏览器 telemetry 只发送 allowlisted status 与 locale，显式 Send 才附带 strict `speech-to-draft` 来源标记，EN/ES E2E 证明请求不含 raw audio 且非法来源值在 Provider 前 422 | `IMPLEMENTED_LOCAL_NOT_APPROVED` |
...
| 可观测性 | 每个 API 成功/失败路径只有一条无内容完成日志；可区分文字、当前帧与已确认 speech-to-draft | allowlisted deployment/course/status/latency/attempt/failure/model/byte 字段；`capability=text|image|voice|null` + `framePresent`；`voice` 只表示客户端不可信来源标记，不构成授权或 raw-audio 证明；API 日志无 message/history/reply/frame/IP/header/provider body；语音浏览器 telemetry 仅 status+locale | `IMPLEMENTED_LOCAL_OPERATIONS_PENDING` |
...
- 当前共享 checkout、origin-tracking branch 与 draft PR HEAD：`eee37e3cc4b25edf0fe330d85116ddb081f24f5f`；该 Owner-authored 外部 commit/push 未由 Codex 执行；当前 Nova 日志/readback patch 仍未提交
...
| 普通 web 单测 | React/client、capability、frame/speech helper、status-only telemetry、fake build guard、worker-alignment guard、deployment/private asset split 与既有 web 回归 | 当前本地 `npm test --workspace @helpmath/web` 419/419；新增私有 readback canvas context 契约；worker/fake guard 聚焦 8/8；已提交 artifact 的 Site workspace CI 此前为 414/414 | `PASS_LOCAL_AND_CI` |
...
PASS: npm test --workspace @helpmath/web — 419/419
...
PASS: exact server-secret scan — 5 sensitive entries / 4 unique values；写入本行数字前的最终扫描快照在 68,926-byte tracked candidate diff、12 个 present modified/untracked regular files（270,736 bytes）、1,666 个 final build files（62,400,055 bytes）、51,972 个 report/browser-evidence files（5,162,069,915 bytes）和 20 个 Git-excluded full-stack temporary files（350,336 bytes）中均为 0 exact match；`.next/static` 的 586 个文件对 16 个 sensitive configuration names 为 0 match
...
上述已提交自动化 PASS 来自 2026-08-23 的精确提交候选，Node `v24.18.0`、npm `11.16.0`；2026-08-24 又在后续本地状态加入 P3、hydration synchronization、status-only speech telemetry、test-lane alignment、K–12 输入/输出防御、untrusted-history role 降权、frame 解码、媒体 lifecycle、错误路径、结构化 capability 日志、strict speech-to-draft 来源标记与私有 canvas readback patch 后完成当前本地复验。server suite 与 full-stack suite 使用 fake/stub upstream，不能替代后面单列的真实 OpenRouter canary。全量 lint 只有三个既有 unused-parameter warning。最新秘密扫描从 `.env.local` 识别 5 个非空 server-sensitive 条目、4 个唯一值；写入本段数字前，它们在 68,926-byte tracked candidate diff、Git 报告的 12 个存在的 modified/untracked regular files（270,736 bytes）、1,666 个 `.next/static`/`.next/server` 文件（62,400,055 bytes）、`reports` + `output/playwright` 的 51,972 个文件（5,162,069,915 bytes），以及 Git-excluded `work/tmp/nova-full-stack` 的 20 个文件（350,336 bytes）中均为 0 exact match。当前 `.next/static` 586 个文件对 16 个 Nova/OpenRouter/fake-transport sensitive 配置名称为 0 match；一个既有 Clerk 客户端 chunk 含字面量环境变量名称 `CLERK_SECRET_KEY`，但没有实际 server-sensitive 值匹配。远端 CI 的 Site workspace 已通过已提交 artifact 的 lint、typecheck、414/414 unit、61/61 server、build、ordinary/modern-wide/full-stack 浏览器门；同一 workflow 的 Workbench job 因继承的 migration ledger/release expected-count 门失败，未通过改写受保护迁移证据来制造绿色。外部人审和发布各门仍需独立收据；不要只抄最后一行。
...
最新 PR head `eee37e3cc…` 的 [run 32707426288](https://github.com/HUDongpin/help-math-2-0-workbench/actions/runs/32707426288) 又形成一个不同的前置失败：Site workspace job `97371399739` 与 Workbench job `97371400160` 均在 checkout 阶段、任何 npm install/test 之前报 `fatal: No url found for submodule path '.codex-validation-p51-e355' in .gitmodules`，exit 128。只读取证确认该路径不是产品 submodule，而是主仓库 `git worktree` 管理的内部本地验证 checkout，绑定同一 GitHub origin、detached HEAD `af33bcecff686dc49e143a698f56fd5e938ff498`，且当前包含 27 个 modified/untracked 用户工作条目；目录必须原样保留，不能通过删除来修 CI。当前本地 `.gitignore` 已增加 `/.codex-validation-*/`，并用独立临时 `GIT_INDEX_FILE` 演练父仓库修复，唯一 staged diff 为 `D .codex-validation-p51-e355`，真实 index 全程为空、临时 index 已删除。远端修复仍需要新的授权 commit 把误入的 gitlink 从父仓库 tree 移除；不应为方便 checkout 而把本地工作树伪装成 submodule 或补一个虚假的 URL。本轮明确禁止 stage/commit/push，Codex 未改写真实 index 或远端历史。该 CI gate 在关闭前禁止 merge/release，但不推翻当前 checkout 上已独立通过的本地 Nova 测试收据。

2026-08-24 09:21 UTC 再次通过 GitHub 只读 API 刷新：PR #4 仍为 `OPEN`/draft，head/base 分别保持 `eee37e3cc…` 与 `codex/g4-l3-fidelity-finish`，最新 run 仍为 `32707426288`，两个 job 仍是同一 checkout failure，没有新的 commit、rerun 或外部修复。该刷新没有 fetch、merge、修改本地 refs、stage、commit 或 push。
...
| 当前公开别名 | 2026-08-24 09:01 UTC 登录后只读复核：`www.helpmath.ai` 仍解析到 deployment `dpl_6pj5sVFj7P1r7M7Cg12DJngSXhhj`，状态 `READY`，target `production`；没有 Vercel 写入或 Provider 请求 |
...
- 在当时的 `22ff85fd7…` HEAD、未提交 P3/telemetry/test-lane/K–12/lifecycle/error patch 和最小 `.env.local` 状态下，最终复跑并通过 web 418/418、Nova server 68/68、FULL_STACK_FAKE_UPSTREAM 77/77、原始 modern-wide mock 55/55、Production deployment asset tests 11/11、typecheck、lint（0 error/3 既有 warning）与 production build（28/28 static pages）。新增 fake/stub 测试覆盖 EN/ES learner disclosure 最小化、浏览器伪造 history role 降权、敏感信息索取/回显、混淆文字、assessment 明确答案，以及 frame 空/非法/不支持格式和独立 2,048-pixel 边长限制；没有触达真实 Provider。构建再次证明八课 `1135 public + 185 server = 1320` 文件闭包及 checksum `54e11e77a8d684fcf9542ba7458c12add3edb1a85c16ac99db756a34ed5c6ad4`。历史五 worker lane 两次分别 50/54、51/54；首轮 trace 无 console/page error，但初始 chunk 请求耗时约 6–8.6 秒，次轮另出现明确 Next proxy `ECONNRESET`。两轮失败用例逐批隔离均通过；随后脚本显式固定与 CI 相同的 2 workers，并由静态测试同时锁定 package script 与 Playwright CI 配置，新增用例后的原始门禁命令最终 55/55。full-stack 在系统临时盘再次 ENOSPC 后，把 test-only `TMPDIR` 固定到外置且 Git excluded 的 `work/tmp/nova-full-stack`，最终同一命令完整 77/77；没有删除用户数据、降低断言或修改产品逻辑来制造绿色。
- 在外部 Owner 提交推进到当前 `eee37e3cc…` 后，本轮又复跑并通过 web 419/419、Nova server 68/68、FULL_STACK_FAKE_UPSTREAM 77/77、CLIENT_RENDER_MOCK 55/55、deployment asset tests 11/11、typecheck、lint（0 error/3 既有 warning）、production build（28/28 static pages）与 `git diff --check`。新增单测证明 Nova 的私有 readback canvas 首次 context 以 `willReadFrequently:true` 创建且不读取 renderer-owned context；route/server 与 full-stack 证据证明结构化日志的 `capability` 对 typed text/current frame/已确认 speech-to-draft/解析前拒绝分别为 `text`/`image`/`voice`/`null`。EN/ES 浏览器用例先编辑转写草稿、证明 Send 前零请求，再证明唯一请求只含确认文字与 strict 来源枚举而无 audio/voice/blob；route 测试直接捕获 typed 与 speech 两份 OpenRouter payload，确认 `inputMethod`/`speech-to-draft` 均未上送；非法来源值 422 且 Provider 调用为零。日志字段仍不含 learner/provider 内容。该轮没有真实 OpenRouter 请求、Vercel 写入、stage、commit 或 push。
...
- 没有 reset、stash、clean、切换主工作树分支或 `git add -A`。早期 Nova 修复的受控提交已推送到远端 draft PR #4，并已创建/验收三个 protected Preview（初始空 rollout、八课 master-off surface、G4 L3 text-only live）。随后共享 checkout 被 Owner-authored 外部提交推进到 `eee37e3cc…`；Codex 未创建或推送该提交。本轮最后的日志/readback patch 只修改精确 hunk，index 为空；没有 promote、回滚、Production 环境/alias、Firewall 或 drain 修改，也没有 stage、commit 或 push。
...
- `reports/nova-tutor-pr-ci-gitlink-repair-preflight-2026-08-24.json`（SHA-256 `e88edda86c9c0f604a10f615360ee43ef3b5a852c9137cd82d68b97fdf5363dc`）记录最新 PR checkout 失败、内部 linked worktree 的 27 个待保全条目、local ignore guard、隔离 index 的单路径删除演练以及真实 index/commit/push/provider/Vercel 均为 0；不保存工作树文件内容、diff 正文或凭据。
...
代码实现、早期受控 Git 提交、远端 draft PR、Site workspace CI、本地真实 typed-text canary、空 rollout protected Preview、八课 master-off protected Preview surface、G4 L3 text-only protected Preview live canary，以及 Nova-OFF staged Production rollback candidate 验收已经完成；当前 PR head `eee37e3cc…` 又包含此前 P3/status-only telemetry/K–12 patch，最后的结构化 text/image/voice/null 日志、strict speech-to-draft 来源、canvas readback 与内部验证工作树 ignore guard patch 也已通过本地检查，但最后 patch 未进入 PR，当前 PR head 与最后 patch 均未形成新的已验收 deployment。这仍不等于 Nova 整体发布完成。当前 G4 L3 Preview 文字列车不再是 `BLOCKED`，而是 `PASS_WITH_NON_BLOCKING_TOOLBAR_CSP_WARNING`；当前本地实现单独为 `PASS_LOCAL_PATCH_NOT_IN_VERIFIED_PREVIEW_ARTIFACT`。PR 合并门当前被误入 gitlink 阻断，且必须用保留嵌套工作树内容的父仓库 tree 删除来修复。下一独立课程列车是 G5 L4，必须获得新的明确授权并单独执行 Preview Provider canary；其余课程按既定 A–D 批次继续。图片和语音继续等待独立的 dated Privacy/法律、Owner 与真实设备收据，custom-domain Production 仍是旧版本；Nova-on staged canary、正式域名发布、监控观察窗和回滚演练均未执行。
