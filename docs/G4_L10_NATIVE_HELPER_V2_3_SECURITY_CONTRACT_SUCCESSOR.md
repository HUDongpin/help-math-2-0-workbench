# Grade 4 Lesson 10 native transaction helper v2.3 successor security contract
...
This file is a non-destructive successor to the frozen v2.2 successor
`docs/G4_L10_NATIVE_HELPER_V2_2_SECURITY_CONTRACT_SUCCESSOR.md`, which remains
byte-for-byte preserved at SHA-256
`d7bb8755cbd8fb3a7f4d709d1ec2879f8aee4fa8b8ad4cbacfd7e5068a9eeb5c`.
That direct predecessor incorporates the frozen v2.1 successor
...
All three predecessors are incorporated by those exact hashes. Their
...
`exact Section 0 value` in this successor: it means only the direct v2.2
predecessor file and its exact SHA-256
`d7bb8755cbd8fb3a7f4d709d1ec2879f8aee4fa8b8ad4cbacfd7e5068a9eeb5c`.
The transitive v2.1 and original-v2 identities above are provenance roots and
...
The v2.1 implementation preflight proved that Darwin's positive
`PROC_PIDLISTFDS` size query is a capacity estimate rather than a promise that
the later complete read returns the same byte count. On the selected host/SDK,
a 360-byte positive estimate and a 24-byte aligned complete read containing the
three inherited descriptors is a valid observation. V2.1 incorrectly called
that ordinary result a blocking short ambiguity. This successor replaces only
that enumeration rule with the closed estimate/capacity/actual-count rule in
Section 2. The failed v2.2 Gate-A review proved that the rule also has to be
propagated into external-launcher and fixture evidence. This successor performs
that propagation, removes predecessor-identity ambiguity, and gives itself a
new no-clobber Gate-A companion grammar. It does not weaken the stable two-pass,
exact-three-FD, pipe, direction, handle, self-identity, custody, authority, or
mutation gates.

After this file is frozen, `protocol_spec_sha256` means SHA-256 of the complete
exact bytes of this successor file. This file binds the three predecessor
hashes above, so one successor hash transitively binds all four documents
without a self-hash.
...
`docs/G4_L10_NATIVE_HELPER_V2_2_SECURITY_CONTRACT_SUCCESSOR.md` and the fixed
...
`77c2479d7be197e62a9cf37e05d71d6051858a29167143ca39ddc5be7b994583`.
The exact direct-predecessor bytes bind the intermediate v2.1 digest; no
separately selectable intermediate field exists in this report.
