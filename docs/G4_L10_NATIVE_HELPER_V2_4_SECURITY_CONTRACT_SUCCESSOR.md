# Grade 4 Lesson 10 native transaction helper v2.4 successor security contract
...
This file is a non-destructive successor to the frozen v2.3 successor
`docs/G4_L10_NATIVE_HELPER_V2_3_SECURITY_CONTRACT_SUCCESSOR.md`, which remains
byte-for-byte preserved at SHA-256
`bf0abed59f8db5be0ef83657530bc81cc93d85c9ae466461142c06933e569320`.
That direct predecessor incorporates the frozen v2.2 successor
`docs/G4_L10_NATIVE_HELPER_V2_2_SECURITY_CONTRACT_SUCCESSOR.md` at SHA-256
`d7bb8755cbd8fb3a7f4d709d1ec2879f8aee4fa8b8ad4cbacfd7e5068a9eeb5c`,
which incorporates the frozen v2.1 successor
`docs/G4_L10_NATIVE_HELPER_V2_1_SECURITY_CONTRACT_SUCCESSOR.md` at SHA-256
`170bd54b031f1f6e693f152aef885a509b2d4328f5032cc620a41dcf49a884ab`,
which in turn incorporates
`docs/G4_L10_NATIVE_HELPER_V2_SECURITY_CONTRACT.md`, which also remains
byte-for-byte preserved at SHA-256
`77c2479d7be197e62a9cf37e05d71d6051858a29167143ca39ddc5be7b994583`.
All four predecessors are incorporated by those exact hashes. Their
...
There is exactly one meaning of the singular term `predecessor` in every
canonical field, equality, held-file role, vector, and phrase
`exact Section 0 value` in this successor: it means only the direct v2.3
predecessor file and its exact SHA-256
`bf0abed59f8db5be0ef83657530bc81cc93d85c9ae466461142c06933e569320`.
The transitive v2.2, v2.1, and original-v2 identities above are provenance roots and
never alternate values for a singular `predecessor_contract_sha256` field.
Their exact digests remain bound by the direct predecessor's bytes and by the
separately named Gate-A root-predecessor field in Section 16. This definition
also controls HMG4VC2's singular incorporated-predecessor slot and every
`predecessor-contract` ReviewedObjectMember role; no encoder choice exists.

The v2.3 implementation field-registry extraction proved that six normative
fields were labeled `S64` although the incorporated type registry admitted only
`0x01..0x10`. Two of those fields require the negative value `-25316`. V2.3
assigned no type byte, width, byte order, negative representation, range, or
canonical decode/re-encode rule to `S64`; a strict implementation therefore
could not encode those fields, while guessed U64, new-type, sign-magnitude,
ZigZag, or textual encodings would produce different authority hashes. The
frozen v2.3 Gate-A conclusion is superseded for production implementation by
that independently confirmed P1 finding.

This successor replaces only the affected scalar type closure and every
dependent source-type range/vector rule. Section 1 adds exact type `0x11 S64`,
its six legal sites, and its canonical signed representation. All inherited
PIDLISTFDS, stable two-pass, exact-three-FD, pipe, direction, handle,
self-identity, custody, authority, evidence-DAG, resource, mutation, review,
and no-acceptance gates remain unchanged. The frozen v2.3 contract and review
companion remain immutable historical evidence and are never relabeled as a
passing v2.4 review.

After this file is frozen, `protocol_spec_sha256` means SHA-256 of the complete
exact bytes of this successor file. This file binds the four predecessor
hashes above, so one successor hash transitively binds all five documents
without a self-hash.
...
The existing v2.3 workspace implementation remains bound to the frozen v2.3
successor and is non-authoritative for this revision. No v2.3 source hash,
Gate-A receipt, or passing test is silently relabeled as v2.4 evidence. A new
no-clobber implementation lineage must bind this successor only after its own
Gate-A review reaches `P0/P1/P2 = 0/0/0`.
...
The complete revision-4 type registry extends the original-v2 incorporated
registry `0x01..0x0c` by exactly five type bytes. The direct v2.3 predecessor
already freezes the first four, `0x0d..0x10`; this successor preserves those
four byte-for-byte and adds exactly one new type, `0x11 S64`. Type `0x0d
CAPABILITY_FIXTURE_ROOT_REL_PATH` has 1..128 bytes and is
...
Type `0x11 S64` has exactly eight value bytes and represents one mathematical
integer in the closed interval `-9,223,372,036,854,775,808` through
`9,223,372,036,854,775,807`. The value bytes are the unique 64-bit two's-
complement bit pattern in network/big-endian byte order. To encode a value `x`,
the mathematical unsigned bit value is `x` when `x >= 0` and `2^64 + x` when
`x < 0`; that unsigned value is emitted most-significant byte first. To decode,
the implementation first assembles the eight bytes into one unsigned 64-bit
value `u` using unsigned shifts. If bit 63 is clear the mathematical result is
`u`; if bit 63 is set the mathematical result is `u - 2^64`. An implementation
must compare that mathematical signed result with the field's signed bounds
without a signed overflow, implementation-defined unsigned-to-signed cast,
host-endian load, variable-width sign extension, or normalization. Re-encoding
the decoded result must reproduce all eight input bytes exactly. A seven- or
nine-byte value, unsigned-magnitude interpretation, sign-magnitude, ZigZag,
BER/DER INTEGER, decimal text, redundant prefix, or use of type `0x02 U64` is
invalid for an S64 field.

S64 is legal only at these six exact sites: `8dec
arg_max_sysconf_return`, `61c5
expected_external_representation_error_code`, `8d29 cferror_code`, `9746
number_s32_value`, `8dcf secitem_status`, and `8749 return_value`. It is
forbidden at every other tag. Each field's displayed exact or range constraint
remains conjunctive with the full signed domain: `61c5` and `8d29` are exactly
`-25316`; `8dec` is exactly `1048576`; `9746` is exactly `3072`; `8dcf` is
exactly zero; and `8749` is nonnegative and otherwise bounded by its enclosing
held-FD observation. A U64 at one of these sites, S64 at an adjacent or
unlisted site, and type `0x12` are all unknown/type-site failures before
semantic authority.

Gate B contains byte-exact positive encode/decode/re-encode cases for
`INT64_MIN`, `-25316`, `-1`, `0`, `1`, `3072`, `1048576`, and `INT64_MAX`,
including all six legal sites. It contains one-sided negative cases for 7/9-
byte lengths, U64/S64 substitution, `0x12`, every adjacent site, representative
unlisted sites in every object family, sign-magnitude, ZigZag, decimal, host-
endian, redundant-prefix, unsigned-compare, and each field-specific exact/range
off-by-one. The two independent Gate-B encoders and decoders must derive these
bytes without sharing an implementation of the S64 mapping.

The complete authority field registry is a closed global tag-to-type map, not
merely a predicate that a type byte is known. It is derived from every direct
field declaration in the exact original-v2, v2.1, v2.2, v2.3, and this v2.4
document, in that lineage order, that matches the byte-anchored ASCII regular
expression
`^\x20*(0x[0-9A-Fa-f]{4})\x20+([A-Za-z][A-Za-z0-9_]*)\x20+([A-Z][A-Z0-9_]*)\b`.
The tag is lowercased; every occurrence of one tag must have the same exact
field name and type token; equal triplets are collapsed; any conflicting name
or type is a contract failure. The five documents contain 17,012 matching
occurrences, which collapse to exactly 4,419 unique, nonconflicting tags. The
only type tokens used are the sixteen tokens listed by the count table below.
The canonical registry stream sorts the collapsed rows by numeric 16-bit tag
and emits lowercase tag, field name, and type token separated by one TAB and
terminated by one LF: `tag<TAB>name<TAB>type<LF>`. Its exact byte count is
152,651 and its SHA-256 is
`ef7908aab36cb7a129de0b9dbdaa800be59c72540a7864b8002b05904e81b54e`.

```text
APPROVED_ABS_ROOT_PATH=1
APPROVED_EVIDENCE_REL_PATH=4
BOOL=215
BUILD_REL_PATH=9
BYTES=258
CAPABILITY_FIXTURE_CLAIM_REL_PATH=3
CAPABILITY_FIXTURE_REL_PATH=2
CAPABILITY_FIXTURE_ROOT_REL_PATH=2
LIST=235
POLICY_REL_PATH=3
S64=6
SAFE_CUSTODY_LEAF=14
SHA256=1195
STRUCT=452
U32=1580
U64=440
...
For every declared tag, the registry's one type is the only legal wire type at
that tag; every other type byte fails before semantic parsing. Every undeclared
tag fails for every type byte. A `known_type_byte` helper may be used only for
framing diagnostics and is never a field-site authorization predicate. The
production decoder and both independent Gate-B decoders use an exact compiled
or generated representation of this frozen registry in addition to each
enclosing object schema. Gate B recomputes the registry count, uniqueness,
stream length, digest, and type counts from this exact successor, then
programmatically exhausts all 65,536 tag values against all seventeen legal
type bytes `0x01..0x11`: exactly the 4,419 frozen pairs pass field-site lookup.
This exhaustive predicate test is build evidence and is not limited by the
65,536-case bound of the separately framed golden-vector catalog. Gate B also
proves that no raw-TLV, rule-definition, diagnostic, or dispatcher call graph
can treat a known type byte as sufficient authority for an unknown or
wrongly-typed field.

These five successor types are distinct from `POLICY_REL_PATH`, `SAFE_CUSTODY_LEAF`,
`APPROVED_EVIDENCE_REL_PATH`, and every other path type. All other type bytes
outside the incorporated predecessor registry `0x01..0x0c` and these five
...
SDK-bound `proc_pidinfo(PROC_PIDLISTFDS)` algorithm named
`PIDLISTFDS_CAPACITY_V1`. This exact named algorithm also controls every
external-launcher child pass, external-launcher parent pass, and dropped-child
fixture pass; no caller-specific short-result interpretation is permitted.
One pass performs a positive
size query. That query is a capacity estimate, not an exact later-return count.
The estimate must be a multiple of frozen `sizeof(struct proc_fdinfo)` and must
represent no more than 65,536 records. The helper allocates by checked addition
exactly one extra record and reads into that larger buffer. A positive read
return strictly less than the full enlarged capacity and divisible by the
record size is the actual returned record list; it may be shorter than the
positive size-query estimate and is not a short ambiguity. A read return not
divisible by the record size, equal to the full enlarged capacity, larger than
capacity, or containing a duplicate FD means growth/truncation and restarts the
complete size-query/read pass. A negative or zero query or read return, an
overflow, or allocation failure blocks. At most eight complete passes are
...
Every evidence-producing caller serializes one complete
`FDEnumerationObservation` for each successful pass. It includes every attempt
started for that pass, including retry attempts, and binds the positive query
estimate, checked enlarged capacity, positive read return, captured byte count,
canonical selected-field `(proc_fd,proc_fdtype)` projections, remainder,
duplicate result, disposition, terminal record count, and SDK layout. It never
serializes a raw SDK aggregate, padding, partial trailing record, or unused
buffer-tail byte. An attempt at which query/read is nonpositive, query is
misaligned/over-limit, addition overflows, or allocation fails produces no
successful pass and cannot be hidden inside a passing observation. A passing
observation has one terminal last attempt and only the three enumerated retry
reasons before it. The helper performs the identical state machine before any
wire read; its Gate-B implementation evidence and vectors bind the same fields
even though no pre-header authority object is emitted on the live wire.
...
LauncherChildFDInventoryPass
  ...
  0x95b9 fd_enumeration_observation STRUCT FDEnumerationObservation,
                                           terminal record count/list equal
                                           `95b4/95b5`
...
LauncherParentFDInventoryPass
  ...
  0x98cc fd_enumeration_observation STRUCT FDEnumerationObservation,
                                           terminal record count/list equal
                                           `98c5/98c6`
...
ProspectivePolicyHoleMember
  ...
  0x8fa5 leaf_source_type           U32, exact wire type byte `0x01..0x11`
...
`proc_pidinfo(PROC_PIDLISTFDS)` enumeration in the single-threaded child using
Section-2 `PIDLISTFDS_CAPACITY_V1`. A positive aligned actual return below the
enlarged capacity is complete even when shorter than the positive size-query
estimate. `95b9` contains the complete per-attempt canonical witness and its
terminal selected-field projections decode/sort to exact `95b5`. A nonpositive query/read,
misaligned or over-limit query, overflow/allocation failure, enumeration error,
or FD-mutating call other than the recorded `dup2`/`close` sequence between
passes blocks. Full/over-capacity return, nonmultiple actual return, or duplicate
FD consumes one of the same eight bounded attempts and restarts the complete
query/read pair; exhausted attempts block.
...
Each parent pass uses Section-2 `PIDLISTFDS_CAPACITY_V1`, including the same
estimate/capacity/actual-return decision table and eight-attempt cap, and
recomputes kind 184 over FD-number-sorted records. `98cc` contains the complete
per-attempt witness and terminal selected-field projections that decode/sort to
exact `98c6`.
Phase 1
...
FixtureChildFDRecord
  ...
  0x825c proc_fdtype                 U32, exact public `proc_fdinfo.proc_fdtype`;
                                         SDK `PROX_FDTYPE_PIPE` exactly when
                                         object kind 1, otherwise a type accepted
                                         only by the successful vnode inspection
...
ProcPIDListFDSAttemptObservation
  0x8243 ordinal                     U32, contiguous from zero
  0x8244 proc_fdinfo_record_size     U32, exact selected SDK selector-0 size
  0x8245 size_query_returned_byte_count U32, positive exact signed-return bits,
                                         divisible by `8244`
  0x8246 query_record_capacity       U32, exactly `8245 / 8244`, 1..65,536
  0x8247 enlarged_buffer_capacity_bytes U64, checked `8245 + 8244`
  0x8248 read_returned_byte_count    U32, positive exact signed-return bits
  0x8249 captured_buffer_prefix_byte_count U64, exactly the lesser of `8248`
                                         and `8247`
  0x824a decoded_complete_record_count U32, exactly floor(`8249 / 8244`)
  0x824b read_record_remainder       U32, exactly `8248 % 8244`
  0x824c duplicate_fd_detected       BOOL; evaluated only when `8248 < 8247`
                                         and `824b=0`, then true iff two decoded
                                         records have equal `8262`; otherwise
                                         exactly false
  0x824d disposition                 U32: 1 retry capacity-full-or-larger,
                                         2 retry nonmultiple, 3 retry duplicate,
                                         4 terminal complete
  0x824e actual_record_count         U32, zero dispositions 1..3; exactly
                                         `8248 / 8244`, 1..65,536 disposition 4
  0x824f result                      U32, exactly 1 observed call pair
  0x8260 decoded_records             LIST ProcFDInfoObservation, exact `824a`

ProcFDInfoObservation
  0x8261 ordinal                     U32, contiguous from zero after canonical
                                         `(8262,8263)` unsigned sort
  0x8262 proc_fd_bits                U32, exact width-preserving unsigned bits of
                                         signed `proc_fdinfo.proc_fd`
  0x8263 proc_fdtype                 U32, exact `proc_fdinfo.proc_fdtype`

FDEnumerationObservation
  0x8250 observation_version         U32, exactly 1
  0x8251 attempt_count               U32, 1..8
  0x8252 attempts                    LIST ProcPIDListFDSAttemptObservation,
                                         exact count and contiguous ordinals
  0x8253 terminal_attempt_ordinal    U32, exactly `8251 - 1`
  0x8254 terminal_query_estimate_bytes U32, equal terminal `8245`
  0x8255 terminal_enlarged_capacity_bytes U64, equal terminal `8247`
  0x8256 terminal_actual_returned_bytes U32, equal terminal `8248`
  0x8257 terminal_record_count       U32, equal terminal `824e` and enclosing
                                         pass record count
  0x8258 terminal_proc_fdinfo_projection_sha256 SHA256, SHA-256 of exact
                                         canonical LIST value bytes of terminal
                                         `8260`
  0x8259 fd_transport_layout_binding_set_sha256 SHA256, equal SDKIdentity `4f39`
  0x825a result                      U32, exactly 1
  0x825b context_attempt_ordinal_start U32
  0x825d context_attempt_ordinal_end_exclusive U32, checked `825b + 8251`,
                                         greater than `825b` and at most 8

The `FDEnumerationObservation` STRUCT is legal at exactly fields `80f8`,
`95b9`, and `98cc`; the attempt STRUCT is legal only as a member of `8252`,
and `ProcFDInfoObservation` is legal only as a member of `8260`.
Every attempt uses the same SDK record size and layout hash. Disposition has
closed precedence: `8248 >= 8247` selects 1; otherwise nonzero `824b` selects
2; otherwise `824c=true` selects 3; otherwise it selects terminal disposition
4. Dispositions 1..3 occur only before the final member, and disposition 4
occurs exactly once as the final member. Thus a full/over-capacity return cannot
be relabeled as a harmless remainder or terminal list, and a duplicate cannot
be hidden by sorting.

For every attempt, `8260` contains only the two selected public fields of each
complete record wholly inside the `8249`-byte captured prefix; it contains no
raw aggregate, padding, partial record, or buffer tail. Every attempt sorts all
members by unsigned `(8262,8263)` and then assigns contiguous `8261`; retaining
kernel-buffer order or choosing another order is invalid. `824c` is recomputed
from that sorted projection using equality of FD bits alone. `824a` may be
nonzero for a retry, but only terminal disposition makes it an admitted FD
list. For the terminal attempt, `8249==8248<8247`, `824b=0`, `824c=false`, and
`824a==824e`. The terminal `8260` members equal the enclosing pass list
one-for-one. At `95b9` and `98cc`, projected
`proc_fd/proc_fdtype` equal the enclosing record's FD and `95c4/98a4`. At
`80f8`, they equal `80d2/825c`. The normalized per-FD inspection fields remain
separately mandatory; the selected-field projection does not classify a
descriptor by itself.

Attempt budgets are context-global rather than per-observation loopholes. For
the helper's two consecutive stable startup passes, the first observation has
`825b=0`, the second `825b` equals the first `825d`, and the second `825d<=8`.
The two byte-identical fixture passes in one `7fc8` set obey the same chained
ordinal and shared-eight-attempt rule. Launcher phase-1 and phase-2 inventories
are distinct state snapshots rather than a stable pair; each snapshot starts
at `825b=0` and ends at `825d<=8`. No context can concatenate two separately
reset observations and call them one stable-pair admission.

Gate B has a separate vector family for each applicable producer domain:
production-helper startup, external-launcher child, external-launcher parent,
and dropped-child fixture. Each family includes a positive aligned actual
return below a larger estimate; the exact `360`-byte estimate, `368`-byte
enlarged capacity, and `24`-byte three-record actual return is mandatory in
every domain/phase where three records are legal. Other phases use the same
`360`/`368` estimate/capacity with their exact valid aligned record count.
Each family independently covers negative and zero query, nonmultiple query,
query count 65,536 and 65,537, checked-add overflow, allocation failure,
negative and zero read, nonmultiple read, read equal to enlarged capacity,
read greater than capacity, duplicate FD, a fourth FD both within capacity and
at the full-capacity seam, all eight attempts retrying, and drift between two
otherwise successful passes. Positive vectors prove exact captured-count and
selected-field-projection equality plus terminal-list equality; negative
vectors prove no passing observation,
authority object, response, or diagnostic is admitted merely because a partial
prefix was available.
...
FixtureChildFDInventoryPass
  ...
  0x80f8 fd_enumeration_observation  STRUCT FDEnumerationObservation,
                                         terminal record count/list equal
                                         `80f4/80f5`
...
95 dropped-child stable FD-inventory pass set
   ...
   kind-92 hashes, and the complete `80f8` enumeration observation, the
   remaining projection is byte-identical; both non-omitted complete `80f8`
   observations remain mandatory, each terminal `8260` equals the common
   normalized `80f5` list, and their `825b/825d` values form exactly one
   contiguous shared attempt sequence starting at zero and ending at most eight
...
code is exactly the predecessor/successor one-byte wire type `0x01..0x11`
...
all of its FD slots twice with Section-2 `PIDLISTFDS_CAPACITY_V1`. A positive
aligned actual return below the enlarged capacity is complete even when shorter
than the positive size-query estimate. Each pass's `80f8` serializes all
attempts and its terminal selected-field projections decode/sort to exact `80f5`, including
exact raw `proc_fdtype` equality with each record's `825c`.
...
field offsets, and descriptor-type constants are SDK ABI/layout inputs; the
query/read return and retry rules are only the named contract algorithm, not an
SDK-defined caller choice. A nonpositive query/read, query misalignment/limit,
overflow/allocation failure, unsupported descriptor type, short type-specific
structure, inspection error, count drift, exhausted retry, pipe-handle
collision, or identity/status drift blocks before the denial syscall. A
full/over-capacity return, nonmultiple actual return, or duplicate FD restarts
the complete query/read pair within the eight-attempt bound.
...
The Gate-A finding union must retain and independently confirm remediation of
at least these predecessor findings: P1 `V23-S64-TLV-ENCODING-UNDEFINED`, P1
`V23-GLOBAL-TAG-TYPE-REGISTRY-NOT-FROZEN`, and P2
`V23-DIRECT-PREDECESSOR-TYPE-LINEAGE-STALE`. They are historical findings with
`disposition=remediated`, not open v2.4 findings: Section 1 supplies the exact
S64 representation and six-site rule, the hash-bound 4,419-entry lineage-wide
field registry and exhaustive Gate-B obligation, and corrected direct-v2.3
lineage wording. Their `first-reviewer-confirmed-remediation-sha256` is the
frozen v2.4 successor digest only after a reviewer actually confirms all three
against byte 1 through EOF. Any reviewer disagreement, registry recomputation
mismatch, unreviewed affected projection, or newly open P0/P1/P2 keeps Gate A
failed; no author may self-attest it away.

The Gate-A companion is exactly
`docs/G4_L10_NATIVE_HELPER_V2_4_SECURITY_CONTRACT_SUCCESSOR_INDEPENDENT_REVIEW.md`.
...
sequence; literal punctuation and key order are part of format version 2:

```text
# G4 L10 Native Helper v2.4 Successor Independent Review

## Frozen identity
format-version=2
successor-sha256=<64-lowercase-hex>
direct-predecessor-sha256=<64-lowercase-hex>
root-predecessor-sha256=<64-lowercase-hex>
...
encoding, that value is also its line count. `direct-predecessor-sha256` equals
both the live SHA-256 of
`docs/G4_L10_NATIVE_HELPER_V2_3_SECURITY_CONTRACT_SUCCESSOR.md` and the fixed
direct-predecessor identity
`bf0abed59f8db5be0ef83657530bc81cc93d85c9ae466461142c06933e569320`.
`root-predecessor-sha256` equals both the live SHA-256 of
`docs/G4_L10_NATIVE_HELPER_V2_SECURITY_CONTRACT.md` and the fixed root identity
`77c2479d7be197e62a9cf37e05d71d6051858a29167143ca39ddc5be7b994583`.
The exact direct-predecessor bytes bind the transitive v2.2 and v2.1 digests;
no separately selectable transitive-predecessor field exists in this report.
...
ASCII bytes `HMG4GAB2`, big-endian U32 version 2, the raw 32-byte successor
digest, the raw 32-byte direct-predecessor digest, the raw 32-byte root-
predecessor digest, big-endian U64 successor byte count, big-endian U64
successor LF count, big-endian U32 unit count, then, for
...
different unit set, order, frozen file, direct predecessor, or root predecessor
is invalid. The direct digest is the sole value used by every canonical
singular `predecessor_contract_sha256` field; the root digest is lineage
evidence only. Omitting, swapping, or reinterpreting either field is invalid.
...
complete token set. Additional units are forbidden in format version 2, so
