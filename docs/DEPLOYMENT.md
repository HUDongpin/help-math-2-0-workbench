...
All CI jobs install from the root lockfile with Node 24. `CI/CD service identity` runs the secretless identity tests and verifier plus the exact deployment-asset closure from a clean Git checkout. `Site workspace` runs lint, type-check, the deployment-safe `npm run test:deployment` runner, and build for `@helpmath/web`; that runner excludes only the top-level `tests/g4-l3-lesson-navigation.test.ts` module, whose import requires a non-deployment report, then uses Node 24's exact anchored `--test-skip-pattern` for seven named source-bound tests. The other tests in those six modules remain in the deployment-safe set, including their security, route, unit, and asset checks. After Chromium installation, Site invokes exactly `npm run test:e2e:deployment`: its runner uses only `site.spec.ts`, `prototype-acceptance.spec.ts`, `modern-wide-geometry.spec.ts`, `canvas-sharpness.spec.ts`, and `learning-experience-polish.spec.ts`, with candidate profile disabled, all eight course flags enabled, deterministic local Nova enabled, two workers, and an exact `--grep-invert` for the four diagnostic/demo names plus current candidate-evidence/design-only names. The separate G5 L4 audio, modern-wide aggregate, and Nova full-stack commands remain available for their own authorized contexts and are not rerun in this clean deployment job. `Workbench` retains `npm run verify:workbench` and `npm test` for a separately authorized source-complete environment, but is not a required check for this Git-hosted site-release path and is not a public-site release or lesson-publication claim.
