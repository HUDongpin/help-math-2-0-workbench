...
(() => {
  'use strict';
  const animationId = "course-g04-l09-fq-002";
  ...
  const asset = Object.freeze({
    ...
    render(canvas, request) {
      ...
      const frame = Math.max(1, Math.min(80, Number(request.frame) || 1));
      const seed = Number.isSafeInteger(request.seed) ? request.seed : 0;
      context.clearRect(0, 0, 800, 600);
      context.fillStyle = colors[Math.abs(seed + 42) % colors.length];
