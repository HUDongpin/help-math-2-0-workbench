...
const selectedOccurrences = [1, 2, 12, 16, 22, 23, 24, 27, 28, 29, 30, 32, 33, 40, 41, 42];

test('builds the exact source-ordered 43-page private descriptor with 16 admitted pages', () => {
  ...
  assert.equal(descriptor.productBridge.registeredAnimationCount, 16);
  ...
  const occurrence29 = descriptor.pages[28]!;
  assert.equal(occurrence29.placementId, 'g04-l09-placement-029');
  assert.equal(occurrence29.animationId, 'course-g04-l09-ti-004');
  assert.deepEqual(occurrence29.rendererAvailability, {
    kind: 'registered',
    moduleKey: 'course-g04-l09-ti-004',
    runtimeQuery: {
