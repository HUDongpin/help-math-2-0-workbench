...
const CURRENT_JS_PAGES = [1, 0, 39, 0, 0, 0, 0, 0, 1, 0, 0, 0];
...
test('Grade 4 catalog coverage records the reviewed active-source promotion without expanding acceptance', () => {
  ...
  assert.deepEqual(coverage.summary, {
    ...
    currentJsPageCount: 41,
    missingCurrentJsPageCount: 604,
    currentJsShellCount: 2,
    missingCurrentJsShellCount: 10,
    currentJsMemberCount: 43,
    missingCurrentJsMemberCount: 614,
...
test('registry changes update coverage counts without fabricating source or acceptance', () => {
  ...
  assert.equal(withoutOneRenderer.summary.currentJsPageCount, 40);
  assert.equal(withoutOneRenderer.summary.missingCurrentJsPageCount, 605);
  assert.equal(
