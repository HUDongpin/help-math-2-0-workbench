...
test('argument parsing is exact and mode-only', () => {
  ...
  assert.equal(DEFAULT_PORT, 3233);
  assert.equal(
    SMOKE_REPORT_PATH.endsWith(
      '/reports/g5-l4-whole-lesson-package-mvp-v7-smoke.json',
    ),
    true,
  );
  assert.equal(
    SMOKE_SCREENSHOT_ROOT.endsWith(
      '/output/playwright/g5-l4-whole-lesson-package-mvp-v7',
    ),
    true,
  );
  assert.equal(
    SOURCE_INPUTS.includes(
...
test('immutable package targets must be absent before any write', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'g5-l4-v7-target-test-'));
  try {
    const target = path.join(root, 'receipt.json');
    assert.equal(await assertPathAbsent(target, 'receipt.json'), true);
    await writeFile(target, '{}\n');
    await assert.rejects(
      assertPathAbsent(target, 'receipt.json'),
      /Immutable target already exists: receipt\.json/,
    );
  } finally {
...
test('README uses a genuinely alternate port when the v7 default is busy', () => {
  const source = readmeText();
  assert.match(source, /--port 3234/);
  assert.doesNotMatch(source, /--port 3233/);
});
