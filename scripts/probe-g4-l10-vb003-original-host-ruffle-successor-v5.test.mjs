...
} from "./probe-g4-l10-vb003-original-host-ruffle-successor-v5.mjs";

const ORIGIN = "http://127.0.0.1:43123";
const RESULT_PATH = `${SUCCESSOR_V5_OUTPUT_RELATIVE}/diagnostic.json`;
const V2_PATH =
...
const SCRIPT_PATH =
  "scripts/probe-g4-l10-vb003-original-host-ruffle-successor-v5.mjs";

function sha256(value) {
  return createHash("sha256").update(value).digest("hex");
}

test("v5 orders two fresh-context attempts without mixing causal axes", () => {
...
test("checked-in v5 preserves the modal-intercepted input failure without rewriting v4", async () => {
  const [resultBytes, v2Bytes, v3Bytes, v4Bytes, scriptBytes] = await Promise.all([
  ...
  const [resultMetadata, outputMetadata] = await Promise.all([
    stat(RESULT_PATH),
    stat(SUCCESSOR_V5_OUTPUT_RELATIVE),
  ]);
