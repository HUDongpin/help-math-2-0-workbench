...
const animationId = "shell-course-g05-l04-index-local";
...
async function buildExpectedManifest() {
  ...
  for (let index = 0; index < expectedFrameCount; index += 1) {
    ...
    assert(record.width === dimensions.width, `frame ${frame} report width mismatch`);
    assert(record.height === dimensions.height, `frame ${frame} report height mismatch`);
    frames.push({
      sourcePath,
      output: {
        frame,
...
export async function inspectG5L4ShellRootFrameAssets() {
  const expected = await buildExpectedManifest();
  return expected.manifest;
}

export async function buildG5L4ShellRootFrameAssets({check = false} = {}) {
  const expected = await buildExpectedManifest();
...
if (process.argv[1] && path.resolve(process.argv[1]) === scriptPath) {
  ...
  const manifest = await buildG5L4ShellRootFrameAssets(options);
  console.log(
