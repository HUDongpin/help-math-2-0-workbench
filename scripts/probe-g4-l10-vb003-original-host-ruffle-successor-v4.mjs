...
export const SUCCESSOR_V4_OUTPUT_RELATIVE =
  "output/playwright/g4-l10-vb003-original-host-ruffle-successor-v4";
const RESULT_RELATIVE = `${SUCCESSOR_V4_OUTPUT_RELATIVE}/diagnostic.json`;
...
function invariant(condition, message) {
  if (!condition) {
    throw new Error(`G4 L10 VB003 Ruffle successor v4: ${message}`);
  }
}
...
export function buildInputAttemptPlans() {
  return [
    {
      attempt: 1,
      id: "fresh-context-window-capture-staged-release-v2-network-policy",
    ...
    {
      attempt: 2,
      id: "fresh-context-window-capture-staged-release-with-exact-xml",
      transport: "page.mouse move/down/up",
      freshContext: true,
...
export function analyzeDomInputEvents(events, cssHitBounds = null) {
  ...
  const pointerEvents = windowCaptureEvents.length > 0
    ? windowCaptureEvents
    : events.filter((event) =>
  ...
  return {
    ...
    observerTarget: windowCaptureEvents.length > 0 ? "window" : "legacy-capture",
...
function launchSources() {
  ...
  const launch = Buffer.from([
    ...
    "    const eventTypes = ['pointerenter', 'pointermove', 'pointerdown', 'pointerup', 'pointercancel', 'mousedown', 'mouseup', 'click'];",
    "    const record = (event) => {",
    ...
    '    player.id = "player";',
    '    player.setAttribute("aria-label", "L10 original-host Ruffle successor v4 forensic diagnostic");',
  ...
  const html = Buffer.from([
    "<!doctype html>",
    '<html lang="en"><head><meta charset="utf-8"><title>L10 contained Ruffle successor v4</title>',
...
async function bindInputs() {
  ...
  const [staticReport, v2Result, v3Result] = await Promise.all([
...
async function createOutput(inputs) {
  const outputRoot = path.join(PROJECT_ROOT, SUCCESSOR_V4_OUTPUT_RELATIVE);
  invariant(
    !(await pathExists(outputRoot)),
    `${SUCCESSOR_V4_OUTPUT_RELATIVE} already exists; immutable output will not be overwritten`,
  );
...
async function performSourcePointRelease(
  ...
  const eventStartIndex = await page.evaluate(() =>
    window.__probe.domInputEvents.length);
  const phases = [];
  ...
  await page.mouse.move(inertViewportPoint.x, inertViewportPoint.y);
  ...
  const domEvents = await page.evaluate((startIndex) =>
    window.__probe.domInputEvents.slice(startIndex), eventStartIndex);
  ...
  return {
    ...
    domEvents,
    domSequence,
    phaseScreenshots: [hoverScreenshot, downScreenshot],
  };
}
...
export async function runSuccessorV4Probe() {
  ...
  const [staticAfter, v2After, v3After] = await Promise.all([
    readBinding(STATIC_REPORT_RELATIVE),
    readBinding(V2_RESULT_RELATIVE),
    readBinding(V3_RESULT_RELATIVE),
  ]);
  ...
  invariant(
    ...
    "an immutable predecessor or canonical XML changed during v4",
  );
  ...
  const result = {
    ...
    status: topLevelFatalError || attempts.every((attempt) => attempt.fatalError)
      ...
      : firstSuccessorAttempt
        ...
        : completeDomReleaseObserved
          ? "complete-window-capture-dom-release-observed-rw002-not-requested-in-ruffle-forensic-only"
    ...
    probe: {
      ...
      browserAutomation:
        "real Chromium via repository-pinned Playwright; window capture is installed before Ruffle player creation, followed by the same staged trusted pointer transport in two bounded fresh-context network-policy variants at most",
    },
    lineage: {
      staticAntecedent: {
      ...
      predecessorV3: {
        ...
        repairInV4:
          "install a window capture observer before Ruffle player creation so the event is recorded before any target listener can stop propagation",
      },
      keytermXml: {
        source: {
          path: inputs.xml.path,
    ...
    hypotheses: {
      ...
      domEventLayer:
        "trusted pointer and mouse phases are recorded first at window capture before the event reaches Ruffle, with post-load shadow-canvas listeners retained only as supporting instrumentation",
