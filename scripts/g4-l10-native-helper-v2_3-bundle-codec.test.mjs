...
async function privateRoot(label) {
  const root = await mkdtemp(path.join(tmpdir(), `hmg4v23-bundle-${label}-`));
...
test("HMG4B2 codec binds exact v2.3 contract, review, and source preimages", async () => {
...
test("HMG4B2 object has a closed symbol surface and reproducible bytes", {
  ...
  assert.deepEqual(undefinedResult.stdout.trim().split("\n").sort(), [
    ...
    "_hmg4v23_checked_add_size", "_hmg4v23_policy_rel_path_is_lexically_safe",
    "_hmg4v23_read_u32_be", "_hmg4v23_read_u64_be",
    "_hmg4v23_tlv_cursor_init", "_hmg4v23_tlv_next_raw", "_memcmp", "_memcpy",
  ].sort());
