...
import {
  ...
  materializeG5L4PreRuntimeSpecificationCandidates,
  parseArguments,
...
async function withTemporaryRoot(callback) {
  const root = await mkdtemp(path.join(tmpdir(), "g5-l4-candidates-"));
  try {
...
test("static reconciliation transition is exactly 0 or 55 receipts", () => {
  assert.equal(
    classifyStaticReconciliationReceiptState(Array(55).fill(false)),
    "pre-adoption",
  );
  assert.equal(
    classifyStaticReconciliationReceiptState(Array(55).fill(true)),
    "historical-post-adoption",
  );
  const partial = Array(55).fill(false);
  partial[54] = true;
  assert.throws(
...
test("historical candidate descriptors reject post-adoption tampering", () => {
  const member = {animationId: "course-g05-l04-test"};
  const expected = {
    path:
      "migrations/course-g05-l04-test/audit/machine/candidate.json",
    bytes: 8,
    sha256: "a".repeat(64),
  };
  assert.equal(
...
test("real first member and shell FFDec bundles match machine counts", async () => {
  const cases = [
    ["course-g05-l04-ir-001-a662633d", 20, []],
    [
...
test("real post-adoption full release verifies 55 immutable historical packages", async () => {
  const dryRunResult =
    await materializeG5L4PreRuntimeSpecificationCandidates({
      root: projectRoot,
      dryRun: true,
    });
  const checkResult =
  ...
  for (const result of [dryRunResult, checkResult]) {
    assert.ok(
      ["dry-run-historical", "checked-historical"].includes(result.mode),
    );
    assert.deepEqual(result.summary, {
    memberCount: 55,
    outputCount: 385,
    definitionCount: 12066,
    scriptCount: 2332,
    ...
    assert.ok(
      result.members.every(
        ({
          outputCount,
          historicalCandidatePackage,
          staticReconciliationReceipt,
        }) =>
          outputCount === 7 &&
          historicalCandidatePackage === true &&
          staticReconciliationReceipt.endsWith(
            "g5-l4-m1-static-reconciliation-receipt.json",
          ),
      ),
    );
    assert.ok(
      Object.values(result.acceptanceEffects).every((value) => !value),
    );
  }
...
test("foreign or acceptance-promoting prior receipt is rejected", () => {
  ...
  assert.throws(
    () => validatePriorReceipt({
      ...
      generatedBy: {
        path: "scripts/materialize-g5-l4-pre-runtime-specification-candidates.mjs",
        sha256: "b".repeat(64),
      },
      ownership: {
  ...
  await withTemporaryRoot(async (root) => {
    const outputPaths = {
      manifestRuntimeFacts:
        "migrations/a/audit/machine/g5-l4-pre-runtime-manifest-runtime-facts-candidate.json",
      receipt:
        "migrations/a/audit/machine/g5-l4-pre-runtime-specification-candidate-receipt.json",
    };
    await writeFile(
