...
export const SELF = `${CANONICAL_ROOT}/scripts/g4-l10-native-helper-v2_17-review-verifier.mjs`;
const TARGET = `${CANONICAL_ROOT}/docs/G4_L10_NATIVE_HELPER_V2_17_SECURITY_CONTRACT_SUCCESSOR.md`;
const PROTOCOL = `${CANONICAL_ROOT}/docs/G4_L10_NATIVE_HELPER_V2_17_REVIEW_PROTOCOL_SUCCESSOR.md`;
const FOCUSED_TEST = `${CANONICAL_ROOT}/scripts/g4-l10-native-helper-v2_17-review-verifier.test.mjs`;
...
const EXPECTED = Object.freeze({
  target: {
    ...
    finalLf: true,
    mode: "0444",
    sha256: "bbeb9bfb7a436e6144026b18b8c3629af192a0cf035f87bd0de26484bf346ef3",
  },
  protocol: {
    bytes: 15452,
    lfCount: 317,
    finalLf: true,
    mode: "0644",
    sha256: "7d4fd2861d53f57c1d1ee06b006784fbf1933739a92ec04733c4364723460f44",
  },
...
const PROTOCOL_MARKERS = Object.freeze([
  "Exact canonical JSON domain and physical bytes",
  "Exact ordered fixed-input closure",
  "Descriptor-relative receipt creation",
  "Retained-buffer syntax and focused-test execution",
  "v2.12 Section 6 is explicitly retired",
...
export function addReceiptId(kind, value) {
  const receiptId = sha256(Buffer.from(`G4L10-V217-${kind}\n${canonicalJson(value)}\n`, "utf8"));
...
export function computeReviewSetDigest(value) {
  ...
  return sha256(Buffer.from(`G4L10-V217-REVIEW-SET\n${canonicalJson(body)}\n`, "utf8"));
}
...
async function assertOutputParentStillHeld(custody) {
  const heldStat = await custody.handle.stat({ bigint: true });
  const pathStat = await lstat(custody.parent, { bigint: true });
  const held = stableStatProjection(heldStat);
  const byPath = stableStatProjection(pathStat);
  const resolved = await realpath(custody.parent);
  invariant(held.directory && byPath.directory, "receipt parent is no longer a directory");
  invariant(sameObject(held, byPath), "receipt parent pathname detached from retained descriptor", {
    held,
    byPath,
  });
  invariant(resolved === custody.physicalParent, "receipt parent realpath changed", {
    expected: custody.physicalParent,
    actual: resolved,
  });
  invariant(Number(heldStat.uid) === process.getuid() && portableMode(heldStat) === "0700", "retained receipt parent custody changed");
  return held;
}
...
async function openOutputCustody(output, expectedParent = null) {
  ...
  invariant(Number.isInteger(fsConstants.O_DIRECTORY) && Number.isInteger(fsConstants.O_NOFOLLOW), "required directory flags are unavailable");
  const handle = await open(parent, fsConstants.O_RDONLY | fsConstants.O_DIRECTORY | fsConstants.O_NOFOLLOW);
  const custody = { output, parent, leaf, physicalParent, handle };
  try {
    const held = await assertOutputParentStillHeld(custody);
    if (expectedParent) {
      invariant(
        parent === expectedParent.parent
          && physicalParent === expectedParent.physicalParent
          && held.dev === expectedParent.dev
          && held.ino === expectedParent.ino,
        "receipt parent differs from authenticated attempt binding",
        { expectedParent, actual: { parent, physicalParent, dev: held.dev, ino: held.ino } },
      );
    }
    return custody;
  } catch (error) {
...
async function safePhysicalTmpOutput(output, expectedParent = null) {
  ...
  try {
    ...
    return {
      ...
      physicalParent: custody.physicalParent,
      dev: held.dev,
      ino: held.ino,
    };
  } finally {
...
export async function assertAttemptLeavesUnused(successOutput, errorOutput, expectedParent = null) {
  ...
  try {
    invariant(await pathIsAbsent(successOutput), "attempt success leaf is already occupied", { successOutput });
    invariant(await pathIsAbsent(errorOutput), "attempt error leaf is already occupied", { errorOutput });
    await assertOutputParentStillHeld(success);
    await assertOutputParentStillHeld(error);
    return true;
  } finally {
...
const OPENAT_WRITER = String.raw`import hashlib,json,os,stat,sys
leaf=sys.argv[1]
...
try:
    ...
    if not stat.S_ISREG(st.st_mode) or st.st_uid != os.getuid() or st.st_nlink != 1 or stat.S_IMODE(st.st_mode) != 0o600:
        raise OSError("created receipt metadata mismatch")
    if bytes(actual) != data:
        raise OSError("created receipt bytes mismatch")
    result={"bytes":st.st_size,"dev":str(st.st_dev),"gid":st.st_gid,"ino":str(st.st_ino),"mode":"0600","nlink":st.st_nlink,"sha256":hashlib.sha256(actual).hexdigest(),"uid":st.st_uid}
finally:
    os.close(fd)
os.fsync(3)
...
async function verifySystemWriter() {
  ...
  const handle = await open(SYSTEM_WRITER, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
  try {
    const before = await handle.stat({ bigint: true });
    const byPath = await lstat(SYSTEM_WRITER, { bigint: true });
    invariant(before.isFile() && byPath.isFile() && sameObject(stableStatProjection(before), stableStatProjection(byPath)), "system writer path/descriptor identity mismatch");
    const bytes = await handle.readFile();
    const after = await handle.stat({ bigint: true });
    invariant(sameStableStat(stableStatProjection(before), stableStatProjection(after)), "system writer changed while hashing");
    const actual = {
      bytes: bytes.length,
    ...
    invariant(canonicalJson(actual) === canonicalJson(EXPECTED.systemWriter), "system writer identity mismatch", {
...
export async function writeNoClobberReceipt(output, value, options = {}) {
  const bytes = receiptBytes(value);
  const custody = await openOutputCustody(output, options.expectedParent ?? null);
  try {
    await verifySystemWriter();
    if (options.afterParentOpen) await options.afterParentOpen({ ...custody, handle: custody.handle });
    await assertOutputParentStillHeld(custody);
    const child = spawnSync(SYSTEM_WRITER, ["-c", OPENAT_WRITER, custody.leaf], {
      input: bytes,
      stdio: ["pipe", "pipe", "pipe", custody.handle.fd],
      timeout: 60000,
      maxBuffer: 4 * 1024 * 1024,
    });
    invariant(!child.error && child.status === 0 && child.signal === null, "descriptor-relative receipt writer failed", {
      status: child.status,
      signal: child.signal,
      error: child.error ? summarizeError(child.error) : null,
      stderr: Buffer.isBuffer(child.stderr) ? child.stderr.toString("utf8").slice(0, 2000) : "",
    });
    if (options.afterCreate) await options.afterCreate({ ...custody, handle: custody.handle });
    let writerBinding;
    try {
      writerBinding = JSON.parse(Buffer.from(child.stdout).toString("utf8"));
    } catch (error) {
      throw new EvidenceInputMismatch("descriptor-relative receipt writer returned invalid metadata", summarizeError(error));
    }
    ...
    invariant(pathStat.isFile(), "created receipt pathname is not a regular file");
    invariant(canonicalJson(actual) === canonicalJson(writerBinding), "created receipt pathname differs from writer descriptor", {
      writerBinding,
      actual,
    });
    invariant(actual.bytes === bytes.length && actual.sha256 === sha256(bytes), "created receipt byte binding mismatch");
    invariant(actual.uid === process.getuid() && actual.nlink === 1 && actual.mode === "0600", "created receipt custody mismatch", actual);
    await assertOutputParentStillHeld(custody);
    await verifySystemWriter();
    return actual;
  } finally {
...
export function validateReviewSetManifestShape(value) {
  ...
  invariant(value.artifactType === "g4-l10-native-helper-v2-17-authenticated-review-set", "review-set artifactType mismatch");
  invariant(value.authority === "correlation-only-never-self-authorizing", "review-set authority must remain correlation-only");
  invariant(value.protocolVersion === "v2.17", "review-set protocolVersion mismatch");
...
async function loadReviewSet(options, heldHandles) {
  ...
  const successParent = await safePhysicalTmpOutput(options.successOutput);
  const errorParent = await safePhysicalTmpOutput(options.errorOutput);
  invariant(
    successParent.parent === errorParent.parent
      && successParent.physicalParent === errorParent.physicalParent
      && successParent.dev === errorParent.dev
      && successParent.ino === errorParent.ino,
    "attempt output leaves do not share one authenticated physical parent",
  ...
  const reviewSet = {
    ...
    outputParentBinding: successParent,
  };
  try {
    await assertAttemptLeavesUnused(options.successOutput, options.errorOutput, successParent);
    return reviewSet;
  } catch (error) {
    attachAuthenticatedBinding(
      error,
      reviewBindingFromReviewSet(options, reviewSet),
      reviewSet.outputParentBinding,
    );
    throw error;
  }
}
...
function validateSuccessorTarget(text, errors) {
  const markers = [
    "v2.13 Section 4 is the sole operative clean-room rule.",
    "v2.12 Section 6 is explicitly retired and non-operative for v2.17.",
    "Any v2.14 language that retained v2.12 Section 6",
  ...
  pushCheck(
    errors,
    "successor-one-clean-room-rule",
    text.includes("The older v2.12 workspace-only procedure is not an alternative path."),
    true,
    false,
  );
}
...
function inputSetDigest(rows) {
  return sha256(Buffer.from(`G4L10-V217-INPUT-SET\n${canonicalJson(rows)}\n`, "utf8"));
}
...
export function syntaxCheckBufferForTest(bytes) {
  invariant(Buffer.isBuffer(bytes), "syntax input must be a retained Buffer");
...
function runFocusedTestsFromBuffers(verifierBytes, testBytes) {
  ...
  const needle = 'from "./g4-l10-native-helper-v2_17-review-verifier.mjs";';
  ...
  const result = spawnSync(process.execPath, ["--input-type=module", "-"], {
    ...
    env: {
      ...
      TMPDIR: "/tmp",
      G4_L10_V217_RETAINED_TEST_EXECUTION: "1",
    },
  });
  return {
    ok: result.status === 0 && result.signal === null,
...
function attachAuthenticatedBinding(error, binding, outputParentBinding = null) {
  if (error && typeof error === "object" && !error.authenticatedReviewBinding) {
    error.authenticatedReviewBinding = binding;
    error.authenticatedOutputParentBinding = outputParentBinding;
  }
  return error;
}

export function authenticatedErrorReceiptForTest(recovered, error, status, binding) {
  attachAuthenticatedBinding(error, binding);
  return errorReceiptFrom(recovered, error, status);
}
...
function phaseBinding(options, context) {
  ...
  return {
    ...
    outputParentBinding: context.reviewSet.outputParentBinding,
  };
}
...
async function preparePreflight(options) {
  ...
  try {
    ...
    try {
      systemWriter = await verifySystemWriter();
      add("system-writer:exact-identity", true, systemWriter);
    } catch (error) {
      add("system-writer:exact-identity", false, summarizeError(error));
    }
    ...
    const syntax = {
      verifier: syntaxCheckBufferForTest(verifierSnapshot.bytes),
      focusedTest: syntaxCheckBufferForTest(focusedTestSnapshot.bytes),
    };
    add("syntax:verifier", syntax.verifier.ok, syntax.verifier);
    add("syntax:focused-test", syntax.focusedTest.ok, syntax.focusedTest);
    const focusedTestExecution = runFocusedTestsFromBuffers(verifierSnapshot, focusedTestSnapshot);
    ...
    const receipt = addReceiptId("PREFLIGHT", {
      schemaVersion: 1,
      artifactType: "g4-l10-native-helper-v2-17-review-preflight",
      status,
    ...
    attachAuthenticatedBinding(
      error,
      reviewBinding(options, context),
      context.reviewSet.outputParentBinding,
    );
    try {
      await context.close();
    } catch (closeError) {
...
async function prepareEvidence(options) {
  ...
  try {
    ...
    const receipt = addReceiptId("EVIDENCE", {
      schemaVersion: 1,
      artifactType: "g4-l10-native-helper-v2-17-deterministic-evidence",
      status,
...
export async function publishFixtureAttemptForTest({
  ...
  try {
    await assertAttemptLeavesUnused(successOutput, errorOutput);
    if (forceError) throw new Error("forced pre-publication fixture failure");
...
function errorReceiptFrom(recovered, error, status, attemptedReceipt = null) {
  const authenticated = attemptedReceipt?.reviewBinding ?? error?.authenticatedReviewBinding ?? null;
  return addReceiptId("ERROR", {
    schemaVersion: 1,
    artifactType: "g4-l10-native-helper-v2-17-verifier-error",
    status,
    ...
    authenticatedOutputParentBinding: error?.authenticatedOutputParentBinding ?? null,
...
export function unpreservedErrorReceiptForTest(report, originalStatus, persistenceError) {
...
async function executeCli(argv) {
  ...
  try {
    ...
    prepared = null;
    await writeNoClobberReceipt(output, receipt, { expectedParent: outputParentBinding });
    ...
    if (prepared) {
      try {
        ...
        if (error && typeof error === "object") error.closeError = summarizeError(closeError);
      }
      prepared = null;
    }
    const status = classifyFailure(recovered.command, error);
    if (error && typeof error === "object"
      && !error.authenticatedOutputParentBinding
      && outputParentBinding) {
      error.authenticatedOutputParentBinding = outputParentBinding;
    }
    const report = errorReceiptFrom(recovered, error, status, attemptedReceipt);
    if (recovered.errorOutput
      && path.isAbsolute(recovered.errorOutput)
      && path.normalize(recovered.errorOutput) === recovered.errorOutput) {
      try {
        await writeNoClobberReceipt(recovered.errorOutput, report, {
          expectedParent: error?.authenticatedOutputParentBinding ?? null,
        });
        process.stdout.write(receiptBytes(report));
        process.exitCode = exitCodeForStatus(status);
        return;
      } catch (persistenceError) {
        const unpreserved = unpreservedErrorReceiptForTest(report, status, persistenceError);
        process.stdout.write(receiptBytes(unpreserved));
        process.exitCode = 74;
        return;
      }
    }
    const missingOutput = new Error("no uniquely recoverable --error-output was supplied");
    missingOutput.code = "NO_ERROR_OUTPUT";
    const unpreserved = unpreservedErrorReceiptForTest(report, status, missingOutput);
    process.stdout.write(receiptBytes(unpreserved));
    process.exitCode = 74;
  }
}
...
if (invokedAsMain) {
  executeCli(process.argv.slice(2)).catch((error) => {
    const fallback = addReceiptId("ERROR", {
      schemaVersion: 1,
      artifactType: "g4-l10-native-helper-v2-17-unhandled-error",
      status: "ATTEMPT_RECEIPT_UNPRESERVED_NO_VERDICT",
      evidenceConclusion: false,
      error: summarizeError(error),
      authorityEffects: closedAuthority(),
