...
const ANIMATION_ID = 'course-g04-l10-vb-004';
const CALIBRATION_ID = 'g4-l10-candidate-to-product-v6';
const FREEZE = Object.freeze({
  path: 'catalog/product-bridge-calibrations/g4-l10-candidate-to-product-v6.json',
  sha256: '4c1bb85bcc3f6d5df6cb339568339688e58ea958fdf2bd35f0abb299f3c0b3b0',
});
const SOURCE_SWF =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/VB/L10VB04.swf';
const SOURCE_SWF_SHA256 =
  'f0d0ebfc9abebcfb13e6fb150a8663503330f1fbc2289713a678d64df307e500';
const SWFMILL_XML =
  'migrations/course-g04-l10-vb-004/audit/machine/swfmill.xml.gz';
const SWFMILL_XML_SHA256 =
  '4f40b6dd4cb56b71509e4a5d538ce17fb9b42913ca7010d716b24ad9dee3efad';
const AUDIO_RUNTIME_AUDIT =
  'migrations/course-g04-l10-vb-004/audit/audio-runtime-evidence.json';
const AUDIO_RUNTIME_AUDIT_SHA256 =
  '0c60bd290bf5b451785604f087809ddaa74d6f1d2dedc74cc3f056f5de57f49c';
const SPANISH_SOURCE =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/SA/L10VB04.mp3';
const PUBLIC_DIRECTORY =
  'apps/web/candidate-assets/flash-assets/2026-08-22-page-only-candidates-v1/courses/course-g04-l10-vb-004/audio';
...
export const EXPECTED_VB004_AUDIO = Object.freeze({
  embedded: Object.freeze({
    sourceTimelineId: 'sprite-45',
    characterId: 45,
    headFrame: 1,
    firstFrame: 4,
    lastFrame: 213,
...
function validateEmbedded(stream, audioAudit) {
  const expected = EXPECTED_VB004_AUDIO.embedded;
  invariant(stream?.head, 'sprite-45 has no SoundStreamHead');
  invariant(
    stream.head.compression === '2' &&
      stream.head.soundRate === '2' &&
      stream.head.soundStereo === '0',
    'sprite-45 is not the frozen mono 22050 Hz MP3 SoundStream',
  );
  invariant(
    ...
    'sprite-45 extracted MP3 bytes differ from the V6 freeze',
  );
  ...
  invariant(
    ...
    'audio-runtime evidence no longer binds sprite-45 frames 4..213',
  );
  invariant(
    stream.mp3[0] === 0xff && (stream.mp3[1] & 0xe0) === 0xe0,
    'sprite-45 extracted bytes have no MP3 sync word',
  );
}

function outputRecords(scriptSha256, embeddedProbe, spanishProbe) {
  const embedded = EXPECTED_VB004_AUDIO.embedded;
  const spanish = EXPECTED_VB004_AUDIO.spanish;
  const assets = [
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-vb-004/audio/embedded-stream-0001.mp3',
    ...
    },
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-vb-004/audio/spanish-host-narration.mp3',
      bytes: spanish.bytes,
  ...
  const common = {
    ...
    freeze: FREEZE,
    generatedBy: {
      path: 'scripts/materialize-g4-l10-vb004-private-audio.mjs',
      sha256: scriptSha256,
  ...
  const manifest = jsonBytes({
    ...common,
    artifactType: 'g4-l10-vb004-private-product-audio-manifest',
    status: 'byte-exact-private-current-js-audio-candidate',
  });
  const receipt = jsonBytes({
    ...common,
    artifactType: 'g4-l10-vb004-private-product-audio-receipt',
...
export async function materializeG4L10Vb004PrivateAudio(options = {}) {
  ...
  const spanishBytes = await readBound(
    root,
    SPANISH_SOURCE,
    EXPECTED_VB004_AUDIO.spanish.sha256,
    EXPECTED_VB004_AUDIO.spanish.bytes,
  );
  const streams = await extractMp3SoundStreams(
    path.join(root, SWFMILL_XML),
    [EXPECTED_VB004_AUDIO.embedded.characterId],
  );
  const stream = streams.get(EXPECTED_VB004_AUDIO.embedded.characterId);
  validateEmbedded(stream, JSON.parse(audioAuditBytes));
  const embeddedProbe = probeMp3(
    stream.mp3,
    EXPECTED_VB004_AUDIO.embedded,
    'VB004 embedded SoundStream',
  );
  const spanishProbe = probeMp3(
    spanishBytes,
    EXPECTED_VB004_AUDIO.spanish,
    'VB004 Spanish host track',
  );
  ...
  if (options.check) {
    invariant(
      stale.length === 0,
      `VB004 private audio outputs are stale:\n${stale.join('\n')}`,
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const result = await materializeG4L10Vb004PrivateAudio(
