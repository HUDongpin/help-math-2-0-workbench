...
const ANIMATION_ID = 'course-g04-l10-ts-005';
const CALIBRATION_ID = 'g4-l10-candidate-to-product-v16';
const FREEZE = Object.freeze({
  path: 'catalog/product-bridge-calibrations/g4-l10-candidate-to-product-v16.json',
  sha256: '10ef28061e0241d5146a43843403a78ed67fccea502c5f320266cc017621736e',
});
const SOURCE_SWF =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/TS/L10TS05.swf';
const SOURCE_SWF_SHA256 =
  'f0ec8f168ec7de0f20dc058730b02c880d9c8e81940b966e0bd2da2f3684d905';
const SWFMILL_XML =
  'migrations/course-g04-l10-ts-005/audit/machine/swfmill.xml.gz';
const SWFMILL_XML_SHA256 =
  '795e395ff13849b36cf3a961f4ad49d329f284b6ce93335295aca44778f91358';
const AUDIO_RUNTIME_AUDIT =
  'migrations/course-g04-l10-ts-005/audit/audio-runtime-evidence.json';
const AUDIO_RUNTIME_AUDIT_SHA256 =
  '5a6d18b65cb3d6e04ffcf40657e6245f2fb75181e2f3888ff21ff834b1fbe239';
const SPANISH_SOURCE =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/SA/L10TS05.mp3';
const PUBLIC_DIRECTORY =
  'apps/web/candidate-assets/flash-assets/2026-08-22-page-only-candidates-v1/courses/course-g04-l10-ts-005/audio';
...
export const EXPECTED_TS005_AUDIO = Object.freeze({
  embedded: Object.freeze({
    sourceTimelineId: 'sprite-32',
    characterId: 32,
    headFrame: 1,
    firstFrame: 1,
    lastFrame: 234,
...
function validateEmbedded(stream, audioAudit) {
  const expected = EXPECTED_TS005_AUDIO.embedded;
  invariant(stream?.head, 'sprite-32 has no SoundStreamHead');
  invariant(
    stream.head.compression === '2' &&
      stream.head.soundRate === '2' &&
      stream.head.soundStereo === '0',
    'sprite-32 is not the frozen mono 22050 Hz MP3 SoundStream',
  );
  invariant(
    ...
    'sprite-32 extracted MP3 bytes differ from the V16 freeze',
  );
  ...
  invariant(
    ...
    'audio-runtime evidence no longer binds sprite-32 frames 1..234',
  );
  invariant(
    stream.mp3[0] === 0xff && (stream.mp3[1] & 0xe0) === 0xe0,
    'sprite-32 extracted bytes have no MP3 sync word',
  );
}

function outputRecords(scriptSha256, embeddedProbe, spanishProbe) {
  const embedded = EXPECTED_TS005_AUDIO.embedded;
  const spanish = EXPECTED_TS005_AUDIO.spanish;
  const assets = [
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-ts-005/audio/embedded-stream-0001.mp3',
    ...
    },
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-ts-005/audio/spanish-host-narration.mp3',
      bytes: spanish.bytes,
  ...
  const common = {
    ...
    freeze: FREEZE,
    generatedBy: {
      path: 'scripts/materialize-g4-l10-ts005-private-audio.mjs',
      sha256: scriptSha256,
    ...
    authorityBoundary:
      'Spoken language/content, natural Adobe runtime reachability, audible quality, Replay parity, listening acceptance, human review, Owner acceptance, strict completion, release, and publication remain unresolved.',
    acceptanceEffects: {
  ...
  const manifest = jsonBytes({
    ...common,
    artifactType: 'g4-l10-ts005-private-product-audio-manifest',
    status: 'byte-exact-private-current-js-audio-candidate',
  });
  const receipt = jsonBytes({
    ...common,
    artifactType: 'g4-l10-ts005-private-product-audio-receipt',
...
export async function materializeG4L10Ts005PrivateAudio(options = {}) {
  ...
  const spanishBytes = await readBound(
    root,
    SPANISH_SOURCE,
    EXPECTED_TS005_AUDIO.spanish.sha256,
    EXPECTED_TS005_AUDIO.spanish.bytes,
  );
  const streams = await extractMp3SoundStreams(
    path.join(root, SWFMILL_XML),
    [EXPECTED_TS005_AUDIO.embedded.characterId],
  );
  const stream = streams.get(EXPECTED_TS005_AUDIO.embedded.characterId);
  validateEmbedded(stream, JSON.parse(audioAuditBytes));
  const embeddedProbe = probeMp3(
    stream.mp3,
    EXPECTED_TS005_AUDIO.embedded,
    'TS005 embedded SoundStream',
  );
  const spanishProbe = probeMp3(
    spanishBytes,
    EXPECTED_TS005_AUDIO.spanish,
    'TS005 Spanish host track',
  );
  ...
  if (options.check) {
    invariant(
      stale.length === 0,
      `TS005 private audio outputs are stale:\n${stale.join('\n')}`,
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const result = await materializeG4L10Ts005PrivateAudio(
