...
export const SUCCESSOR_V5_OUTPUT_RELATIVE =
  "output/playwright/g4-l10-vb003-original-host-ruffle-successor-v5";
const RESULT_RELATIVE = `${SUCCESSOR_V5_OUTPUT_RELATIVE}/diagnostic.json`;
...
function invariant(condition, message) {
  if (!condition) {
    throw new Error(`G4 L10 VB003 Ruffle successor v5: ${message}`);
  }
}
...
export function buildInputAttemptPlans() {
  return [
    {
      attempt: 1,
      id: "fresh-context-pre-ruffle-init-window-capture-staged-release-v2-network-policy",
    ...
    {
      attempt: 2,
      id: "fresh-context-pre-ruffle-init-window-capture-staged-release-with-exact-xml",
      transport: "page.mouse move/down/up",
      freshContext: true,
...
function launchSources() {
  ...
  const launch = Buffer.from([
    ...
    '    player.id = "player";',
    '    player.setAttribute("aria-label", "L10 original-host Ruffle successor v5 forensic diagnostic");',
  ...
  const html = Buffer.from([
    "<!doctype html>",
    '<html lang="en"><head><meta charset="utf-8"><title>L10 contained Ruffle successor v5</title>',
...
async function bindInputs() {
  ...
  const [staticReport, v2Result, v3Result, v4Result] = await Promise.all([
...
async function createOutput(inputs) {
  const outputRoot = path.join(PROJECT_ROOT, SUCCESSOR_V5_OUTPUT_RELATIVE);
  invariant(
    !(await pathExists(outputRoot)),
    `${SUCCESSOR_V5_OUTPUT_RELATIVE} already exists; immutable output will not be overwritten`,
  );
...
export async function runSuccessorV5Probe() {
  ...
  const [staticAfter, v2After, v3After, v4After] = await Promise.all([
    readBinding(STATIC_REPORT_RELATIVE),
    readBinding(V2_RESULT_RELATIVE),
    readBinding(V3_RESULT_RELATIVE),
    readBinding(V4_RESULT_RELATIVE),
  ...
  invariant(
    ...
    "an immutable predecessor or canonical XML changed during v5",
  );
  ...
  const result = {
    ...
    status: topLevelFatalError || attempts.every((attempt) => attempt.fatalError)
      ...
      : firstSuccessorAttempt
        ...
        : completeDomReleaseObserved
          ? "complete-pre-ruffle-window-capture-dom-release-observed-rw002-not-requested-in-ruffle-forensic-only"
    ...
    probe: {
      ...
      browserAutomation:
        "real Chromium via repository-pinned Playwright; BrowserContext.addInitScript installs controlling window capture before every page script including Ruffle, followed by the same staged trusted pointer transport in two bounded fresh-context network-policy variants at most",
    },
    lineage: {
      staticAntecedent: {
    ...
    hypotheses: {
      ...
      v4InstrumentationCorrection:
        "v4 moved capture to window but registered it only after ruffle.js loaded; it still missed pointerdown/pointerup and remains an input-layer failure",
