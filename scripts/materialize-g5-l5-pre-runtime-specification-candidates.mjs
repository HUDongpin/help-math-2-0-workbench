...
export const RELEASE_ID = "lesson-g05-l05-add-subtract-negative-numbers";
...
const SOURCE_SCOPE_REPORT_PATH = "reports/g5-l5-source-scope-freeze.json";
...
export const OUTPUT_NAMES = Object.freeze({
  manifestRuntimeFacts: "manifest-runtime-facts-candidate.json",
  assetDefinitionCensus: "swf-asset-definition-census.json",
  definitionInventory: "swf-definition-inventory.csv",
  scriptInventory: "ffdec-script-inventory-candidate.json",
  dependencyInventory: "static-dependency-inventory-candidate.json",
  briefStaticPrefill: "migration-brief-static-prefill-candidate.md",
  receipt: "pre-runtime-specification-candidate-receipt.json",
});
...
function selectRelease(document) {
  ...
  invariant(
    ...
    `${RELEASE_ID}: exact 56-page plus shell release shape drifted`,
  );
  invariant(
...
function validateReleaseReports(release, sourceScope, workspaceReadiness) {
  invariant(
    ...
    "G5 L5 source-scope report drifted",
  );
  invariant(
    ...
    "G5 L5 workspace-readiness report drifted",
  );
...
function buildCandidateDocuments({
  ...
  const brief = Buffer.from(
    `# ${member.animationId} Static Migration-Brief Prefill Candidate\n\n` +
      ...
      `- Release: \`${RELEASE_ID}\`; ordinal: **${member.ordinal}/57**; role: \`${member.releaseRole}\`; shard: \`${member.shardId}\`.\n` +
...
export function classifyStaticReconciliationReceiptState(
  ...
  invariant(
    Array.isArray(receiptPresence) &&
      receiptPresence.length === 57 &&
      receiptPresence.every((value) => typeof value === "boolean"),
    "static reconciliation receipt presence must contain 57 booleans",
  );
  ...
  return count === 57 ? "historical-post-adoption" : "pre-adoption";
}
...
async function inspectHistoricalCandidatePackages({
  ...
  invariant(
    check || dryRun,
    "G5 L5 candidate packages are immutable historical pre-adoption evidence after 57/57 static reconciliation; use --check or --dry-run",
  );
  const {
  ...
  const summary = {
    ...
    candidatePackageCompleteCount: 57,
    staticReconciliationCount: 57,
...
export async function materializeG5L5PreRuntimeSpecificationCandidates({
  ...
  const [sourceScope, workspaceReadiness, generator, parser] =
    await Promise.all([
      readJsonBinding(
        ...
        "G5 L5 source-scope report",
      ),
      readJsonBinding(
        resolvedRoot,
        WORKSPACE_READINESS_PATH,
        "G5 L5 workspace-readiness report",
      ),
  ...
  invariant(
    prepared.length === 57 && outputs.length === 57 * 7,
    "full 57-member preflight did not produce exactly 399 outputs",
  );
  if (check) {
    for (const output of outputs) {
      invariant(
  ...
  const summary = {
    ...
    candidatePackageCompleteCount: check || !dryRun ? 57 : 0,
    canonicalFilesChanged: 0,
    runtimeSessionsExecuted: 0,
    guiApplicationsLaunched: 0,
    legacyEndpointsExecuted: 0,
...
function usage() {
  return `Usage:
  node scripts/materialize-g5-l5-pre-runtime-specification-candidates.mjs
  node scripts/materialize-g5-l5-pre-runtime-specification-candidates.mjs --dry-run
  node scripts/materialize-g5-l5-pre-runtime-specification-candidates.mjs --check

Preflights all 57 G5 L5 release members, then atomically writes or rolls back
...
reviews, acceptance, or publication. After 57/57 M1 static reconciliation,
--check/--dry-run validate the seven files as immutable historical packages;
write mode fails closed. It launches no GUI or runtime.`;
}
...
async function main() {
  ...
  const result =
    await materializeG5L5PreRuntimeSpecificationCandidates(options);
  const label = result.mode.startsWith("checked")
    ? "PASS"
    : result.mode.startsWith("dry-run")
      ? "DRY-RUN"
      : "WROTE";
  process.stdout.write(
    `${label}: ` +
      `${result.summary.memberCount}/57 G5 L5 candidate packages; ` +
      ...
      `implementation-ready 0/57; strict 0/57; published false.\n`,
  );
}
