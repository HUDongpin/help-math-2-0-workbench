...
const ANIMATION_ID = 'course-g04-l10-vb-002';
const CALIBRATION_ID = 'g4-l10-candidate-to-product-v18';
const FREEZE = Object.freeze({
  path: 'catalog/product-bridge-calibrations/g4-l10-candidate-to-product-v18.json',
  sha256: '8f771a1ed5900da347338f26d19fb53581c2f89b456b3a4e983f2d0a64ca1aeb',
});
const SOURCE_SWF =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/VB/L10VB02.swf';
const SOURCE_SWF_SHA256 =
  'a46fa315148118d58a379a2d7b921684f5a0a210c72cae9433550e755ae42a81';
const SWFMILL_XML =
  'migrations/course-g04-l10-vb-002/audit/machine/swfmill.xml.gz';
const SWFMILL_XML_SHA256 =
  '1757d59d0c4fa7728c8dfbd32a63aea3e564717dcffa255b32ef28203f4674ba';
const AUDIO_RUNTIME_AUDIT =
  'migrations/course-g04-l10-vb-002/audit/audio-runtime-evidence.json';
const AUDIO_RUNTIME_AUDIT_SHA256 =
  '72857ffebf877314cc50f8967e911eb272e21e9e61b0d49d5f1401cf3b881f65';
const SPANISH_SOURCE =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/SA/L10VB02.mp3';
const PUBLIC_DIRECTORY =
  'apps/web/candidate-assets/flash-assets/2026-08-22-page-only-candidates-v1/courses/course-g04-l10-vb-002/audio';
...
export const EXPECTED_VB002_AUDIO = Object.freeze({
  embedded: Object.freeze({
    sourceTimelineId: 'sprite-84',
    characterId: 84,
    headFrame: 1,
    firstFrame: 4,
    lastFrame: 280,
...
function validateEmbedded(stream, audioAudit) {
  const expected = EXPECTED_VB002_AUDIO.embedded;
  invariant(stream?.head, 'sprite-84 has no SoundStreamHead');
  invariant(
    stream.head.compression === '2' &&
      stream.head.soundRate === '2' &&
      stream.head.soundStereo === '0',
    'sprite-84 is not the frozen mono 22050 Hz MP3 SoundStream',
  );
  invariant(
    ...
    'sprite-84 extracted MP3 bytes differ from the V18 freeze',
  );
  ...
  invariant(
    ...
    'audio-runtime evidence no longer binds sprite-84 frames 4..280',
  );
  invariant(
    stream.mp3[0] === 0xff && (stream.mp3[1] & 0xe0) === 0xe0,
    'sprite-84 extracted bytes have no MP3 sync word',
  );
}

function outputRecords(scriptSha256, embeddedProbe, spanishProbe) {
  const embedded = EXPECTED_VB002_AUDIO.embedded;
  const spanish = EXPECTED_VB002_AUDIO.spanish;
  const assets = [
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-vb-002/audio/embedded-stream-0001.mp3',
    ...
    },
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-vb-002/audio/spanish-host-narration.mp3',
      bytes: spanish.bytes,
  ...
  const common = {
    ...
    freeze: FREEZE,
    generatedBy: {
      path: 'scripts/materialize-g4-l10-vb002-private-audio.mjs',
      sha256: scriptSha256,
  ...
  const manifest = jsonBytes({
    ...common,
    artifactType: 'g4-l10-vb002-private-product-audio-manifest',
    status: 'byte-exact-private-current-js-audio-candidate',
  });
  const receipt = jsonBytes({
    ...common,
    artifactType: 'g4-l10-vb002-private-product-audio-receipt',
...
export async function materializeG4L10Vb002PrivateAudio(options = {}) {
  ...
  const spanishBytes = await readBound(
    root,
    SPANISH_SOURCE,
    EXPECTED_VB002_AUDIO.spanish.sha256,
    EXPECTED_VB002_AUDIO.spanish.bytes,
  );
  const streams = await extractMp3SoundStreams(
    path.join(root, SWFMILL_XML),
    [EXPECTED_VB002_AUDIO.embedded.characterId],
  );
  const stream = streams.get(EXPECTED_VB002_AUDIO.embedded.characterId);
  validateEmbedded(stream, JSON.parse(audioAuditBytes));
  const embeddedProbe = probeMp3(
    stream.mp3,
    EXPECTED_VB002_AUDIO.embedded,
    'VB002 embedded SoundStream',
  );
  const spanishProbe = probeMp3(
    spanishBytes,
    EXPECTED_VB002_AUDIO.spanish,
    'VB002 Spanish host track',
  );
  ...
  if (options.check) {
    invariant(
      stale.length === 0,
      `VB002 private audio outputs are stale:\n${stale.join('\n')}`,
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const result = await materializeG4L10Vb002PrivateAudio(
