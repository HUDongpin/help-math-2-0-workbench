...
/**
 * Build source-static visual compiler evidence for G4 L5. This script is
...
export const FACTORY_ROOT =
  "tools/g4-l5-ffdec-canvas-pcode-product-factory";
...
function releasePageCandidates(catalog, release) {
  ...
  const candidates = catalog.animations
    .map((entry) => {
      ...
      invariant(matching.length === 1, `${entry.animationId}: duplicate G4 L5 XML occurrence`);
  ...
  invariant(candidates.length === release.expectedActivePageCount,
    `G4 L5 expected ${release.expectedActivePageCount} active pages, found ${candidates.length}`);
  candidates.forEach((candidate, index) => invariant(candidate.ordinal === index + 1,
    `G4 L5 XML source order is incomplete at ordinal ${index + 1}`));
  ...
  invariant(
    ...
    `G4 L5 expected ${release.expectedExactExternalAudioPageCount} exact external-audio pages, found ${externalAudioPages.length}`,
  );
...
export function selectFactoryMembers(catalog, corpus, mode) {
  ...
  const selected = calibration.map((selection) => {
    ...
    invariant(candidate, `calibration member is not an active G4 L5 page: ${selection.animationId}`);
...
async function buildMember(member, stageRoot) {
  ...
  const manifest = Object.freeze({
    ...
    compiler: Object.freeze({
      ...
      exportProfile:
        "g4-l5-resource-bounded-frame-sprite-script-image-sound",
...
async function build(options) {
  ...
  try {
    ...
    throw new Error(`G4 L5 FFDec factory failed; retained staging diagnostics at ${relativeProject(stage)}: ${error.message}`);
  }
}
