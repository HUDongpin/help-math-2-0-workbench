#!/usr/bin/env python3
"""Extract the four VB006 glossary-hotspot source records from swfmill XML.
...
HOTSPOTS = (
    (11, "Zero", 1, 5),
    (12, "Value", 1, 7),
    (42, "Positive number", 116, 67),
    (43, "Negative number", 116, 69),
)
...
def main() -> None:
    ...
    root_frame, root_placement = one(root_placements, "sprite-44 named root placement")
    root_raw, root_matrix = transform(root_placement, "root placement")
    if root_frame != 6 or root_placement.get("depth") != "4":
        raise ValueError("sprite-44 root frame/depth changed")
