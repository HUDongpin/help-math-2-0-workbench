...
/**
 * Apply the reviewed G5 L2 active-source copy set to the canonical main
 * checkout after the G5 L3 source promotion.
 *
 * This executor adds only the plan's exact two active-page SWFs and one
 * same-path FLA,
...
const PLAN_RELATIVE =
  "catalog/source-promotions/g5-l2-active-source-promotion-v1.json";
const RECEIPT_RELATIVE =
  "catalog/source-promotions/g5-l2-active-source-promotion-main-applied-v1.json";
const PLAN_SHA256 =
  "d491cc48880d360a96ebe2702f09e14ad4f512a2e03453d4cafed41ee2251a25";
...
const BASE_FILE_COUNT = 9_287;
const BASE_TOTAL_BYTES = 3_271_150_427;
const COPY_COUNT = 3;
const COPY_BYTES = 6_631_132;
const COPY_RECORD_SET_SHA256 =
  "577428b3536a74560a5cc2d46cbb8d7c171123cddb682130ee40e8a3b35e3735";
...
const POST_FILE_COUNT = 9_290;
const POST_TOTAL_BYTES = 3_277_781_559;
...
async function loadPlan() {
  ...
  invariant(planFile.sha256 === PLAN_SHA256, "Pinned G5 L2 promotion-plan SHA-256 drift");
  const planText = await readFile(planPath, "utf8");
  const plan = JSON.parse(planText);
  invariant(plan.artifactType === "help-math-g5-l2-active-source-promotion-plan" &&
    plan.mode === "plan-only-no-source-mutation" &&
    plan.scope?.grade === 5 && plan.scope?.lesson === 2 &&
    plan.scope?.pageOnly === true &&
    plan.scope?.legacyFlashCourseShellExcluded === true,
  "Wrong G5 L2 promotion-plan scope");
  ...
  "G5 L2 promotion plan is not a reviewed, conflict-free copy transaction");
  ...
  "G5 L2 promotion-plan copy set drift");
  invariant(plan.currentMainBaseline?.sourceManifestSha256 === BASE_MANIFEST_SHA256 &&
    plan.currentMainBaseline?.sourceFileCount === BASE_FILE_COUNT &&
    plan.currentMainBaseline?.sourceTotalBytes === BASE_TOTAL_BYTES &&
    plan.currentMainBaseline?.catalogChecksumSetSha256 === BASE_CHECKSUM_SET_SHA256 &&
    plan.expectedMainPostState?.sourceManifestSha256 === POST_MANIFEST_SHA256 &&
    plan.expectedMainPostState?.sourceFileCount === POST_FILE_COUNT &&
    plan.expectedMainPostState?.sourceTotalBytes === POST_TOTAL_BYTES &&
    plan.expectedMainPostState?.catalogChecksumSetSha256 === POST_CHECKSUM_SET_SHA256 &&
    plan.expectedMainPostState?.currentSourceProfileSha256 ===
      POST_CURRENT_SOURCE_PROFILE_SHA256,
  "G5 L2 promotion-plan source currentness drift");
...
function transactionPaths(transactionId) {
  ...
  return {
    sourceStage: path.join(sourceParent,
      `.HELP MATH_ORIGINAL FILES.g5-l2-stage-${transactionId}`),
    catalogStage: path.join(catalogParent, `.catalog.g5-l2-stage-${transactionId}`),
  };
}
...
async function preflight() {
  ...
  invariant(!(await exists(path.join(PROJECT_ROOT, RECEIPT_RELATIVE))),
    "Main G5 L2 applied receipt already exists");
  const [
    {plan, planFile},
    sourceCatalogFile,
    sourceManifestFile,
...
async function validateStagedCatalog(stagedSourceRoot, stagedCatalogRoot, plan) {
  ...
  invariant(stillMissing.length === 0,
    `Promoted G5 L2 SWFs remain missing from staged catalog: ${stillMissing.length}`);
  return {
    freeze,
...
async function apply() {
  ...
  try {
    ...
    const receipt = {
      schemaVersion: 1,
      artifactType: "help-math-g5-l2-main-source-promotion-applied-receipt",
      appliedAt: new Date().toISOString(),
      transactionId,
      scope: {grade: 5, lesson: 2, pageOnly: true},
      canonicalBoundary: {
      ...
      acceptanceEffects: {
        ...
        wholeLessonActivePageSourceCoverage: "64/64",
    ...
    if (rollbackErrors.length) {
      throw new AggregateError([error, ...rollbackErrors.map((message) => new Error(message))],
        "Main G5 L2 promotion failed and rollback was incomplete");
    }
    throw error;
  }
}
