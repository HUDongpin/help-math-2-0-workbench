...
import {buildVb005SourcePreaudit} from "./build-g4-l3-vb005-source-preaudit.mjs";
...
const SOURCE_SWF = `${ARCHIVE_PREFIX}/HELP_COURSES/ELMGR4/L3/VB/L3VB05.swf`;
const SOURCE_FLA = `${ARCHIVE_PREFIX}/HELP_COURSES/ELMGR4/L3/VB/L3VB05.fla`;
const SOURCE_ASSOCIATED_AUDIO =
  `${ARCHIVE_PREFIX}/HELP_COURSES/ELMGR4/L3/SA/L3VB05.mp3`;
...
const OUTPUT_SCRIPT =
  "public/flash-assets/courses/course-g04-l03-vb-005/canvas-renderer.js";
const OUTPUT_MANIFEST =
  "public/flash-assets/courses/course-g04-l03-vb-005/manifest.json";
const OUTPUT_REPORT_JSON =
  "reports/g4-l3-vb005-current-javascript-candidate.json";
const OUTPUT_REPORT_MARKDOWN =
  "reports/g4-l3-vb005-current-javascript-candidate.md";
...
const INTEGRATION_BINDINGS = Object.freeze([
  ...
  "packages/demos/src/timelines/course-g04-l03-vb-005.ts",
  "packages/demos/src/modules/course-g04-l03-vb-005.tsx",
]);
...
const UNRESOLVED = Object.freeze([
  ...
  "sprite-53 frames 1..180 are source-static drawing states only. This candidate does not establish natural runtime reachability, root compositing, terminal behavior, or visual parity.",
  "The separate one-frame sprite-5 page-title domain is inventoried but not rendered or composited.",
  "Three source hit-only glossary buttons call unresolved host ActionScript. Their placement geometry is recorded, but pointer events, KeyAttribute writes, DoHyperLinks(), and host animation stops are disabled.",
...
const HOTSPOT_EXPECTATIONS = Object.freeze([
  Object.freeze({
    ...
    lastInclusive: 180,
    depth: 5,
    placement: Object.freeze({scaleX: "2.882003784179688", skewX: "0", skewY: "0", scaleY: "1.175582885742188", transX: "-4690", transY: "-2281"}),
    composed: Object.freeze({scaleX: "2.882003784179688", skewX: "0", skewY: "0", scaleY: "1.175582885742188", transX: "3336", transY: "2604"}),
    bounds: Object.freeze({left: "79.7634857177734224", right: "253.6924140930175932", top: "118.9144042968749952", bottom: "141.4268165588378954", width: "173.9289283752441708", height: "22.5124122619629002"}),
  }),
  Object.freeze({
    characterId: 12,
    ...
    lastInclusive: 180,
    depth: 7,
    placement: Object.freeze({scaleX: "1.418106079101562", skewX: "0", skewY: "0", scaleY: "1.175582885742188", transX: "-1302", transY: "-2281"}),
    composed: Object.freeze({scaleX: "1.418106079101562", skewX: "0", skewY: "0", scaleY: "1.175582885742188", transX: "6724", transY: "2604"}),
    bounds: Object.freeze({left: "293.3731964111328276", right: "378.9558982849120943", top: "118.9144042968749952", bottom: "141.4268165588378954", width: "85.5827018737792667", height: "22.5124122619629002"}),
  }),
  Object.freeze({
    characterId: 13,
    keyAttribute: "Zero",
    ...
    lastInclusive: 180,
    depth: 9,
    placement: Object.freeze({scaleX: "0.6846008300781250", skewX: "0", skewY: "0", scaleY: "1.175582885742188", transX: "56", transY: "-2281"}),
    composed: Object.freeze({scaleX: "0.684600830078125", skewX: "0", skewY: "0", scaleY: "1.175582885742188", transX: "8082", transY: "2604"}),
    bounds: Object.freeze({left: "383.425054931640625", right: "424.74071502685546875", top: "118.9144042968749952", bottom: "141.4268165588378954", width: "41.31566009521484375", height: "22.5124122619629002"}),
  }),
]);

export function validateVb005HotspotEvidence(evidence) {
  invariant(evidence?.schemaVersion === 1 && evidence.parser === "python-xml.etree.ElementTree",
    "VB005 hotspot parser identity changed");
  ...
  "VB005 hotspot stage/sprite identity changed");
  invariant(evidence.rootPlacement.frame === 6 && evidence.rootPlacement.objectId === 53 &&
  ...
  "VB005 hotspot root placement changed");
  ...
  evidence.hotspots.forEach((hotspot, index) => {
    ...
    `VB005 hotspot ${expected.characterId} identity/interval/depth changed`);
    ...
    `VB005 hotspot ${expected.characterId} matrix changed`);
    ...
    `VB005 hotspot ${expected.characterId} hit-state geometry changed`);
    invariant(hotspot.behaviorExecutedByCandidate === false &&
      hotspot.pointerEventsEnabledByCandidate === false,
    `VB005 hotspot ${expected.characterId} was enabled`);
  });
  ...
  "VB005 hotspot evidence boundary was promoted");
  return evidence;
}
...
async function browserRenderability(runtime) {
  ...
  try {
    ...
    await page.setContent("<!doctype html><meta charset=utf-8><title>VB005 renderability harness</title>");
    ...
    const result = await page.evaluate(async ({animationId, frameCount}) => {
      ...
      for (let frame = 1; frame <= frameCount; frame += 1) {
        ...
        const attributes = {
          ...
          "data-flash-frame-domain": "sprite-53",
    ...
    return {
      ...
      firstFrame: 1,
      lastFrame: 180,
      executedFrameCount: 180,
      pngEncodedFrameCount: 180,
...
function renderMarkdown(report) {
  ...
  return `# G4 L3 VB005 current-JavaScript engineering candidate\n\n` +
    ...
    `- Implemented address space: English-only \`sprite-53\` frames 1–180, muted and noninteractive.\n` +
    `- Runtime: \`${report.outputs.canvasRuntime.path}\` (SHA-256 \`${report.outputs.canvasRuntime.sha256}\`).\n` +
    `- Browser execution: ${report.candidateRenderability.executedFrameCount}/180 frames encoded at 800×600; manifest SHA-256 \`${report.candidateRenderability.frameManifestSha256}\`.\n` +
    ...
    `| Button | KeyAttribute | sprite-53 frames | placement depth | source state | candidate pointer behavior |\n` +
    `|---:|---|---:|---:|---|---:|\n${hotspotRows}\n\n` +
    `All three records retain their exact placement matrices and native-stage hit bounds in the JSON report. Geometry is evidence only; no DOM controls or host callbacks were created.\n\n` +
    `## Write boundary\n\n` +
    `The generator wrote only the four declared candidate outputs. Its before/after protected manifest stayed at \`${report.writeScope.protectedAfter.combinedManifestSha256}\`; source assets, every file under \`migrations/\`, the completion ledger, protected acceptance/review records, and candidate-specific integration bindings were unchanged during generation.\n\n` +
    `## Acceptance boundary\n\n| Gate | Accepted |\n|---|---:|\n${acceptanceRows}\n\n` +
...
function buildArtifacts({
  ...
  const manifest = {
    ...
    sourcePreaudit: {
      reportType: preaudit.reportType,
      reportFingerprintSha256: preaudit.reportFingerprintSha256,
    ...
    runtime: {
      ...
      boundedScope: {
        frameDomain: "sprite-53",
        frames: {first: 1, lastInclusive: 180},
        language: "en",
        scenario: "source-static-frame",
        audio: "disabled",
  ...
  const report = {
    ...
    animationId: ANIMATION_ID,
    batch: {lesson: "G4 L3", batchId: "batch-001", batchOrdinal: 8},
    classification: {
      section: "VB",
      page: 5,
      titleRaw: "Negative Numbers",
      titleDisplay: "Negative Numbers",
      titleSpanishCatalogOnly: "Números negativos",
      domain: "vocabulary",
    },
    disposition: {
    ...
    outputs: {
      ...
      prototypeModule: "packages/demos/src/modules/course-g04-l03-vb-005.tsx",
      pureTimeline: "packages/demos/src/timelines/course-g04-l03-vb-005.ts",
    },
    writeScope,
    authorization,
    acceptance,
...
export function validateG4L3Vb005CurrentJsCandidate(report) {
  invariant(report?.schemaVersion === 2 && report.reportType === REPORT_TYPE &&
    report.animationId === ANIMATION_ID,
  "VB005 candidate report identity is invalid");
  ...
  "VB005 candidate disposition was promoted");
  ...
  "VB005 authorization fields must all remain false");
  ...
  "VB005 acceptance fields must all remain false");
  validateVb005HotspotEvidence(report.hotspotEvidence);
  invariant(report.candidateRenderability.classification ===
    "candidate-renderability-only-not-visual-parity" &&
    report.candidateRenderability.executedFrameCount === 180 &&
    report.candidateRenderability.pngEncodedFrameCount === 180 &&
    report.candidateRenderability.frames.length === 180 &&
  ...
  "VB005 browser evidence exceeds or fails the candidate-renderability boundary");
  ...
  "VB005 candidate write boundary is not closed");
  ...
  invariant(report.reportFingerprintSha256 === fingerprint(projected),
    "VB005 candidate report fingerprint is stale");
  return report;
}
...
export async function generateG4L3Vb005CurrentJsCandidate({
  check = false,
  ffdec = "ffdec",
  swfmill = "swfmill",
  python = "python3",
  ...
  if (regenerationOnly) {
    const [sourceSwf, sourceFla, sourceAssociatedAudio, safeBuilder, generator,
      ...
      readPinnedSource(SOURCE_SWF, EXPECTED.sourceSwf, "VB005 source SWF",
        sourceRoot),
      readPinnedSource(SOURCE_FLA, EXPECTED.sourceFla, "VB005 source FLA",
        sourceRoot),
      readPinnedSource(SOURCE_ASSOCIATED_AUDIO, EXPECTED.associatedAudio,
        "VB005 associated audio", sourceRoot),
      readBinding(SAFE_ADAPTER_BUILDER),
      readBinding(portable(path.relative(ROOT, scriptPath))),
      readV9Identity(ANIMATION_ID),
    ]);
    const temporaryRoot = await mkdtemp(
      path.join(os.tmpdir(), "help-math-vb005-regeneration-only-"),
    );
    try {
      ...
      const [helper, framesHtml] = await Promise.all([
        readFile(path.join(canvasDirectory, "DefineSprite_53", "canvas.js")),
        readFile(path.join(canvasDirectory, "DefineSprite_53", "frames.html")),
      ]);
      ...
      "fresh FFDec sprite-53 frame export changed");
      ...
      "VB005 regenerated runtime differs from the exact v9 input");
      return {
        animationId: ANIMATION_ID,
        check: true,
  ...
  const [sourceSwf, sourceFla, sourceAssociatedAudio, hotspotParser, safeBuilder,
    ...
    readPinned(SOURCE_SWF, EXPECTED.sourceSwf, "VB005 source SWF"),
    readPinned(SOURCE_FLA, EXPECTED.sourceFla, "VB005 source FLA"),
    readPinned(SOURCE_ASSOCIATED_AUDIO, EXPECTED.associatedAudio, "VB005 associated audio"),
  ...
  const preaudit = await buildVb005SourcePreaudit({ffdec, swfmill, python});
  invariant(
    preaudit.currentJsEngineeringCandidate
      .sourceStaticCurrentJsEngineeringCandidateEligible === true &&
      Object.values(preaudit.permissions).every((value) => value === false) &&
      Object.values(preaudit.acceptance).every((value) => value === false),
    "fresh VB005 preaudit no longer supports an acceptance-neutral bounded candidate",
  );

  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "help-math-vb005-candidate-"));
  try {
    const canvasDirectory = path.join(temporaryRoot, "canvas");
    ...
    const hotspotResult = await run(toolchain.python.invokedPath,
      ["-B", projectPath(HOTSPOT_PARSER), "--swfmill", swfmillXmlPath]);
    const hotspots = validateVb005HotspotEvidence(JSON.parse(hotspotResult.stdout));
    const built = buildSafeRuntime({
      helperSource: helper.toString("utf8"),
      framesHtml: framesHtml.toString("utf8"),
      spec: adapterSpec(),
    ...
    if (check) {
      ...
      validateG4L3Vb005CurrentJsCandidate(artifacts.report);
      await Promise.all([
    ...
    validateG4L3Vb005CurrentJsCandidate(finalArtifacts.report);
    await Promise.all([
...
function help() {
  return `Usage: node scripts/build-g4-l3-vb005-current-js-candidate.mjs [options]\n\n` +
    `Options:\n` +
...
async function main() {
  ...
  process.stdout.write(stableJson(await generateG4L3Vb005CurrentJsCandidate(options)));
}
