...
replaces only migrations/<id>/audit/${MANAGED_DIRECTORY}/ with deterministic
machine evidence. Full FLA/runtime/visual review remains a separate gate.`;
}
...
export function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (["--migration-root", "--id", "--ffdec", "--swfmill", "--java", "--python", "--timeout-ms"].includes(value)) {
...
export async function auditPilotSwfs({
  ...
  const approvedIds = new Set(PILOT_MIGRATIONS.map(({ id }) => id));
  const selectedIds = ids.length ? ids : [...approvedIds];
  for (const id of selectedIds) {
    if (!approvedIds.has(id)) throw new Error(`Not an approved pilot animation ID: ${id}`);
  }
  if (new Set(selectedIds).size !== selectedIds.length) throw new Error("Pilot animation IDs must not be repeated");
...
async function main() {
  try {
    ...
    console.log(`Audited ${reports.length} approved pilot SWF(s). Migration statuses were not changed.`);
  } catch (error) {
