...
static int check_acl_vector(
  ...
  uint8_t actual_digest[HMG4V23_CANONICAL_SHA256_LENGTH];
  uint8_t expected_digest[HMG4V23_CANONICAL_SHA256_LENGTH];
  ...
  REQUIRE(expected_digest_length == HMG4V23_CANONICAL_SHA256_LENGTH);
  REQUIRE(hmg4v23_acl_stream_size(entries, entry_count, &encoded_size) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(encoded_size == expected_stream_length);
  memset(encoded, 0xa5, sizeof(encoded));
  REQUIRE(hmg4v23_acl_stream_encode(entries, entry_count, encoded,
  ...
  REQUIRE(hmg4v23_acl_stream_validate(encoded, bytes_written, &parsed_count) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(parsed_count == entry_count);
  REQUIRE(hmg4v23_acl_stream_sha256(encoded, bytes_written, actual_digest) ==
          HMG4V23_CANONICAL_OK);
...
static int test_acl_normative_vectors(void) {
  ...
  const hmg4v23_acl_entry one[] = {{
      HMG4V23_ACL_TAG_ALLOW_NAMED_UUID,
      qualifier_allow,
      sizeof(qualifier_allow),
      UINT64_C(1),
      UINT64_C(0)}};
  const hmg4v23_acl_entry two[] = {
      {HMG4V23_ACL_TAG_ALLOW_NAMED_UUID,
       qualifier_allow,
       sizeof(qualifier_allow),
       UINT64_C(1),
...
static int test_acl_rejections_and_order(void) {
  ...
  static uint8_t over_bound_stream[
      16 + ((HMG4V23_CANONICAL_MAX_ACL_ENTRIES + 1u) * 40u)];
  size_t one_length = 0;
  size_t two_length = 0;
  size_t bounded_size = 0;
  size_t bytes_written = 99;
  const hmg4v23_acl_entry reversed[] = {
      {HMG4V23_ACL_TAG_DENY_NAMED_UUID,
       qualifier_deny,
       sizeof(qualifier_deny),
       UINT64_C(3),
      ...
      {HMG4V23_ACL_TAG_ALLOW_NAMED_UUID,
       ...
       UINT64_C(1),
       UINT64_C(0)}};
  const hmg4v23_acl_entry duplicate[] = {
      {HMG4V23_ACL_TAG_ALLOW_NAMED_UUID,
       qualifier_allow,
       sizeof(qualifier_allow),
       UINT64_C(1),
      ...
      {HMG4V23_ACL_TAG_DENY_NAMED_UUID,
       ...
       UINT64_C(1),
       UINT64_C(0)}};
  hmg4v23_acl_entry bad = reversed[0];
  ...
  REQUIRE(hmg4v23_acl_stream_validate(
  ...
  write_u32_be(over_bound_stream + 12,
               HMG4V23_CANONICAL_MAX_ACL_ENTRIES + 1u);
  REQUIRE(hmg4v23_acl_stream_validate(
              over_bound_stream, sizeof(over_bound_stream), NULL) ==
          HMG4V23_CANONICAL_BOUND_EXCEEDED);
  REQUIRE(hmg4v23_acl_stream_size(
              NULL, HMG4V23_CANONICAL_MAX_ACL_ENTRIES + 1u, &bounded_size) ==
          HMG4V23_CANONICAL_BOUND_EXCEEDED);
  REQUIRE(bounded_size == 0);

  memcpy(mutated, one, one_length);
  mutated[0] ^= 1;
  REQUIRE(hmg4v23_acl_stream_validate(mutated, one_length, NULL) ==
  ...
  REQUIRE(hmg4v23_acl_stream_validate(mutated, one_length + 1, NULL) ==
          HMG4V23_CANONICAL_TRAILING_BYTES);
  memcpy(mutated, two, two_length);
  memcpy(mutated + 64, mutated + 24, 16);
  REQUIRE(hmg4v23_acl_stream_validate(mutated, two_length, NULL) ==
          HMG4V23_CANONICAL_DUPLICATE);

  memset(output, 0xa5, sizeof(output));
  REQUIRE(hmg4v23_acl_stream_encode(duplicate, 2, output, sizeof(output),
                                   &bytes_written) ==
          HMG4V23_CANONICAL_DUPLICATE);
  REQUIRE(bytes_written == 0);
  REQUIRE(output[0] == 0xa5 && output[sizeof(output) - 1] == 0xa5);
  bad.tag = 3;
  REQUIRE(hmg4v23_acl_stream_encode(&bad, 1, output, sizeof(output),
                                   &bytes_written) ==
          HMG4V23_CANONICAL_BAD_TAG);
  bad = reversed[0];
  bad.qualifier_length = 0;
  REQUIRE(hmg4v23_acl_stream_size(&bad, 1, &one_length) ==
          HMG4V23_CANONICAL_BAD_QUALIFIER);
  bad = reversed[0];
  ...
  REQUIRE(hmg4v23_acl_stream_encode(reversed, 2, output, sizeof(output),
                                   &bytes_written) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(hmg4v23_acl_stream_validate(output, bytes_written, NULL) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(memcmp(output, two, two_length) != 0);
  memset(digest, 0x5a, sizeof(digest));
  REQUIRE(hmg4v23_acl_stream_sha256(mutated, two_length, digest) ==
          HMG4V23_CANONICAL_DUPLICATE);
  REQUIRE(digest[0] == 0x5a && digest[31] == 0x5a);
  return 0;
}

static hmg4v23_xattr_bounds ordinary_xattr_bounds(void) {
...
static int test_xattr_vector_and_hash(void) {
  ...
  const hmg4v23_xattr_bounds bounds = ordinary_xattr_bounds();
  uint8_t encoded[256];
  ...
  REQUIRE(hmg4v23_xattr_set_stream_size(attributes, 4, &bounds,
                                       &encoded_size) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(encoded_size == expected_length);
  REQUIRE(hmg4v23_xattr_set_stream_encode(attributes, 4, &bounds, encoded,
                                         sizeof(encoded), &bytes_written) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(bytes_written == expected_length);
  REQUIRE(memcmp(encoded, expected, expected_length) == 0);
  REQUIRE(hmg4v23_xattr_set_stream_validate(encoded, bytes_written, &bounds,
                                           &parsed_count) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(parsed_count == 4);
  REQUIRE(hmg4v23_xattr_set_stream_sha256(encoded, bytes_written, &bounds,
                                         digest) ==
          HMG4V23_CANONICAL_OK);
...
static int test_xattr_rejections_and_bounds(void) {
  ...
  hmg4v23_xattr_bounds bounds = ordinary_xattr_bounds();
  hmg4v23_xattr_bounds changed = bounds;
  const hmg4v23_xattr sorted[] = {
      {name_a, sizeof(name_a), value_one, sizeof(value_one)},
      {name_b, sizeof(name_b), value_three, sizeof(value_three)}};
  const hmg4v23_xattr unsorted[] = {
      {name_b, sizeof(name_b), value_one, sizeof(value_one)},
      {name_a, sizeof(name_a), value_one, sizeof(value_one)}};
  const hmg4v23_xattr duplicate[] = {
      {name_a, sizeof(name_a), value_one, sizeof(value_one)},
      {name_a, sizeof(name_a), value_three, sizeof(value_three)}};
  hmg4v23_xattr invalid = sorted[0];
  uint8_t encoded[128];
  uint8_t mutated[129];
  uint8_t digest[32];
  size_t encoded_size = 0;
  size_t bytes_written = 0;
  REQUIRE(hmg4v23_xattr_set_stream_size(unsorted, 2, &bounds, &encoded_size) ==
          HMG4V23_CANONICAL_BAD_ORDER);
  REQUIRE(hmg4v23_xattr_set_stream_size(duplicate, 2, &bounds, &encoded_size) ==
          HMG4V23_CANONICAL_DUPLICATE);
  invalid.name = name_nul;
  invalid.name_length = sizeof(name_nul);
  REQUIRE(hmg4v23_xattr_set_stream_size(&invalid, 1, &bounds, &encoded_size) ==
  ...
  REQUIRE(hmg4v23_xattr_set_stream_size(sorted, 2, &changed, &encoded_size) ==
          HMG4V23_CANONICAL_BOUND_EXCEEDED);
  changed = bounds;
  ...
  REQUIRE(hmg4v23_xattr_set_stream_size(sorted, 2, &changed, &encoded_size) ==
          HMG4V23_CANONICAL_BOUND_EXCEEDED);
  REQUIRE(hmg4v23_xattr_set_stream_size(sorted, 2, &bounds, &encoded_size) ==
          HMG4V23_CANONICAL_OK);
  changed = bounds;
  changed.maximum_stream_length = encoded_size - 1;
  REQUIRE(hmg4v23_xattr_set_stream_size(sorted, 2, &changed, &bytes_written) ==
          HMG4V23_CANONICAL_BOUND_EXCEEDED);

  memset(encoded, 0xa5, sizeof(encoded));
  REQUIRE(hmg4v23_xattr_set_stream_encode(unsorted, 2, &bounds, encoded,
                                         sizeof(encoded), &bytes_written) ==
          HMG4V23_CANONICAL_BAD_ORDER);
  REQUIRE(bytes_written == 0 && encoded[0] == 0xa5);
  REQUIRE(hmg4v23_xattr_set_stream_encode(sorted, 2, &bounds, encoded,
  ...
  mutated[0] ^= 1;
  REQUIRE(hmg4v23_xattr_set_stream_validate(mutated, bytes_written, &bounds,
                                           NULL) ==
          HMG4V23_CANONICAL_BAD_MAGIC);
  memcpy(mutated, encoded, bytes_written);
  mutated[11] = 3;
  ...
  REQUIRE(hmg4v23_xattr_set_stream_validate(mutated, bytes_written + 1, &bounds,
                                           NULL) ==
          HMG4V23_CANONICAL_TRAILING_BYTES);
  REQUIRE(hmg4v23_xattr_set_stream_validate(encoded, bytes_written - 1, &bounds,
                                           NULL) ==
          HMG4V23_CANONICAL_TRUNCATED);
  memset(digest, 0x5a, sizeof(digest));
  REQUIRE(hmg4v23_xattr_set_stream_sha256(mutated, bytes_written + 1, &bounds,
                                         digest) ==
          HMG4V23_CANONICAL_TRAILING_BYTES);
  REQUIRE(digest[0] == 0x5a && digest[31] == 0x5a);
  ...
  REQUIRE(hmg4v23_xattr_set_stream_size(&invalid, 1, &changed, &encoded_size) ==
...
static int test_symlink_vector_hash_and_rejections(void) {
  ...
  REQUIRE(hmg4v23_symlink_target_stream_size(sizeof(target) - 1,
                                            &encoded_size) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(encoded_size == expected_length);
  REQUIRE(hmg4v23_symlink_target_stream_encode(
              target, sizeof(target) - 1, encoded, sizeof(encoded),
              &bytes_written) == HMG4V23_CANONICAL_OK);
  REQUIRE(bytes_written == expected_length);
  REQUIRE(memcmp(encoded, expected, expected_length) == 0);
  REQUIRE(hmg4v23_symlink_target_stream_validate(encoded, bytes_written,
                                                &parsed_length) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(parsed_length == sizeof(target) - 1);
  REQUIRE(hmg4v23_symlink_target_stream_sha256(encoded, bytes_written,
                                              stream_digest) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(hmg4v23_symlink_target_sha256(target, sizeof(target) - 1,
                                      direct_digest) ==
          HMG4V23_CANONICAL_OK);
  ...
  encoded[0] ^= 1;
  REQUIRE(hmg4v23_symlink_target_stream_validate(encoded, bytes_written, NULL) ==
          HMG4V23_CANONICAL_BAD_MAGIC);
  encoded[0] ^= 1;
  encoded[11] = 3;
  REQUIRE(hmg4v23_symlink_target_stream_validate(encoded, bytes_written, NULL) ==
  ...
  REQUIRE(hmg4v23_symlink_target_stream_validate(encoded, bytes_written - 1,
                                                NULL) ==
          HMG4V23_CANONICAL_TRUNCATED);

  REQUIRE(hmg4v23_symlink_target_stream_encode(NULL, 0, encoded,
                                              sizeof(encoded),
                                              &bytes_written) ==
          HMG4V23_CANONICAL_OK);
  ...
  REQUIRE(hmg4v23_symlink_target_stream_encode(
              ...
              sizeof(encoded), &bytes_written) == HMG4V23_CANONICAL_OK);
...
static int test_null_and_capacity_contracts(void) {
  ...
  size_t size = 0;
  size_t written = 0;
  REQUIRE(hmg4v23_acl_stream_size(NULL, 0, NULL) ==
          HMG4V23_CANONICAL_NULL_ARGUMENT);
  REQUIRE(hmg4v23_acl_stream_encode(NULL, 0, NULL, 0, &written) ==
          HMG4V23_CANONICAL_NULL_ARGUMENT);
  REQUIRE(written == 0);
  memset(output, 0xa5, sizeof(output));
  REQUIRE(hmg4v23_acl_stream_encode(NULL, 0, output, 15, &written) ==
          HMG4V23_CANONICAL_BUFFER_TOO_SMALL);
  REQUIRE(written == 0 && output[0] == 0xa5);
  REQUIRE(hmg4v23_acl_stream_validate(NULL, 0, NULL) ==
          HMG4V23_CANONICAL_TRUNCATED);
  REQUIRE(hmg4v23_acl_stream_sha256(NULL, 0, digest) ==
          HMG4V23_CANONICAL_TRUNCATED);

  REQUIRE(hmg4v23_xattr_set_stream_size(NULL, 0, NULL, &size) ==
          HMG4V23_CANONICAL_NULL_ARGUMENT);
  REQUIRE(hmg4v23_xattr_set_stream_size(NULL, 0, &bounds, &size) ==
          HMG4V23_CANONICAL_OK);
  REQUIRE(size == 16);
  REQUIRE(hmg4v23_xattr_set_stream_validate(NULL, 0, &bounds, NULL) ==
          HMG4V23_CANONICAL_TRUNCATED);

  REQUIRE(hmg4v23_symlink_target_stream_size(0, NULL) ==
          HMG4V23_CANONICAL_NULL_ARGUMENT);
  REQUIRE(hmg4v23_symlink_target_stream_encode(NULL, 1, output,
                                              sizeof(output), &written) ==
          HMG4V23_CANONICAL_NULL_ARGUMENT);
  REQUIRE(hmg4v23_symlink_target_sha256(NULL, 1, digest) ==
          HMG4V23_CANONICAL_NULL_ARGUMENT);
  return 0;
}
