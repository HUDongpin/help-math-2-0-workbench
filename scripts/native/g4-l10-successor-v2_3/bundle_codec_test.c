...
#define EXPECT_RESULT(actual, expected)                                       \
  do {                                                                        \
    const hmg4v23_bundle_result actual_result = (actual);                       \
    const hmg4v23_bundle_result expected_result = (expected);                   \
    if (actual_result != expected_result) {                                    \
      (void)fprintf(stderr,                                                    \
                    "FAIL %s:%d: got %s, expected %s\n", __FILE__, __LINE__, \
                    hmg4v23_bundle_result_name(actual_result),                  \
                    hmg4v23_bundle_result_name(expected_result));               \
      g_failures += 1;                                                        \
    }                                                                         \
  } while (0)
...
static int append_bytes(test_buffer *buffer, const void *bytes, size_t length) {
  if (buffer == NULL || bytes == NULL ||
      !hmg4v23_range_within(buffer->length, length, buffer->capacity)) {
    return 0;
  }
...
static int append_u32_raw(test_buffer *buffer, uint32_t value) {
  uint8_t bytes[4];
  hmg4v23_write_u32_be(bytes, value);
...
static int append_tlv_u64(test_buffer *buffer, uint16_t tag, uint64_t value) {
  uint8_t bytes[8];
  hmg4v23_write_u64_be(bytes, value);
...
static int build_table(const build_options *options, size_t *table_length) {
  ...
  for (entry_index = 0; entry_index < HMG4V23_BUNDLE_ENTRY_COUNT;
       entry_index += 1) {
    ...
    uint64_t offset = (uint64_t)entry_index * HMG4V23_BUNDLE_ALIGNMENT;
    uint64_t size = 1;
    uint32_t encoded_index = entry_index;
    uint8_t path_type = 0x06;
    hmg4v23_span blob = {&blob_byte, 1};
    if (hmg4v23_sha256(blob, blob_sha256) != HMG4V23_OK) return 0;
    ...
    if (options->oversized_blob && entry_index == 0) {
      size = HMG4V23_BUNDLE_MAX_BLOB_SIZE + 1;
    }
...
static int build_bundle(
  ...
  size_t data_length =
      (size_t)HMG4V23_BUNDLE_ENTRY_COUNT * (size_t)HMG4V23_BUNDLE_ALIGNMENT;
  ...
  if (!hmg4v23_checked_add_size(aligned_start, data_length, &file_length) ||
      file_length > sizeof(g_bundle)) {
    return 0;
  }
  memset(g_bundle, 0, file_length);
  memcpy(g_bundle + HMG4V23_BUNDLE_HEADER_SIZE, g_table, table_length);
  ...
  if (hmg4v23_sha256(span, table_sha256) != HMG4V23_OK) return 0;
  span.bytes = g_bundle + aligned_start;
  span.length = data_length;
  if (hmg4v23_sha256(span, data_sha256) != HMG4V23_OK) return 0;
  memcpy(g_bundle, k_magic, sizeof(k_magic));
  hmg4v23_write_u32_be(g_bundle + 8, 2);
  hmg4v23_write_u32_be(g_bundle + 12, HMG4V23_BUNDLE_ENTRY_COUNT);
  hmg4v23_write_u64_be(g_bundle + 16, (uint64_t)table_length);
  hmg4v23_write_u64_be(g_bundle + 24, (uint64_t)data_length);
...
static void refresh_table_hash(void) {
  const uint64_t table_length = hmg4v23_read_u64_be(g_bundle + 16);
  hmg4v23_span table = {g_bundle + HMG4V23_BUNDLE_HEADER_SIZE,
                       (size_t)table_length};
  uint8_t digest[32];
  EXPECT_TRUE(hmg4v23_sha256(table, digest) == HMG4V23_OK);
  memcpy(g_bundle + 32, digest, sizeof(digest));
}

static void refresh_data_hash(size_t data_start) {
  const uint64_t data_length = hmg4v23_read_u64_be(g_bundle + 24);
  hmg4v23_span data = {g_bundle + data_start, (size_t)data_length};
  uint8_t digest[32];
  EXPECT_TRUE(hmg4v23_sha256(data, digest) == HMG4V23_OK);
...
static build_options default_options(void) {
  ...
  options.list_count = HMG4V23_BUNDLE_ENTRY_COUNT;
  return options;
}

static void test_valid_bundle(void) {
  build_options options = default_options();
  hmg4v23_span bundle = {0};
  hmg4v23_bundle_view view;
  size_t data_start = 0;
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view), HMG4V23_BUNDLE_OK);
...
static void test_header_rejections(void) {
  ...
  uint8_t saved = 0;
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  EXPECT_RESULT(hmg4v23_parse_bundle_header((hmg4v23_span){bundle.bytes, 95},
                                           &header),
                HMG4V23_BUNDLE_TRUNCATED_HEADER);
  EXPECT_RESULT(hmg4v23_parse_bundle_header((hmg4v23_span){bundle.bytes, 97},
                                           &header),
                HMG4V23_BUNDLE_HEADER_LENGTH_MISMATCH);
  saved = g_bundle[0];
  g_bundle[0] ^= 1u;
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_BAD_MAGIC);
  g_bundle[0] = saved;
  hmg4v23_write_u32_be(g_bundle + 8, 3);
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_BAD_VERSION);
  hmg4v23_write_u32_be(g_bundle + 8, 2);
  hmg4v23_write_u32_be(g_bundle + 12, 113);
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_BAD_ENTRY_COUNT);
  hmg4v23_write_u32_be(g_bundle + 12, 114);
  hmg4v23_write_u64_be(g_bundle + 16,
                      HMG4V23_BUNDLE_MAX_TABLE_LENGTH + 1);
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_TABLE_TOO_LARGE);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  hmg4v23_write_u64_be(g_bundle + 24, HMG4V23_BUNDLE_MAX_DATA_LENGTH + 1);
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_DATA_TOO_LARGE);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  bundle.length -= 1;
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_FILE_LENGTH_MISMATCH);
}
...
static void test_digest_padding_gap_and_blob_rejections(void) {
  ...
  g_bundle[32] ^= 1u;
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_TABLE_HASH_MISMATCH);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  g_bundle[64] ^= 1u;
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_DATA_HASH_MISMATCH);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  table_end = HMG4V23_BUNDLE_HEADER_SIZE +
              (size_t)hmg4v23_read_u64_be(g_bundle + 16);
  EXPECT_TRUE(table_end < data_start);
  g_bundle[table_end] = 1;
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_NONZERO_PREDATA_PADDING);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  g_bundle[data_start + 1] = 1;
  refresh_data_hash(data_start);
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_NONZERO_DATA_GAP);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  g_bundle[data_start] ^= 1u;
  refresh_data_hash(data_start);
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_BLOB_HASH_MISMATCH);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
...
static void expect_build_rejection(
    build_options options,
    hmg4v23_bundle_result expected) {
  hmg4v23_span bundle = {0};
  hmg4v23_bundle_view view;
  size_t data_start = 0;
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view), expected);
}

static void test_table_and_entry_rejections(void) {
  build_options options = default_options();
  options.list_count = 113;
  expect_build_rejection(options, HMG4V23_BUNDLE_BAD_ENTRY_COUNT);
  options = default_options();
  options.bad_index = 1;
  expect_build_rejection(options, HMG4V23_BUNDLE_BAD_ENTRY);
  options = default_options();
  options.bad_type = 1;
  expect_build_rejection(options, HMG4V23_BUNDLE_BAD_ENTRY);
  options = default_options();
  options.bad_path = 1;
  expect_build_rejection(options, HMG4V23_BUNDLE_BAD_PATH);
  options = default_options();
  options.duplicate_path = 1;
  expect_build_rejection(options, HMG4V23_BUNDLE_DUPLICATE_PATH);
  options = default_options();
  options.case_collision = 1;
  expect_build_rejection(options, HMG4V23_BUNDLE_CASE_COLLISION);
  options = default_options();
  options.misaligned_offset = 1;
  expect_build_rejection(options, HMG4V23_BUNDLE_BAD_RANGE);
  options = default_options();
...
static void test_table_structure_mutations(void) {
  ...
  hmg4v23_bundle_view view;
  size_t data_start = 0;
  size_t first_member = HMG4V23_BUNDLE_HEADER_SIZE + 8;
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  g_bundle[first_member + 3] = 1;
  refresh_table_hash();
  EXPECT_RESULT(hmg4v23_validate_bundle(bundle, &view),
                HMG4V23_BUNDLE_BAD_ENTRY);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
