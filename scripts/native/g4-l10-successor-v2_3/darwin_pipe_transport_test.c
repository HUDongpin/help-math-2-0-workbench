...
#define EXPECT_PIPE(actual, expected)                                         \
  do {                                                                        \
    const hmg4v23_pipe_result actual_result = (actual);                       \
    ++assertions;                                                             \
    if (actual_result != (expected)) {                                        \
      fprintf(stderr,                                                         \
              "FAIL line %d: got %s expected %s\n",                          \
              __LINE__,                                                       \
              hmg4v23_pipe_result_name(actual_result),                        \
              hmg4v23_pipe_result_name((expected)));                          \
      exit(1);                                                                \
    }                                                                         \
  } while (0)
...
static void install_backend(fake_context *context) {
  const hmg4v23_transport_test_backend backend = {
      ...
      fake_allocate,
      fake_deallocate};
  EXPECT_TRUE(hmg4v23_transport_test_set_backend(&backend));
}
...
static size_t make_request(
  ...
  EXPECT_TRUE(payload_length <= HMG4V23_REQUEST_MAX_PAYLOAD);
  memset(output, 0, HMG4V23_REQUEST_HEADER_SIZE + payload_length);
  memcpy(output, "HMG4V2\0\0", 8);
  hmg4v23_write_u32_be(output + 8, 2);
  hmg4v23_write_u32_be(output + 12, operation);
  hmg4v23_write_u64_be(output + 16, (uint64_t)payload_length);
  EXPECT_TRUE(hmg4v23_sha256(
                  (hmg4v23_span){payload, payload_length}, digest) ==
              HMG4V23_OK);
  memcpy(output + 24, digest, sizeof(digest));
  if (payload_length != 0) {
    memcpy(output + HMG4V23_REQUEST_HEADER_SIZE, payload, payload_length);
  }
  return HMG4V23_REQUEST_HEADER_SIZE + payload_length;
}
...
static size_t make_status_five_response(uint8_t output[MAX_CAPTURE]) {
  ...
  EXPECT_TRUE(hmg4v23_encode_response_frame(
...
static void test_bootstrap(void) {
  ...
  EXPECT_PIPE(hmg4v23_install_and_verify_sigpipe_ignore(), HMG4V23_PIPE_OK);
  context.install_ok = 0;
  EXPECT_PIPE(hmg4v23_install_and_verify_sigpipe_ignore(),
...
static void test_valid_empty_request(void) {
  ...
  install_backend(&context);
  EXPECT_PIPE(hmg4v23_receive_request_fd0(&request), HMG4V23_PIPE_OK);
  EXPECT_TRUE(request.fixed_header_valid == 1);
  EXPECT_TRUE(request.owned_frame_length == HMG4V23_REQUEST_HEADER_SIZE);
  EXPECT_TRUE(request.frame.header.operation == 1);
  EXPECT_TRUE(request.frame.payload.length == 0);
  EXPECT_TRUE(hmg4v23_sha256(
                  (hmg4v23_span){context.input, context.input_length},
                  expected_frame_hash) == HMG4V23_OK);
...
static void test_unframed_and_token_profiles(void) {
  ...
  install_backend(&context);
  EXPECT_PIPE(hmg4v23_receive_request_fd0(&request),
              HMG4V23_PIPE_UNFRAMED_INVALID_HEADER);
  EXPECT_TRUE(request.invalid_header_token_completed == 1);
  EXPECT_TRUE(context.fd2_length == HMG4V23_INVALID_HEADER_TOKEN_SIZE);
  ...
  context.writes[1] =
      (io_event){(int64_t)HMG4V23_INVALID_HEADER_TOKEN_SIZE, 0};
...
static void test_post_header_failure_mappings(void) {
  ...
  EXPECT_TRUE(hmg4v23_pipe_result_diagnostic_code(
                  HMG4V23_PIPE_REQUEST_PAYLOAD_TRUNCATED) ==
              UINT32_C(0x00010001));
  ...
  context.special_clock_value =
      UINT64_C(1000) + HMG4V23_REQUEST_DEADLINE_NS;
  install_backend(&context);
  EXPECT_PIPE(hmg4v23_receive_request_fd0(&request),
...
static void test_response_profiles(void) {
  ...
  reset_context(&context);
  install_backend(&context);
  EXPECT_PIPE(hmg4v23_emit_response_fd1(
                  1,
...
static void test_public_mappings(void) {
  EXPECT_TRUE(hmg4v23_pipe_result_diagnostic_code(HMG4V23_PIPE_OK) == 0);
  EXPECT_TRUE(hmg4v23_pipe_result_diagnostic_code(
  ...
  EXPECT_TRUE(hmg4v23_pipe_result_diagnostic_code(
              ...
              UINT32_C(0x00020017));
  EXPECT_TRUE(hmg4v23_pipe_result_exit_code(
                  HMG4V23_PIPE_UNFRAMED_INVALID_HEADER, 5) == 64);
  EXPECT_TRUE(hmg4v23_pipe_result_exit_code(
                  HMG4V23_PIPE_RESPONSE_TRANSPORT_FAILED, 5) == 74);
  EXPECT_TRUE(hmg4v23_pipe_result_exit_code(HMG4V23_PIPE_OK, 5) == 5);
  EXPECT_TRUE(strcmp(hmg4v23_pipe_result_name((hmg4v23_pipe_result)99),
...
int main(void) {
  ...
  hmg4v23_transport_test_clear_backend();
