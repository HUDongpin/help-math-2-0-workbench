...
static void make_valid(hmg4v23_startup_fd_record records[3], uint32_t bytes) {
  size_t index = 0;
...
int main(void) {
  ...
  hmg4v23_startup_fd_record copy[3];
  const uint32_t bytes = UINT32_C(424);

  make_valid(records, bytes);
  memcpy(copy, records, sizeof(records));
  ASSERT_TRUE(hmg4v23_validate_startup_fd_pass(
                  records, 3, PROX_FDTYPE_PIPE, bytes) ==
              HMG4V23_STARTUP_FD_OK);
  ASSERT_TRUE(hmg4v23_startup_fd_passes_equal(records, copy, 3) == 1);
  ASSERT_TRUE(hmg4v23_startup_fd_passes_equal(NULL, copy, 3) == 0);
  ASSERT_TRUE(hmg4v23_startup_fd_passes_equal(records, copy, 2) == 0);
  ASSERT_TRUE(hmg4v23_validate_startup_fd_pass(
...
#define MUTATE_EXPECT(field, value, expected)                                 \
  ...
  MUTATE_EXPECT(fd, 9, HMG4V23_STARTUP_FD_NUMBER_INVALID);
  MUTATE_EXPECT(fdtype, 1, HMG4V23_STARTUP_FD_TYPE_INVALID);
  MUTATE_EXPECT(status_flags, O_WRONLY, HMG4V23_STARTUP_FD_STATUS_FLAGS_INVALID);
  ...
  MUTATE_EXPECT(pipe.result, 0, HMG4V23_STARTUP_FD_PIPE_INFO_SIZE_INVALID);
  MUTATE_EXPECT(pipe.pipe_handle, 0, HMG4V23_STARTUP_FD_HANDLE_INVALID);
  ...
  ASSERT_TRUE(hmg4v23_startup_fd_passes_equal(records, copy, 3) == 0);
  memcpy(copy, records, sizeof(records));
  copy[2].pipe.pipe_peer_handle ^= 1;
  ASSERT_TRUE(hmg4v23_startup_fd_passes_equal(records, copy, 3) == 0);

  ASSERT_TRUE(strcmp(hmg4v23_startup_fd_result_name(HMG4V23_STARTUP_FD_OK),
                     "ok") == 0);
  ASSERT_TRUE(strcmp(hmg4v23_startup_fd_result_name((hmg4v23_startup_fd_result)999),
                     "unknown") == 0);
