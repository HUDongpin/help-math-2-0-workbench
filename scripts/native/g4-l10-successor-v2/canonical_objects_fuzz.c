...
enum {
  HMG4V2_FUZZ_CASES = 120000,
  HMG4V2_FUZZ_STREAM_CAPACITY = 1024,
  HMG4V2_FUZZ_RAW_CAPACITY = 256,
  HMG4V2_FUZZ_MAX_ACL_ENTRIES = 6,
  HMG4V2_FUZZ_MAX_XATTRS = 5,
  HMG4V2_FUZZ_MAX_XATTR_NAME = 4,
  HMG4V2_FUZZ_MAX_XATTR_VALUE = 16,
  HMG4V2_FUZZ_MAX_SYMLINK_TARGET = 64
};
...
static uint64_t rng_next(hmg4v2_fuzz_rng *rng) {
...
static size_t rng_bounded(hmg4v2_fuzz_rng *rng, size_t bound) {
  if (bound == 0) return 0;
...
static void record_validation(
    hmg4v2_fuzz_stats *stats,
    size_t object_kind,
    hmg4v2_canonical_result result) {
  if (result == HMG4V2_CANONICAL_OK) {
...
static void validate_and_hash_acl(
  ...
  const hmg4v2_canonical_result result =
      hmg4v2_acl_stream_validate(stream, stream_length, NULL);
  record_alignment(stats, pointer_offset);
  record_validation(stats, 0, result);
  if (result == HMG4V2_CANONICAL_OK) {
    const hmg4v2_canonical_result hash_result =
        hmg4v2_acl_stream_sha256(stream, stream_length, digest);
    fold_u64(stats, (uint32_t)hash_result);
    if (hash_result == HMG4V2_CANONICAL_OK) {
...
static void exercise_acl(
    ...
    uint32_t iteration,
    hmg4v2_fuzz_stats *stats) {
  hmg4v2_acl_entry entries[HMG4V2_FUZZ_MAX_ACL_ENTRIES];
  uint8_t qualifiers[HMG4V2_FUZZ_MAX_ACL_ENTRIES][16];
  _Alignas(max_align_t)
      uint8_t output_storage[HMG4V2_FUZZ_STREAM_CAPACITY + 2];
  _Alignas(max_align_t)
      uint8_t mutation_storage[HMG4V2_FUZZ_STREAM_CAPACITY + 2];
  _Alignas(max_align_t) uint8_t raw_storage[HMG4V2_FUZZ_RAW_CAPACITY + 2];
  const size_t pointer_offset = iteration & 1u;
  const size_t mutation_offset = pointer_offset == 0 ? 1 : 0;
  const uint32_t entry_count =
      (uint32_t)rng_bounded(rng, HMG4V2_FUZZ_MAX_ACL_ENTRIES + 1);
  ...
  size_t raw_length = rng_bounded(rng, HMG4V2_FUZZ_RAW_CAPACITY + 1);
  uint32_t index = 0;
  hmg4v2_canonical_result size_result;
  hmg4v2_canonical_result encode_result;
  for (index = 0; index < entry_count; index += 1) {
    ...
    entries[index].qualifier_length = HMG4V2_ACL_NAMED_UUID_LENGTH;
    entries[index].permissions =
        rng_next(rng) & HMG4V2_ACL_ALLOWED_PERMISSION_MASK;
    entries[index].flags = rng_next(rng) & HMG4V2_ACL_ALLOWED_FLAG_MASK;
  }
  if (entry_count > 0) {
  ...
  size_result =
      hmg4v2_acl_stream_size(entries, entry_count, &encoded_size);
...
static hmg4v2_xattr_bounds fuzz_xattr_bounds(void) {
  hmg4v2_xattr_bounds bounds;
  bounds.maximum_attribute_count = HMG4V2_FUZZ_MAX_XATTRS;
  bounds.maximum_name_length = HMG4V2_FUZZ_MAX_XATTR_NAME;
  bounds.maximum_value_length = HMG4V2_FUZZ_MAX_XATTR_VALUE;
  bounds.maximum_total_value_length =
      HMG4V2_FUZZ_MAX_XATTRS * HMG4V2_FUZZ_MAX_XATTR_VALUE;
  bounds.maximum_stream_length = HMG4V2_FUZZ_STREAM_CAPACITY;
  return bounds;
}
...
static void exercise_xattrs(
    ...
    uint32_t iteration,
    hmg4v2_fuzz_stats *stats) {
  hmg4v2_xattr attributes[HMG4V2_FUZZ_MAX_XATTRS];
  uint8_t names[HMG4V2_FUZZ_MAX_XATTRS][HMG4V2_FUZZ_MAX_XATTR_NAME];
  uint8_t values[HMG4V2_FUZZ_MAX_XATTRS][HMG4V2_FUZZ_MAX_XATTR_VALUE];
  _Alignas(max_align_t)
      uint8_t output_storage[HMG4V2_FUZZ_STREAM_CAPACITY + 2];
  _Alignas(max_align_t)
      uint8_t mutation_storage[HMG4V2_FUZZ_STREAM_CAPACITY + 2];
  _Alignas(max_align_t) uint8_t raw_storage[HMG4V2_FUZZ_RAW_CAPACITY + 2];
  hmg4v2_xattr_bounds bounds = fuzz_xattr_bounds();
  const size_t pointer_offset = (iteration + 1u) & 1u;
  const size_t mutation_offset = pointer_offset == 0 ? 1 : 0;
  const uint32_t attribute_count =
      (uint32_t)rng_bounded(rng, HMG4V2_FUZZ_MAX_XATTRS + 1);
  uint8_t *output = output_storage + pointer_offset;
  uint8_t *mutation = mutation_storage + mutation_offset;
  uint8_t *raw = raw_storage + pointer_offset;
  size_t output_capacity = sizeof(output_storage) - pointer_offset;
  ...
  for (index = 0; index < attribute_count; index += 1) {
    const uint32_t name_length =
        (uint32_t)(1 + rng_bounded(rng, HMG4V2_FUZZ_MAX_XATTR_NAME));
    const uint64_t value_length =
        (uint64_t)rng_bounded(rng, HMG4V2_FUZZ_MAX_XATTR_VALUE + 1);
  ...
  if (attribute_count > 0) {
    switch (rng_bounded(rng, 11)) {
      ...
      case 8:
        if (attribute_count > 1) {
          hmg4v2_xattr temporary = attributes[0];
...
static void validate_and_hash_symlink(
  ...
  uint8_t digest[HMG4V2_CANONICAL_SHA256_LENGTH];
  const hmg4v2_canonical_result result =
      hmg4v2_symlink_target_stream_validate(
          stream, stream_length, NULL);
  record_alignment(stats, pointer_offset);
  record_validation(stats, 2, result);
  if (result == HMG4V2_CANONICAL_OK) {
    const hmg4v2_canonical_result hash_result =
        hmg4v2_symlink_target_stream_sha256(
...
static void exercise_symlink(
    ...
    uint32_t iteration,
    hmg4v2_fuzz_stats *stats) {
  uint8_t target[HMG4V2_FUZZ_MAX_SYMLINK_TARGET];
  _Alignas(max_align_t)
      uint8_t output_storage[HMG4V2_FUZZ_STREAM_CAPACITY + 2];
  _Alignas(max_align_t)
      uint8_t mutation_storage[HMG4V2_FUZZ_STREAM_CAPACITY + 2];
  ...
  const uint32_t target_length =
      (uint32_t)rng_bounded(rng, HMG4V2_FUZZ_MAX_SYMLINK_TARGET + 1);
  ...
  size_result =
      hmg4v2_symlink_target_stream_size(target_length, &encoded_size);
  ...
  encode_result = hmg4v2_symlink_target_stream_encode(
      target_pointer, target_length, output, output_capacity, &written);
  record_encode(stats, 2, encode_result);
  record_alignment(stats, pointer_offset);
  if (encode_result == HMG4V2_CANONICAL_OK) {
    uint8_t direct_digest[HMG4V2_CANONICAL_SHA256_LENGTH];
    const hmg4v2_canonical_result direct_hash_result =
        hmg4v2_symlink_target_sha256(
            target_pointer, target_length, direct_digest);
    fold_u64(stats, encoded_size);
    fold_u64(stats, (uint32_t)direct_hash_result);
    if (direct_hash_result == HMG4V2_CANONICAL_OK) {
...
int main(void) {
  hmg4v2_fuzz_rng rng = {UINT64_C(0x6a09e667f3bcc909)};
  hmg4v2_fuzz_stats stats;
  uint32_t iteration = 0;
  size_t object_kind = 0;
  memset(&stats, 0, sizeof(stats));
  stats.checksum = UINT64_C(0xcbf29ce484222325);
  for (iteration = 0; iteration < HMG4V2_FUZZ_CASES; iteration += 1) {
  ...
  printf(
      ...
      (unsigned int)HMG4V2_FUZZ_CASES,
