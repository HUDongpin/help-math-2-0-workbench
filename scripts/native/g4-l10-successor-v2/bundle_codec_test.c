...
#define EXPECT_RESULT(actual, expected)                                       \
  do {                                                                        \
    const hmg4v2_bundle_result actual_result = (actual);                       \
    const hmg4v2_bundle_result expected_result = (expected);                   \
    if (actual_result != expected_result) {                                    \
      (void)fprintf(stderr,                                                    \
                    "FAIL %s:%d: got %s, expected %s\n", __FILE__, __LINE__, \
                    hmg4v2_bundle_result_name(actual_result),                  \
                    hmg4v2_bundle_result_name(expected_result));               \
      g_failures += 1;                                                        \
    }                                                                         \
  } while (0)
...
static int append_bytes(test_buffer *buffer, const void *bytes, size_t length) {
  if (buffer == NULL || bytes == NULL ||
      !hmg4v2_range_within(buffer->length, length, buffer->capacity)) {
    return 0;
  }
...
static int build_table(const build_options *options, size_t *table_length) {
  ...
  for (entry_index = 0; entry_index < HMG4V2_MANAGED_ENTRY_COUNT;
       entry_index += 1) {
    ...
    uint64_t offset = (uint64_t)entry_index * HMG4V2_BUNDLE_ALIGNMENT;
    uint64_t size = 1;
    uint32_t encoded_index = entry_index;
    uint8_t path_type = 0x06;
    hmg4v2_span blob = {&blob_byte, 1};
    if (hmg4v2_sha256(blob, blob_sha256) != HMG4V2_CORE_OK) return 0;
    ...
    if (options->oversized_blob && entry_index == 0) {
      size = HMG4V2_BUNDLE_MAX_BLOB_SIZE + 1;
    }
...
static int build_bundle(
  ...
  memcpy(g_bundle + HMG4V2_BUNDLE_HEADER_SIZE, g_table, table_length);
  ...
  hmg4v2_write_u32_be(g_bundle + 8, 2);
  hmg4v2_write_u32_be(g_bundle + 12, HMG4V2_MANAGED_ENTRY_COUNT);
  hmg4v2_write_u64_be(g_bundle + 16, (uint64_t)table_length);
  hmg4v2_write_u64_be(g_bundle + 24, (uint64_t)data_length);
...
static void refresh_table_hash(void) {
  const uint64_t table_length = hmg4v2_read_u64_be(g_bundle + 16);
  hmg4v2_span table = {g_bundle + HMG4V2_BUNDLE_HEADER_SIZE,
                       (size_t)table_length};
  uint8_t digest[32];
  EXPECT_TRUE(hmg4v2_sha256(table, digest) == HMG4V2_CORE_OK);
  memcpy(g_bundle + 32, digest, sizeof(digest));
}

static void refresh_data_hash(size_t data_start) {
  const uint64_t data_length = hmg4v2_read_u64_be(g_bundle + 24);
  hmg4v2_span data = {g_bundle + data_start, (size_t)data_length};
  uint8_t digest[32];
  EXPECT_TRUE(hmg4v2_sha256(data, digest) == HMG4V2_CORE_OK);
...
static void test_header_rejections(void) {
  ...
  uint8_t saved = 0;
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  EXPECT_RESULT(hmg4v2_parse_bundle_header((hmg4v2_span){bundle.bytes, 95},
                                           &header),
                HMG4V2_BUNDLE_TRUNCATED_HEADER);
  EXPECT_RESULT(hmg4v2_parse_bundle_header((hmg4v2_span){bundle.bytes, 97},
                                           &header),
                HMG4V2_BUNDLE_HEADER_LENGTH_MISMATCH);
  saved = g_bundle[0];
  g_bundle[0] ^= 1u;
  EXPECT_RESULT(hmg4v2_validate_bundle(bundle, &view),
                HMG4V2_BUNDLE_BAD_MAGIC);
  g_bundle[0] = saved;
  hmg4v2_write_u32_be(g_bundle + 8, 3);
  EXPECT_RESULT(hmg4v2_validate_bundle(bundle, &view),
                HMG4V2_BUNDLE_BAD_VERSION);
  hmg4v2_write_u32_be(g_bundle + 8, 2);
  hmg4v2_write_u32_be(g_bundle + 12, 113);
  EXPECT_RESULT(hmg4v2_validate_bundle(bundle, &view),
                HMG4V2_BUNDLE_BAD_ENTRY_COUNT);
  hmg4v2_write_u32_be(g_bundle + 12, 114);
  hmg4v2_write_u64_be(g_bundle + 16,
                      HMG4V2_BUNDLE_MAX_TABLE_LENGTH + 1);
  EXPECT_RESULT(hmg4v2_validate_bundle(bundle, &view),
                HMG4V2_BUNDLE_TABLE_TOO_LARGE);
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  hmg4v2_write_u64_be(g_bundle + 24, HMG4V2_BUNDLE_MAX_DATA_LENGTH + 1);
  EXPECT_RESULT(hmg4v2_validate_bundle(bundle, &view),
...
static void test_digest_padding_gap_and_blob_rejections(void) {
  ...
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  table_end = HMG4V2_BUNDLE_HEADER_SIZE +
              (size_t)hmg4v2_read_u64_be(g_bundle + 16);
...
static void expect_build_rejection(
    build_options options,
    hmg4v2_bundle_result expected) {
  hmg4v2_span bundle = {0};
  hmg4v2_bundle_view view;
  size_t data_start = 0;
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
  EXPECT_RESULT(hmg4v2_validate_bundle(bundle, &view), expected);
}

static void test_table_and_entry_rejections(void) {
  build_options options = default_options();
  options.list_count = 113;
  expect_build_rejection(options, HMG4V2_BUNDLE_BAD_ENTRY_COUNT);
  options = default_options();
  options.bad_index = 1;
  expect_build_rejection(options, HMG4V2_BUNDLE_BAD_ENTRY);
  options = default_options();
  options.bad_type = 1;
  expect_build_rejection(options, HMG4V2_BUNDLE_BAD_ENTRY);
  options = default_options();
  options.bad_path = 1;
  expect_build_rejection(options, HMG4V2_BUNDLE_BAD_PATH);
  options = default_options();
  options.duplicate_path = 1;
  expect_build_rejection(options, HMG4V2_BUNDLE_DUPLICATE_PATH);
  options = default_options();
  options.case_collision = 1;
  expect_build_rejection(options, HMG4V2_BUNDLE_CASE_COLLISION);
  options = default_options();
  options.misaligned_offset = 1;
  expect_build_rejection(options, HMG4V2_BUNDLE_BAD_RANGE);
  options = default_options();
...
static void test_table_structure_mutations(void) {
  ...
  hmg4v2_bundle_view view;
  size_t data_start = 0;
  size_t first_member = HMG4V2_BUNDLE_HEADER_SIZE + 8;
  EXPECT_TRUE(build_bundle(&options, &bundle, &data_start));
