...
enum {
  HMG4V2_CANONICAL_HEADER_LENGTH = 16,
  HMG4V2_ACL_ENTRY_LENGTH = 40
};
...
static hmg4v2_canonical_result validate_acl_entry_fields(
    uint32_t tag,
    const uint8_t *qualifier,
    uint32_t qualifier_length,
    uint64_t permissions,
    uint64_t flags) {
  if (tag != HMG4V2_ACL_TAG_ALLOW_NAMED_UUID &&
      tag != HMG4V2_ACL_TAG_DENY_NAMED_UUID) {
    return HMG4V2_CANONICAL_BAD_TAG;
  }
  if (qualifier_length != HMG4V2_ACL_NAMED_UUID_LENGTH || qualifier == NULL) {
    return HMG4V2_CANONICAL_BAD_QUALIFIER;
  }
  if ((permissions & ~HMG4V2_ACL_ALLOWED_PERMISSION_MASK) != 0) {
    return HMG4V2_CANONICAL_BAD_PERMISSION_MASK;
  }
  if ((flags & ~HMG4V2_ACL_ALLOWED_FLAG_MASK) != 0) {
    return HMG4V2_CANONICAL_BAD_FLAG_MASK;
  }
  return HMG4V2_CANONICAL_OK;
}

static hmg4v2_canonical_result validate_acl_entries(
    const hmg4v2_acl_entry *entries,
    uint32_t entry_count) {
  hmg4v2_canonical_result status = HMG4V2_CANONICAL_OK;
  uint32_t index = 0;
  if (entry_count > 0 && entries == NULL) {
    return HMG4V2_CANONICAL_NULL_ARGUMENT;
  }
  for (index = 0; index < entry_count; index += 1) {
    ...
    status = validate_acl_entry_fields(
        ...
        entries[index].flags);
    if (status != HMG4V2_CANONICAL_OK) return status;
    for (prior = 0; prior < index; prior += 1) {
      if (memcmp(
              entries[prior].qualifier,
              entries[index].qualifier,
              HMG4V2_ACL_NAMED_UUID_LENGTH) == 0) {
        return HMG4V2_CANONICAL_DUPLICATE;
      }
    }
  }
  return HMG4V2_CANONICAL_OK;
...
static hmg4v2_canonical_result sha256_update_bytes(
  ...
  while (remaining > 0) {
    ...
    if (CC_SHA256_Update(context, cursor, (CC_LONG)chunk) != 1) {
      return HMG4V2_CANONICAL_HASH_FAILURE;
    }
    cursor += chunk;
    remaining -= chunk;
  }
...
static hmg4v2_canonical_result sha256_two_parts(
    const uint8_t *first,
    size_t first_length,
  ...
  CC_SHA256_CTX context;
  hmg4v2_canonical_result status = HMG4V2_CANONICAL_OK;
  uint8_t local_digest[HMG4V2_CANONICAL_SHA256_LENGTH];
  if (digest == NULL || (first_length > 0 && first == NULL) ||
      (second_length > 0 && second == NULL)) {
    return HMG4V2_CANONICAL_NULL_ARGUMENT;
  }
  if (CC_SHA256_Init(&context) != 1) return HMG4V2_CANONICAL_HASH_FAILURE;
...
static hmg4v2_canonical_result sha256_bytes(
    const uint8_t *bytes,
    size_t length,
    uint8_t digest[HMG4V2_CANONICAL_SHA256_LENGTH]) {
...
hmg4v2_canonical_result hmg4v2_acl_stream_size(
  ...
  if (encoded_size == NULL) return HMG4V2_CANONICAL_NULL_ARGUMENT;
  *encoded_size = 0;
  if (entry_count > HMG4V2_CANONICAL_MAX_ACL_ENTRIES) {
    return HMG4V2_CANONICAL_BOUND_EXCEEDED;
  }
  if ((size_t)entry_count > SIZE_MAX / HMG4V2_ACL_ENTRY_LENGTH) {
    return HMG4V2_CANONICAL_SIZE_OVERFLOW;
  }
  entries_size = (size_t)entry_count * HMG4V2_ACL_ENTRY_LENGTH;
  if (!checked_add_size(
          HMG4V2_CANONICAL_HEADER_LENGTH, entries_size, encoded_size)) {
    return HMG4V2_CANONICAL_SIZE_OVERFLOW;
  }
  status = validate_acl_entries(entries, entry_count);
  if (status != HMG4V2_CANONICAL_OK) {
    *encoded_size = 0;
    return status;
  }
  return status;
...
hmg4v2_canonical_result hmg4v2_acl_stream_encode(
  ...
  hmg4v2_canonical_result status = HMG4V2_CANONICAL_OK;
  size_t required = 0;
  size_t offset = HMG4V2_CANONICAL_HEADER_LENGTH;
  uint32_t index = 0;
  if (bytes_written == NULL) return HMG4V2_CANONICAL_NULL_ARGUMENT;
  *bytes_written = 0;
  status = hmg4v2_acl_stream_size(entries, entry_count, &required);
  if (status != HMG4V2_CANONICAL_OK) return status;
  if (output == NULL) return HMG4V2_CANONICAL_NULL_ARGUMENT;
  if (output_capacity < required) return HMG4V2_CANONICAL_BUFFER_TOO_SMALL;
  ...
  for (index = 0; index < entry_count; index += 1) {
    ...
    memcpy(output + offset, entries[index].qualifier,
           HMG4V2_ACL_NAMED_UUID_LENGTH);
    offset += HMG4V2_ACL_NAMED_UUID_LENGTH;
...
hmg4v2_canonical_result hmg4v2_acl_stream_validate(
  ...
  if (stream_length < HMG4V2_CANONICAL_HEADER_LENGTH) {
    return HMG4V2_CANONICAL_TRUNCATED;
  }
  if (memcmp(stream, k_acl_magic, sizeof(k_acl_magic)) != 0) {
    return HMG4V2_CANONICAL_BAD_MAGIC;
  }
  if (read_u32_be(stream + 8) != 2) return HMG4V2_CANONICAL_BAD_VERSION;
  count = read_u32_be(stream + 12);
  if ((size_t)count > SIZE_MAX / HMG4V2_ACL_ENTRY_LENGTH ||
      !checked_add_size(
          HMG4V2_CANONICAL_HEADER_LENGTH,
          (size_t)count * HMG4V2_ACL_ENTRY_LENGTH,
          &expected_length)) {
    return HMG4V2_CANONICAL_SIZE_OVERFLOW;
  }
  if (stream_length < expected_length) return HMG4V2_CANONICAL_TRUNCATED;
  if (stream_length > expected_length) {
    return HMG4V2_CANONICAL_TRAILING_BYTES;
  }
  if (count > HMG4V2_CANONICAL_MAX_ACL_ENTRIES) {
    return HMG4V2_CANONICAL_BOUND_EXCEEDED;
  }
  cursor.bytes = stream;
  cursor.length = stream_length;
  cursor.offset = HMG4V2_CANONICAL_HEADER_LENGTH;
  for (index = 0; index < count; index += 1) {
    ...
    if (qualifier_length != HMG4V2_ACL_NAMED_UUID_LENGTH) {
      return HMG4V2_CANONICAL_BAD_QUALIFIER;
    }
    ...
    if (status != HMG4V2_CANONICAL_OK) return status;
    for (prior = 0; prior < index; prior += 1) {
      const size_t prior_offset = HMG4V2_CANONICAL_HEADER_LENGTH +
          ((size_t)prior * HMG4V2_ACL_ENTRY_LENGTH) + 8;
...
hmg4v2_canonical_result hmg4v2_acl_stream_sha256(
    ...
    uint8_t digest[HMG4V2_CANONICAL_SHA256_LENGTH]) {
  hmg4v2_canonical_result status = HMG4V2_CANONICAL_OK;
  if (digest == NULL) return HMG4V2_CANONICAL_NULL_ARGUMENT;
  status = hmg4v2_acl_stream_validate(stream, stream_length, NULL);
  if (status != HMG4V2_CANONICAL_OK) return status;
  return sha256_bytes(stream, stream_length, digest);
}

static hmg4v2_canonical_result validate_xattr_inputs(
    const hmg4v2_xattr *attributes,
    uint32_t attribute_count,
    const hmg4v2_xattr_bounds *bounds,
    size_t *encoded_size) {
  size_t required = HMG4V2_CANONICAL_HEADER_LENGTH;
  ...
  *encoded_size = 0;
  ...
  for (index = 0; index < attribute_count; index += 1) {
    const hmg4v2_xattr *attribute = &attributes[index];
    if (attribute->name_length == 0 || attribute->name == NULL) {
      return HMG4V2_CANONICAL_BAD_NAME;
    }
    ...
    if (index > 0) {
      const hmg4v2_xattr *previous = &attributes[index - 1];
      const int order = unsigned_name_compare(
          previous->name,
          previous->name_length,
          attribute->name,
          attribute->name_length);
      if (order == 0) return HMG4V2_CANONICAL_DUPLICATE;
      if (order > 0) return HMG4V2_CANONICAL_BAD_ORDER;
    }
...
hmg4v2_canonical_result hmg4v2_xattr_set_stream_validate(
  ...
  for (index = 0; index < count; index += 1) {
    ...
    if (name_length == 0 || name_length > bounds->maximum_name_length) {
      return name_length == 0 ? HMG4V2_CANONICAL_BAD_NAME
                              : HMG4V2_CANONICAL_BOUND_EXCEEDED;
    }
    if (!cursor_take(&cursor, name_length, &name)) {
      return HMG4V2_CANONICAL_TRUNCATED;
    }
    if (bytes_have_nul(name, name_length)) return HMG4V2_CANONICAL_BAD_NAME;
    ...
    if (value_length > SIZE_MAX) return HMG4V2_CANONICAL_SIZE_OVERFLOW;
...
hmg4v2_canonical_result hmg4v2_xattr_set_stream_sha256(
  ...
  hmg4v2_canonical_result status = HMG4V2_CANONICAL_OK;
  if (digest == NULL) return HMG4V2_CANONICAL_NULL_ARGUMENT;
  status = hmg4v2_xattr_set_stream_validate(
...
hmg4v2_canonical_result hmg4v2_symlink_target_stream_encode(
  ...
  *bytes_written = 0;
  ...
  status = hmg4v2_symlink_target_stream_size(target_length, &required);
  if (status != HMG4V2_CANONICAL_OK) return status;
  if (output == NULL) return HMG4V2_CANONICAL_NULL_ARGUMENT;
  if (output_capacity < required) return HMG4V2_CANONICAL_BUFFER_TOO_SMALL;
  ...
  if (target_length > 0) {
    memcpy(output + HMG4V2_CANONICAL_HEADER_LENGTH, target, target_length);
  }
...
hmg4v2_canonical_result hmg4v2_symlink_target_stream_sha256(
  ...
  hmg4v2_canonical_result status = HMG4V2_CANONICAL_OK;
  if (digest == NULL) return HMG4V2_CANONICAL_NULL_ARGUMENT;
  status = hmg4v2_symlink_target_stream_validate(
...
hmg4v2_canonical_result hmg4v2_symlink_target_sha256(
    ...
    uint32_t target_length,
    uint8_t digest[HMG4V2_CANONICAL_SHA256_LENGTH]) {
  uint8_t header[HMG4V2_CANONICAL_HEADER_LENGTH];
