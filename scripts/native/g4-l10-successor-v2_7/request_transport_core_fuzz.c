...
int main(void) {
  ...
  for (iteration = 0; iteration < 300000; ++iteration) {
    ...
    if (length >= 56) {
      (void)hmg4v27_parse_request_header((hmg4v27_span){bytes, 56}, &header);
    }
    (void)hmg4v27_validate_buffered_request_frame(
        (hmg4v27_span){bytes, length}, &frame);
    (void)hmg4v27_request_read_decide(
        &symbols, (hmg4v27_read_phase)((next_u32() % 5) + 1),
        (int)(next_u32() & 1), (int64_t)(int32_t)next_u32(),
        (int)(next_u32() % 64), (size_t)(next_u32() % 1024), &decision);
    (void)hmg4v27_response_write_decide(
        &symbols, (int)(next_u32() & 1), (int64_t)(int32_t)next_u32(),
        (int)(next_u32() % 64), (size_t)(next_u32() % 1024), &decision);
    (void)hmg4v27_diagnostic_write_decide(
        &symbols, (int)(next_u32() & 1), (int64_t)(int32_t)next_u32(),
        (int)(next_u32() % 64), &decision);
    (void)hmg4v27_timespec_parts_to_ns(
        (uint64_t)next_u32(), (int64_t)(int32_t)next_u32(), &time_value);
    (void)hmg4v27_deadline_from_start(
        ((uint64_t)next_u32() << 32) | next_u32(),
        (uint64_t)next_u32(), &time_value);
    (void)hmg4v27_poll_timeout_ms(
        (uint64_t)next_u32(), (uint64_t)next_u32(), &timeout);
    (void)hmg4v27_transport_result_name(
        (hmg4v27_transport_result)(next_u32() % 16));
  }
