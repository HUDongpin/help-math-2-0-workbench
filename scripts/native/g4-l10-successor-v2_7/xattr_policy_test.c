...
static int append_u32(buffer *destination, uint16_t tag, uint32_t value) {
  uint8_t bytes[4];
  put_u32(bytes, value);
  return append_tlv(destination, tag, HMG4V27_TLV_U32, bytes, 4);
}

static int append_u64(buffer *destination, uint16_t tag, uint64_t value) {
  uint8_t bytes[8];
  put_u64(bytes, value);
  return append_tlv(destination, tag, HMG4V27_TLV_U64, bytes, 8);
}
...
static int append_rule(
  ...
  if (name.length > UINT32_MAX || value.length > UINT32_MAX ||
      ...
      !append_tlv(&member, 0x7102, HMG4V27_TLV_BYTES,
                  name.bytes, (uint32_t)name.length) ||
      !append_tlv(&member, 0x7103, HMG4V27_TLV_BYTES,
...
static int canonical_xattr_hash(
    const hmg4v27_xattr *rules,
    uint32_t count,
    uint8_t digest[32]) {
  static uint8_t stream[HMG4V27_XATTR_POLICY_MAX_STREAM_LENGTH];
  size_t written = 0;
  if (hmg4v27_xattr_set_stream_encode(
          rules, count, hmg4v27_canonical_xattr_policy_bounds(),
          stream, sizeof(stream), &written) != HMG4V27_CANONICAL_OK) {
    return 0;
  }
  return hmg4v27_xattr_set_stream_sha256(
             stream, written, hmg4v27_canonical_xattr_policy_bounds(), digest) ==
...
static int build_policy(
  ...
  uint8_t header[HMG4V27_AUTHORITY_HEADER_SIZE];
  ...
  for (index = 0; index < count; ++index) {
    ...
    if (!append_rule(&list, ordinal,
                     (hmg4v27_span){rules[index].name, rules[index].name_length},
                     (hmg4v27_span){rules[index].value,
  ...
  if (!append_tlv(&payload, 0x7001, HMG4V27_TLV_SHA256,
                  specification, 32) ||
      !append_u32(&payload, 0x7002, 2) ||
      !append_u32(&payload, 0x7003, 1) ||
      !append_u32(&payload, 0x7004, count) ||
      !append_tlv(&payload, 0x7005, HMG4V27_TLV_LIST,
                  list.bytes, (uint32_t)list.length) ||
      !append_tlv(&payload, 0x7006, HMG4V27_TLV_SHA256,
                  canonical_hash, 32) ||
      ...
      !append_tlv(&payload, 0x700b, HMG4V27_TLV_BOOL, &bool_value, 1) ||
...
int main(void) {
  ...
  static hmg4v27_xattr large_rules[17];
  ...
  hmg4v27_xattr rules[2];
  hmg4v27_xattr reversed[2];
  hmg4v27_xattr_policy_view view;
  ...
  rules[0] = (hmg4v27_xattr){name_a, sizeof(name_a) - 1,
                            value_a, sizeof(value_a)};
  rules[1] = (hmg4v27_xattr){name_b, sizeof(name_b) - 1,
                            value_b, sizeof(value_b)};

  CHECK(hmg4v27_xattr_bounds_are_exact_policy(
      hmg4v27_canonical_xattr_policy_bounds()));
  CHECK(build_policy(rules, NULL, 2, 0, hmg4v27_successor_sha256, NULL, &frame));
  CHECK(hmg4v27_validate_xattr_policy(
            (hmg4v27_span){frame.bytes, frame.length}, &view) ==
        HMG4V27_XATTR_POLICY_OK);
  ...
  frame.length = 0;
  CHECK(build_policy(NULL, NULL, 0, 1, hmg4v27_successor_sha256, NULL, &frame));
  CHECK(hmg4v27_validate_xattr_policy(
            (hmg4v27_span){frame.bytes, frame.length}, &view) ==
        HMG4V27_XATTR_POLICY_OK);
  ...
  mutation.bytes[24] ^= 1;
  CHECK(hmg4v27_validate_xattr_policy(
            (hmg4v27_span){mutation.bytes, mutation.length}, &view) ==
  ...
  CHECK(build_policy(rules, bad_ordinals, 2, 0,
                     hmg4v27_successor_sha256, NULL, &frame));
  CHECK(hmg4v27_validate_xattr_policy(
            (hmg4v27_span){frame.bytes, frame.length}, &view) ==
  ...
  CHECK(build_policy(reversed, NULL, 2, 0,
                     hmg4v27_successor_sha256, bad_hash, &frame));
  CHECK(hmg4v27_validate_xattr_policy(
            (hmg4v27_span){frame.bytes, frame.length}, &view) ==
  ...
  for (index = 0; index < 17; ++index) {
    ...
    names[index][1] = 0;
    large_rules[index] = (hmg4v27_xattr){names[index], 1,
  ...
  CHECK(hmg4v27_validate_xattr_policy((hmg4v27_span){NULL, 1}, &view) ==
        HMG4V27_XATTR_POLICY_NULL_ARGUMENT);
  CHECK(hmg4v27_validate_xattr_policy(
            (hmg4v27_span){frame.bytes, frame.length}, NULL) ==
        HMG4V27_XATTR_POLICY_NULL_ARGUMENT);
  CHECK(strcmp(hmg4v27_xattr_policy_result_name(
                   HMG4V27_XATTR_POLICY_STREAM_HASH_MISMATCH),
               "stream_hash_mismatch") == 0);
  CHECK(strcmp(hmg4v27_xattr_policy_result_name(
                   (hmg4v27_xattr_policy_result)999),
               "unknown") == 0);
