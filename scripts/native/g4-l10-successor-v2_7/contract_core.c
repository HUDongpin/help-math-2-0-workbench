...
const uint8_t hmg4v27_successor_sha256[32] = {
    0x72, 0xb2, 0x88, 0x27, 0xb7, 0xc7, 0xba, 0xff,
    0x35, 0x8a, 0xbe, 0xa3, 0x3c, 0x0b, 0x91, 0x9c,
    0x32, 0x95, 0x3e, 0xc9, 0xbc, 0xb0, 0x2f, 0x4f,
    0x56, 0xa7, 0x53, 0x4a, 0x4f, 0x78, 0xe4, 0xcc};
const uint8_t hmg4v27_predecessor_sha256[32] = {
    0x3c, 0xe5, 0xbf, 0x0d, 0x79, 0xc0, 0x03, 0xa7,
    0x81, 0x15, 0xbe, 0x85, 0x82, 0x8b, 0x0d, 0x36,
    0xca, 0x8e, 0x18, 0x2e, 0x65, 0xd4, 0x32, 0x9c,
    0x58, 0xba, 0x9a, 0xa3, 0x39, 0x3c, 0x43, 0x6a};
const uint8_t hmg4v27_gate_a_sha256[32] = {
    0xcf, 0x91, 0x9f, 0xe4, 0x47, 0x87, 0x95, 0x14,
    0x01, 0x57, 0xc6, 0x03, 0x34, 0x80, 0x64, 0xc1,
    0x7b, 0x2c, 0x8c, 0x65, 0x51, 0x9a, 0x27, 0x35,
    0xc8, 0x42, 0x75, 0x9f, 0x59, 0xb6, 0x88, 0x26};
...
static const hmg4v27_diagnostic diagnostics[] = {
...
static const hmg4v27_rollback_reason rollback_reasons[] = {
...
static const hmg4v27_direction directions[] = {
...
static const hmg4v27_xattr_limits xattr_limits = {
...
uint64_t hmg4v27_read_u64_be(const uint8_t bytes[8]) {
  return ((uint64_t)hmg4v27_read_u32_be(bytes) << 32) |
         (uint64_t)hmg4v27_read_u32_be(bytes + 4);
}

int64_t hmg4v27_read_s64_be(const uint8_t bytes[8]) {
  const uint64_t bits = hmg4v27_read_u64_be(bytes);
  if ((bits & (UINT64_C(1) << 63)) == 0) return (int64_t)bits;
  if (bits == (UINT64_C(1) << 63)) return INT64_MIN;
...
void hmg4v27_write_s64_be(uint8_t bytes[8], int64_t value) {
  ...
  if (value >= 0) {
    ...
    bits = (~(uint64_t)(-value)) + UINT64_C(1);
  }
  hmg4v27_write_u64_be(bytes, bits);
}
...
hmg4v27_result hmg4v27_sha256(
  ...
  if (digest == NULL || (input.length != 0 && input.bytes == NULL)) {
    return HMG4V27_NULL_ARGUMENT;
  }
  if (cursor == NULL) cursor = &empty;
  if (CC_SHA256_Init(&context) != 1) return HMG4V27_PAYLOAD_HASH_MISMATCH;
  while (remaining != 0) {
    const size_t chunk = remaining > (size_t)UINT32_MAX
                             ? (size_t)UINT32_MAX
                             : remaining;
    if (CC_SHA256_Update(&context, cursor, (CC_LONG)chunk) != 1) {
      return HMG4V27_PAYLOAD_HASH_MISMATCH;
    }
    cursor += chunk;
    remaining -= chunk;
  }
...
hmg4v27_result hmg4v27_parse_authority_header(
  ...
  if (result == NULL || (header_bytes.bytes == NULL && header_bytes.length != 0)) return HMG4V27_NULL_ARGUMENT;
  if (header_bytes.length != HMG4V27_AUTHORITY_HEADER_SIZE) return HMG4V27_TRUNCATED_HEADER;
  memcpy(result->magic, header_bytes.bytes, 8);
  if (!magic_exists(result->magic)) return HMG4V27_BAD_MAGIC;
  if (hmg4v27_read_u32_be(header_bytes.bytes + 8) != 2) return HMG4V27_BAD_VERSION;
  result->kind = hmg4v27_read_u32_be(header_bytes.bytes + 12);
  rule = find_rule(result->magic, result->kind);
  if (rule == NULL) return HMG4V27_BAD_KIND;
  result->payload_length = hmg4v27_read_u64_be(header_bytes.bytes + 16);
  if (result->payload_length > rule->maximum_payload) return HMG4V27_PAYLOAD_TOO_LARGE;
...
hmg4v27_result hmg4v27_validate_authority_envelope(
  ...
  hmg4v27_result parsed;
  size_t total;
  uint8_t digest[CC_SHA256_DIGEST_LENGTH];
  if (result == NULL || (frame_bytes.bytes == NULL && frame_bytes.length != 0)) return HMG4V27_NULL_ARGUMENT;
  if (frame_bytes.length < HMG4V27_AUTHORITY_HEADER_SIZE) return HMG4V27_TRUNCATED_HEADER;
  parsed = hmg4v27_parse_authority_header((hmg4v27_span){frame_bytes.bytes, HMG4V27_AUTHORITY_HEADER_SIZE}, &result->header);
  if (parsed != HMG4V27_OK) return parsed;
  if (result->header.payload_length > (uint64_t)SIZE_MAX) return HMG4V27_SIZE_OVERFLOW;
  if (!hmg4v27_checked_add_size(HMG4V27_AUTHORITY_HEADER_SIZE, (size_t)result->header.payload_length, &total)) return HMG4V27_SIZE_OVERFLOW;
  if (total != frame_bytes.length) return HMG4V27_FRAME_LENGTH_MISMATCH;
  result->payload.bytes = frame_bytes.bytes + HMG4V27_AUTHORITY_HEADER_SIZE;
  result->payload.length = (size_t)result->header.payload_length;
  if (CC_SHA256(result->payload.bytes, (CC_LONG)result->payload.length, digest) == NULL) return HMG4V27_PAYLOAD_HASH_MISMATCH;
  if (memcmp(digest, result->header.payload_sha256, sizeof(digest)) != 0) return HMG4V27_PAYLOAD_HASH_MISMATCH;
  return HMG4V27_OK;
}

int hmg4v27_tlv_type_is_known(uint8_t type) {
  return type >= UINT8_C(0x01) && type <= UINT8_C(0x11);
}

const hmg4v27_xattr_limits *hmg4v27_xattr_policy_limits(void) { return &xattr_limits; }

const hmg4v27_diagnostic *hmg4v27_diagnostic_at(size_t ordinal) {
  return ordinal < HMG4V27_DIAGNOSTIC_COUNT ? &diagnostics[ordinal] : NULL;
}

hmg4v27_result hmg4v27_lookup_diagnostic(uint32_t code, const hmg4v27_diagnostic **result) {
  size_t low = 0, high = HMG4V27_DIAGNOSTIC_COUNT;
  if (result == NULL) return HMG4V27_NULL_ARGUMENT;
  while (low < high) {
    size_t mid = low + (high - low) / 2;
    if (diagnostics[mid].code == code) { *result = &diagnostics[mid]; return HMG4V27_OK; }
    if (diagnostics[mid].code < code) low = mid + 1; else high = mid;
  }
  *result = NULL;
  return HMG4V27_UNKNOWN_DIAGNOSTIC;
}

hmg4v27_result hmg4v27_validate_diagnostic_status(uint32_t code, uint32_t status) {
  const hmg4v27_diagnostic *diagnostic = NULL;
  hmg4v27_result found = hmg4v27_lookup_diagnostic(code, &diagnostic);
  if (found != HMG4V27_OK) return found;
  return diagnostic->status == status ? HMG4V27_OK : HMG4V27_DIAGNOSTIC_STATUS_MISMATCH;
}

const hmg4v27_rollback_reason *hmg4v27_rollback_reason_at(size_t ordinal) {
  return ordinal < HMG4V27_ROLLBACK_REASON_COUNT ? &rollback_reasons[ordinal] : NULL;
}

hmg4v27_result hmg4v27_lookup_rollback_reason(uint32_t reason, const hmg4v27_rollback_reason **result) {
  if (result == NULL) return HMG4V27_NULL_ARGUMENT;
  if (reason >= 1 && reason <= HMG4V27_ROLLBACK_REASON_COUNT) { *result = &rollback_reasons[reason - 1]; return HMG4V27_OK; }
  *result = NULL;
  return HMG4V27_UNKNOWN_ROLLBACK_REASON;
}

const hmg4v27_direction *hmg4v27_direction_at(size_t ordinal) {
  return ordinal < HMG4V27_DIRECTION_COUNT ? &directions[ordinal] : NULL;
}

hmg4v27_result hmg4v27_lookup_direction(uint32_t direction, const hmg4v27_direction **result) {
  if (result == NULL) return HMG4V27_NULL_ARGUMENT;
  if (direction >= 1 && direction <= HMG4V27_DIRECTION_COUNT) { *result = &directions[direction - 1]; return HMG4V27_OK; }
  *result = NULL;
  return HMG4V27_UNKNOWN_DIRECTION;
}
...
hmg4v27_result hmg4v27_poll_decide(
  ...
  uint16_t values[5];
  size_t left, right;
  if (symbols == NULL || decision == NULL) return HMG4V27_NULL_ARGUMENT;
  if (endpoint != HMG4V27_POLL_ENDPOINT_REQUEST && endpoint != HMG4V27_POLL_ENDPOINT_RESPONSE) return HMG4V27_BAD_POLL_ENDPOINT;
  ...
  for (left=0; left<5; ++left) {
    if (!single_bit(values[left])) return HMG4V27_BAD_POLL_SYMBOLS;
    for (right=left+1; right<5; ++right) if (values[left] == values[right]) return HMG4V27_BAD_POLL_SYMBOLS;
  }
  known = (uint16_t)(values[0]|values[1]|values[2]|values[3]|values[4]);
  if (post_at_or_after_deadline) { *decision = HMG4V27_POLL_DEADLINE; return HMG4V27_OK; }
  if (poll_return <= 0 && revents != 0) { *decision = HMG4V27_POLL_TRANSPORT_ERROR; return HMG4V27_OK; }
  if (poll_return == -1) {
    *decision = poll_errno == symbols->eintr_value ? HMG4V27_POLL_AGAIN : HMG4V27_POLL_TRANSPORT_ERROR;
    return HMG4V27_OK;
  }
  if (poll_return == 0) { *decision = HMG4V27_POLL_AGAIN; return HMG4V27_OK; }
  if (poll_return != 1 || (revents & (uint16_t)~known) != 0) { *decision = HMG4V27_POLL_TRANSPORT_ERROR; return HMG4V27_OK; }
  if (endpoint == HMG4V27_POLL_ENDPOINT_REQUEST) {
    if ((revents & symbols->pollout) != 0 || (revents & symbols->pollnval) != 0 || (revents & symbols->pollerr) != 0) *decision = HMG4V27_POLL_TRANSPORT_ERROR;
    else if (revents == symbols->pollin) *decision = HMG4V27_POLL_REQUEST_RETRY_READ;
    else if (revents == symbols->pollhup) *decision = HMG4V27_POLL_REQUEST_EOF_READ;
    else if (revents == (uint16_t)(symbols->pollin|symbols->pollhup)) *decision = HMG4V27_POLL_REQUEST_DRAIN_READ;
    else *decision = HMG4V27_POLL_TRANSPORT_ERROR;
  } else {
    if ((revents & symbols->pollin) != 0 || (revents & symbols->pollnval) != 0 || (revents & symbols->pollerr) != 0 || (revents & symbols->pollhup) != 0) *decision = HMG4V27_POLL_TRANSPORT_ERROR;
    else if (revents == symbols->pollout) *decision = HMG4V27_POLL_RESPONSE_RETRY_WRITE;
    else *decision = HMG4V27_POLL_TRANSPORT_ERROR;
  }
  return HMG4V27_OK;
}
