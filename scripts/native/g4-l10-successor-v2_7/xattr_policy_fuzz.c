...
static size_t append_u32(
  ...
  uint8_t bytes[4];
  put_u32(bytes, value);
  return append_tlv(output, offset, tag, HMG4V27_TLV_U32, bytes, 4);
}
...
static size_t append_u64(
  ...
  uint8_t bytes[8];
  put_u64(bytes, value);
  return append_tlv(output, offset, tag, HMG4V27_TLV_U64, bytes, 8);
}
...
static size_t make_empty_policy(uint8_t frame[512], uint8_t empty_hash[32]) {
  ...
  offset = append_tlv(payload, offset, 0x7001, HMG4V27_TLV_SHA256,
  ...
  offset = append_tlv(payload, offset, 0x7005, HMG4V27_TLV_LIST, list, 4);
  offset = append_tlv(payload, offset, 0x7006, HMG4V27_TLV_SHA256,
                      empty_hash, 32);
  ...
  offset = append_tlv(payload, offset, 0x700b, HMG4V27_TLV_BOOL, &one, 1);
...
int main(void) {
  ...
  for (iteration = 0; iteration < UINT64_C(300000); ++iteration) {
    ...
    result = hmg4v27_validate_xattr_policy(
        (hmg4v27_span){candidate + 1, length}, &view);
    if (result == HMG4V27_XATTR_POLICY_OK) {
