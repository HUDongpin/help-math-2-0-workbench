...
typedef struct {
  char path_bytes[HMG4V27_MANAGED_ENTRY_COUNT][32];
  hmg4v27_span paths[HMG4V27_MANAGED_ENTRY_COUNT];
  hmg4v27_request_path_authority authority;
} authority_fixture;
...
static void initialize_authority(authority_fixture *fixture) {
  size_t index;
  CHECK(fixture != NULL);
  for (index = 0; index < HMG4V27_MANAGED_ENTRY_COUNT; ++index) {
...
static void build_root(builder *root,
  ...
  append_tlv(root, 0x0207, HMG4V27_TLV_BYTES, fsid, sizeof(fsid));
  append_hash(root, 0x0208, 0x52);
  append_tlv(root, 0x0209, HMG4V27_TLV_APPROVED_ABS_ROOT_PATH,
...
static void build_entry(builder *entry, size_t ordinal,
  ...
  uint32_t role = ordinal == HMG4V27_MANAGED_ENTRY_COUNT - 1 ? 2 : 1;
  ...
  append_tlv(entry, 0x0103, HMG4V27_TLV_POLICY_REL_PATH,
...
static void build_entries(builder *list,
  ...
  size_t ordinal;
  list->length = 0;
  put_u32(count, (uint32_t)HMG4V27_MANAGED_ENTRY_COUNT);
  append_bytes(list, count, sizeof(count));
  for (ordinal = 0; ordinal < HMG4V27_MANAGED_ENTRY_COUNT; ++ordinal) {
...
static void append_common(builder *payload, const builder *root,
  ...
  append_exact_hash(payload, 0x0001, hmg4v27_successor_sha256);
  ...
  append_tlv(payload, 0x0007, HMG4V27_TLV_STRUCT, root->bytes, root->length);
  ...
  append_tlv(payload, 0x000d, HMG4V27_TLV_LIST,
...
static void build_verify(builder *payload, const builder *root,
  ...
  if (target == 2) {
    ...
    append_tlv(payload, 0x000f, HMG4V27_TLV_SAFE_CUSTODY_LEAF,
               leaf, strlen(leaf));
...
static void build_apply(builder *payload, const builder *root,
  ...
  append_tlv(payload, 0x0026, HMG4V27_TLV_APPROVED_EVIDENCE_REL_PATH,
             path, strlen(path));
  append_u32(payload, 0x0027, 12);
  append_hash(payload, 0x0028, 0x28);
}
...
static void build_recover(builder *payload, const builder *root,
  ...
  append_tlv(payload, 0x0011, HMG4V27_TLV_BYTES,
  ...
  append_tlv(payload, 0x0012, HMG4V27_TLV_SAFE_CUSTODY_LEAF,
             leaf, strlen(leaf));
  ...
  CHECK(snprintf(path, sizeof(path), "authorizations/recover-%s.auth",
                 authorization_hex) > 0);
  append_tlv(payload, 0x0014, HMG4V27_TLV_APPROVED_EVIDENCE_REL_PATH,
             path, strlen(path));
...
static hmg4v27_request_result validate(
    uint32_t operation, const builder *payload,
    const hmg4v27_request_path_authority *authority,
    hmg4v27_request_summary *summary) {
  return hmg4v27_validate_request_payload(
      operation, (hmg4v27_span){payload->bytes, payload->length},
      authority, summary);
}
...
static void test_positive_matrices(void) {
  ...
  CHECK(validate(1, &payload, &fixture.authority, &summary) == HMG4V27_REQUEST_OK);
  CHECK(summary.operation == 1 && summary.expected_transition_count == 0);
  build_verify(&payload, &root, &entries, 1, 0);
  CHECK(validate(2, &payload, &fixture.authority, &summary) == HMG4V27_REQUEST_OK);
  ...
  CHECK(validate(3, &payload, &fixture.authority, &summary) == HMG4V27_REQUEST_OK);
  CHECK(summary.expected_transition_count == 171);
  build_recover(&payload, &root, &entries, 0, 0);
  CHECK(validate(4, &payload, &fixture.authority, &summary) == HMG4V27_REQUEST_OK);
  CHECK(summary.predecessor_present_count == 57 && summary.tlv_result == HMG4V27_TLV_OK);
}
...
static void test_fail_closed_cases(void) {
  ...
  CHECK(hmg4v27_validate_request_payload(1,
      (hmg4v27_span){payload.bytes, payload.length}, &fixture.authority, NULL) ==
      HMG4V27_REQUEST_NULL_ARGUMENT);

  altered = fixture;
  for (index = 0; index < HMG4V27_MANAGED_ENTRY_COUNT; ++index) {
    altered.paths[index] = (hmg4v27_span){
  ...
  CHECK(validate(1, &payload, &altered.authority, &summary) == HMG4V27_REQUEST_OK);
  build_verify(&payload, &root, &entries, 1, 0);
  ...
  altered.authority.approved_root_path = (hmg4v27_span){
...
static void test_result_names(void) {
  unsigned value;
  for (value = 0; value <= (unsigned)HMG4V27_REQUEST_EVIDENCE_BINDING_INVALID;
       ++value) {
    CHECK(strcmp(hmg4v27_request_result_name((hmg4v27_request_result)value),
                 "UNKNOWN_REQUEST_RESULT") != 0);
  }
  CHECK(strcmp(hmg4v27_request_result_name((hmg4v27_request_result)999),
               "UNKNOWN_REQUEST_RESULT") == 0);
}
