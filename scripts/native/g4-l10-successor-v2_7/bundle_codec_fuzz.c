...
static int append_bytes(
  ...
  if (builder == NULL || bytes == NULL ||
      !hmg4v27_range_within(builder->length, length, builder->capacity)) {
    return 0;
  }
...
static int append_tlv_u64(
    ...
    uint64_t value) {
  uint8_t encoded[8];
  hmg4v27_write_u64_be(encoded, value);
...
static int build_valid_table(size_t *table_length) {
  ...
  if (table_length == NULL ||
      !append_raw_u32(&table, HMG4V27_BUNDLE_ENTRY_COUNT)) {
    return 0;
  }
  for (index = 0; index < HMG4V27_BUNDLE_ENTRY_COUNT; index += 1) {
    ...
    const hmg4v27_span blob = {&blob_byte, 1};
    path_length = snprintf(path, sizeof(path), "bundle/output-%03u.bin", index);
    if (path_length <= 0 || (size_t)path_length >= sizeof(path) ||
        hmg4v27_sha256(blob, blob_sha256) != HMG4V27_OK) {
      return 0;
    }
    ...
    if (!append_tlv_u32(&entry, 0x0401, index) ||
        ...
        !append_tlv_u64(&entry, 0x0403,
                        (uint64_t)index * HMG4V27_BUNDLE_ALIGNMENT) ||
...
static int build_valid_bundle(hmg4v27_span *bundle, size_t *data_start) {
  ...
  if (bundle == NULL || data_start == NULL ||
      ...
      !hmg4v27_checked_add_size(HMG4V27_BUNDLE_HEADER_SIZE, table_length,
                              &table_end) ||
      !hmg4v27_checked_add_size(table_end, 4095u, &aligned_start)) {
    return 0;
  }
  ...
  memcpy(g_valid_bundle + HMG4V27_BUNDLE_HEADER_SIZE, g_table, table_length);
  for (index = 0; index < HMG4V27_BUNDLE_ENTRY_COUNT; index += 1) {
  ...
  hmg4v27_write_u32_be(g_valid_bundle + 8, 2);
  hmg4v27_write_u32_be(g_valid_bundle + 12, HMG4V27_BUNDLE_ENTRY_COUNT);
  hmg4v27_write_u64_be(g_valid_bundle + 16, (uint64_t)table_length);
  hmg4v27_write_u64_be(g_valid_bundle + 24, (uint64_t)data_length);
...
static int refresh_hash(
  ...
  uint8_t digest[32];
  if (hmg4v27_sha256(bytes, digest) != HMG4V27_OK) return 0;
...
static uint64_t run_malformed_buffer_fuzz(void) {
  ...
  for (iteration = 0; iteration < FUZZ_ITERATIONS; iteration += 1) {
    size_t length = HMG4V27_BUNDLE_HEADER_SIZE +
                    (size_t)(next_random_u64() %
                             (SMALL_BUFFER_CAPACITY -
                              HMG4V27_BUNDLE_HEADER_SIZE));
    size_t header_length = HMG4V27_BUNDLE_HEADER_SIZE;
    uint8_t *const case_bytes =
        g_small_buffer + (((iteration & 1u) == 0u) ? 0u : 1u);
    hmg4v27_span input;
    hmg4v27_bundle_header header;
    hmg4v27_bundle_view view;
    hmg4v27_bundle_result header_result;
    hmg4v27_bundle_result bundle_result;
    uint64_t table_length = 0;
    uint64_t data_length = 0;
    const uint64_t aligned =
        (uint64_t)(next_random_u64() % 4u) * HMG4V27_BUNDLE_ALIGNMENT;
    fill_random(case_bytes, length);
    memcpy(case_bytes, magic, sizeof(magic));
    hmg4v27_write_u32_be(case_bytes + 8,
                        (iteration % 11u) == 0u ? 3u : 2u);
    hmg4v27_write_u32_be(case_bytes + 12,
    ...
    hmg4v27_write_u64_be(case_bytes + 16, table_length);
    hmg4v27_write_u64_be(case_bytes + 24, data_length);
    if ((iteration % 7u) == 0u) case_bytes[0] ^= 1u;
    if ((iteration % 17u) == 0u) {
      header_length = (size_t)(next_random_u64() %
                               HMG4V27_BUNDLE_HEADER_SIZE);
    } else if ((iteration % 19u) == 0u) {
      header_length = HMG4V27_BUNDLE_HEADER_SIZE + 1u;
    }
    input.bytes = case_bytes;
    input.length = length;
    header_result = hmg4v27_parse_bundle_header(
        (hmg4v27_span){case_bytes, header_length}, &header);
    bundle_result = hmg4v27_validate_bundle(input, &view);
    if (bundle_result == HMG4V27_BUNDLE_OK) {
...
static int run_valid_bundle_mutations(uint64_t *accumulator) {
  ...
  if (accumulator == NULL || !build_valid_bundle(&bundle, &data_start) ||
      hmg4v27_validate_bundle(bundle, &view) != HMG4V27_BUNDLE_OK) {
    return 0;
  }
  table_length = (size_t)hmg4v27_read_u64_be(g_valid_bundle + 16);
  ...
  for (iteration = 0; iteration < RAW_VALID_MUTATIONS; iteration += 1) {
    ...
    status = hmg4v27_validate_bundle(bundle, &view);
  ...
  for (iteration = 0; iteration < REHASHED_TABLE_MUTATIONS; iteration += 1) {
    ...
    const size_t offset = HMG4V27_BUNDLE_HEADER_SIZE + relative;
    const uint8_t saved = g_valid_bundle[offset];
    const hmg4v27_span table = {
        g_valid_bundle + HMG4V27_BUNDLE_HEADER_SIZE, table_length};
    hmg4v27_bundle_result status;
    g_valid_bundle[offset] ^= (uint8_t)(1u << (iteration & 7u));
  ...
  for (iteration = 0; iteration < REHASHED_DATA_MUTATIONS; iteration += 1) {
    ...
    const uint8_t saved = g_valid_bundle[offset];
    const hmg4v27_span data = {g_valid_bundle + data_start, data_length};
    hmg4v27_bundle_result status;
    g_valid_bundle[offset] ^= (uint8_t)(1u << (iteration & 7u));
  ...
  return hmg4v27_validate_bundle(bundle, &view) == HMG4V27_BUNDLE_OK;
}
