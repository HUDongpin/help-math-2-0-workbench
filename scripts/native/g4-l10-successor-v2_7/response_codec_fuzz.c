...
static int response_result_is_bounded(hmg4v27_response_result result) {
  return result >= HMG4V27_RESPONSE_OK &&
         result <= HMG4V27_RESPONSE_HASH_ENGINE_FAILURE;
}
...
int main(void) {
  ...
  for (iteration = 0; iteration < FUZZ_CASES; ++iteration) {
    ...
    if (family == 0) {
      ...
      result = hmg4v27_validate_response_frame(
          (iteration % 4u) + 1u, (hmg4v27_span){bytes, length},
      ...
      const uint8_t *context =
          (base->fields.present_fields & HMG4V27_RESPONSE_HAS_TRANSACTION) != 0
      ...
      if ((iteration % 7u) == 0u &&
          base->frame_length >= HMG4V27_RESPONSE_HEADER_SIZE) {
        uint8_t payload_hash[32];
        const size_t payload_length =
            base->frame_length - HMG4V27_RESPONSE_HEADER_SIZE;
        if (hmg4v27_sha256(
                (hmg4v27_span){bytes + HMG4V27_RESPONSE_HEADER_SIZE,
                               payload_length},
                payload_hash) != HMG4V27_OK) {
          return 3;
        }
      ...
      result = hmg4v27_validate_response_frame(
          ...
          (hmg4v27_span){bytes, base->frame_length}, context, &view);
      ...
      if (selector == 9) {
        mutated.fields.journal_leaf =
            (hmg4v27_span){storage, (size_t)(next_u64() % 300u)};
      }
      result = hmg4v27_encode_response_frame(
