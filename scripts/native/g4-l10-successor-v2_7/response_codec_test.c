...
#define EXPECT_RESULT(actual, expected)                                       \
  do {                                                                        \
    const hmg4v27_response_result actual_result = (actual);                    \
    const hmg4v27_response_result expected_result = (expected);                \
    g_assertions += 1;                                                        \
    if (actual_result != expected_result) {                                   \
      (void)fprintf(stderr,                                                    \
                    "FAIL %s:%d: got %s, expected %s\n", __FILE__, __LINE__, \
                    hmg4v27_response_result_name(actual_result),               \
                    hmg4v27_response_result_name(expected_result));            \
      g_failures += 1;                                                        \
    }                                                                         \
  } while (0)
...
static uint32_t fixture_presence(uint32_t operation,
  ...
  if (operation == 1) {
    return status == 5 ? 0 : HMG4V27_RESPONSE_HAS_CAPABILITY;
  }
  if (operation == 2) {
    return status == 5
               ? 0
               : HMG4V27_RESPONSE_HAS_CURRENT_STATE |
                     (status == 6 ? HMG4V27_RESPONSE_HAS_CAPABILITY : 0u);
  }
  if (status == 0 || status == 2) {
    return HMG4V27_RESPONSE_HAS_TRANSACTION |
           HMG4V27_RESPONSE_HAS_CURRENT_STATE |
  ...
  if (status == 1) return HMG4V27_RESPONSE_HAS_CURRENT_STATE;
  if (status == 6) {
    return HMG4V27_RESPONSE_HAS_CURRENT_STATE |
...
static int make_fixture(uint32_t operation,
  ...
  fixture->fields.journal_leaf = (hmg4v27_span){
      (const uint8_t *)fixture->journal_leaf, (size_t)journal_length};
  fixture->fields.terminal_receipt_leaf = (hmg4v27_span){
...
static void round_trip(response_fixture *fixture) {
  ...
  const uint8_t *context =
      (fixture->fields.present_fields & HMG4V27_RESPONSE_HAS_TRANSACTION) != 0
          ? fixture->fields.sequence_zero_record_sha256
          : NULL;
  EXPECT_RESULT(hmg4v27_response_payload_length(&fixture->fields,
                                                &payload_length),
                HMG4V27_RESPONSE_OK);
  EXPECT_RESULT(hmg4v27_encode_response_frame(&fixture->fields, frame,
                                              sizeof(frame), &frame_length),
                HMG4V27_RESPONSE_OK);
  EXPECT_TRUE(frame_length == payload_length + HMG4V27_RESPONSE_HEADER_SIZE);
  EXPECT_TRUE(memcmp(frame, "HMG4R2\0\0", 8) == 0);
  EXPECT_TRUE(hmg4v27_read_u32_be(frame + 8) == 2);
  EXPECT_TRUE(hmg4v27_read_u32_be(frame + 12) == fixture->fields.status);
  EXPECT_TRUE(hmg4v27_read_u64_be(frame + 16) == payload_length);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    fixture->fields.operation,
                    (hmg4v27_span){frame, frame_length}, context, &view),
                HMG4V27_RESPONSE_OK);
...
static void refresh_payload_hash(uint8_t *frame, size_t frame_length) {
  uint8_t digest[32];
  const size_t payload_length = frame_length - HMG4V27_RESPONSE_HEADER_SIZE;
  EXPECT_TRUE(hmg4v27_sha256(
                  (hmg4v27_span){frame + HMG4V27_RESPONSE_HEADER_SIZE,
                                 payload_length},
                  digest) == HMG4V27_OK);
  ...
  for (operation = 1; operation <= 4; ++operation) {
    for (status = 0; status <= 6; ++status) {
      ...
      if (operation_status_is_legal(operation, status)) {
        ...
        EXPECT_RESULT(hmg4v27_encode_response_frame(
...
static void test_every_registered_diagnostic(void) {
  size_t ordinal;
  for (ordinal = 0; ordinal < HMG4V27_DIAGNOSTIC_COUNT; ++ordinal) {
    const hmg4v27_diagnostic *diagnostic = hmg4v27_diagnostic_at(ordinal);
    response_fixture fixture;
    uint32_t operation;
...
static void test_semantic_rejections(void) {
  ...
  fixture.fields.present_fields |= HMG4V27_RESPONSE_HAS_TERMINAL;
  EXPECT_RESULT(hmg4v27_response_payload_length(&fixture.fields,
                                                &payload_length),
  ...
  EXPECT_RESULT(hmg4v27_encode_response_frame(
                ...
                HMG4V27_RESPONSE_OK);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_OK);
  ...
  EXPECT_RESULT(hmg4v27_encode_response_frame(
                ...
                HMG4V27_RESPONSE_OK);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length},
  ...
  for (fixture.fields.status = 0; fixture.fields.status <= 6;
    ...
    EXPECT_TRUE(hmg4v27_response_exit_code(fixture.fields.status, &exit_code));
    EXPECT_TRUE(exit_code == expected[fixture.fields.status]);
  }
  EXPECT_TRUE(!hmg4v27_response_exit_code(7, &exit_code));
  EXPECT_TRUE(!hmg4v27_response_exit_code(0, NULL));
}
...
static void test_frame_rejections(void) {
  ...
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, 55}, NULL, &view),
  ...
  hmg4v27_write_u32_be(frame + 8, 3);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_VERSION_INVALID);
  memcpy(frame, original, frame_length);
  hmg4v27_write_u32_be(frame + 12, 7);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_STATUS_INVALID);
  memcpy(frame, original, frame_length);
  hmg4v27_write_u64_be(frame + 16, HMG4V27_RESPONSE_MAX_PAYLOAD + 1);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE);
  memcpy(frame, original, frame_length);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length - 1}, NULL, &view),
  ...
  memcpy(frame, original, frame_length);
  frame[HMG4V27_RESPONSE_HEADER_SIZE + 3] = 1;
  refresh_payload_hash(frame, frame_length);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_TLV_INVALID);
  memcpy(frame, original, frame_length);
  frame[HMG4V27_RESPONSE_HEADER_SIZE + 2] = HMG4V27_TLV_U64;
  refresh_payload_hash(frame, frame_length);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_TLV_INVALID);
  memcpy(frame, original, frame_length);
  frame[HMG4V27_RESPONSE_HEADER_SIZE + 1] = 2;
  refresh_payload_hash(frame, frame_length);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_TLV_INVALID);
  ...
  EXPECT_RESULT(hmg4v27_encode_response_frame(
                    &fixture.fields, frame, sizeof(frame), &frame_length),
                HMG4V27_RESPONSE_OK);
  hmg4v27_write_u16_be(frame + frame_length, 0x8004);
  frame[frame_length + 2] = HMG4V27_TLV_SAFE_CUSTODY_LEAF;
  frame[frame_length + 3] = 0;
  hmg4v27_write_u32_be(frame + frame_length + 4,
                       (uint32_t)fixture.fields.journal_leaf.length);
  memcpy(frame + frame_length + HMG4V27_TLV_HEADER_SIZE,
         fixture.fields.journal_leaf.bytes, fixture.fields.journal_leaf.length);
  frame_length += HMG4V27_TLV_HEADER_SIZE + fixture.fields.journal_leaf.length;
  hmg4v27_write_u64_be(frame + 16,
                       (uint64_t)(frame_length - HMG4V27_RESPONSE_HEADER_SIZE));
  refresh_payload_hash(frame, frame_length);
  EXPECT_RESULT(hmg4v27_validate_response_frame(
                    1, (hmg4v27_span){frame, frame_length}, NULL, &view),
                HMG4V27_RESPONSE_PRESENCE_INVALID);
