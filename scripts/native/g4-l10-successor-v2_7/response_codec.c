...
static int lower_hex_equals_digest(hmg4v27_span encoded,
...
static hmg4v27_response_result expected_presence(
    uint32_t operation,
    uint32_t status,
    uint32_t supplied_presence,
    uint32_t *expected) {
  uint32_t value = 0;
  if (expected == NULL) return HMG4V27_RESPONSE_NULL_ARGUMENT;
  if (operation < 1 || operation > 4) {
    return HMG4V27_RESPONSE_OPERATION_INVALID;
  }
  if (status > 6) return HMG4V27_RESPONSE_STATUS_INVALID;
  if (operation == 1) {
    if (status == 0 || status == 1 || status == 6) {
      value = HMG4V27_RESPONSE_HAS_CAPABILITY;
    } else if (status != 5) {
      return HMG4V27_RESPONSE_STATUS_OPERATION_INVALID;
    }
  } else if (operation == 2) {
    if (status == 0 || status == 1 || status == 3 || status == 4) {
      value = HMG4V27_RESPONSE_HAS_CURRENT_STATE;
    } else if (status == 6) {
      value = HMG4V27_RESPONSE_HAS_CURRENT_STATE |
              HMG4V27_RESPONSE_HAS_CAPABILITY;
    } else if (status != 5) {
      return HMG4V27_RESPONSE_STATUS_OPERATION_INVALID;
    }
  } else if (status == 0 || status == 2) {
    value = HMG4V27_RESPONSE_HAS_TRANSACTION |
            HMG4V27_RESPONSE_HAS_CURRENT_STATE |
            HMG4V27_RESPONSE_HAS_TERMINAL;
  } else if (status == 3) {
    value = HMG4V27_RESPONSE_HAS_TRANSACTION |
            HMG4V27_RESPONSE_HAS_CURRENT_STATE;
  } else if (status == 4) {
    value = HMG4V27_RESPONSE_HAS_TRANSACTION |
            HMG4V27_RESPONSE_HAS_CURRENT_STATE;
    if ((supplied_presence & HMG4V27_RESPONSE_HAS_TERMINAL) != 0) {
      value |= HMG4V27_RESPONSE_HAS_TERMINAL;
    }
  } else if (status == 1) {
    value = HMG4V27_RESPONSE_HAS_CURRENT_STATE;
  } else if (status == 6) {
  ...
  *expected = value;
  return HMG4V27_RESPONSE_OK;
}

static hmg4v27_response_result validate_custody_binding(
    hmg4v27_span leaf,
    hmg4v27_custody_variant expected_variant,
    const uint8_t transaction_id[32],
    const uint8_t digest[32],
    hmg4v27_response_result failure) {
  hmg4v27_custody_leaf parsed;
  if (!hmg4v27_parse_custody_leaf(leaf, &parsed) ||
      parsed.variant != expected_variant ||
      parsed.managed_index != HMG4V27_NO_INDEX ||
...
static hmg4v27_response_result validate_fields(
    const hmg4v27_response_fields *fields) {
  uint32_t expected = 0;
  hmg4v27_response_result result;
  if (fields == NULL || !span_pointer_is_valid(fields->journal_leaf) ||
      !span_pointer_is_valid(fields->terminal_receipt_leaf)) {
    return HMG4V27_RESPONSE_NULL_ARGUMENT;
  }
  result = expected_presence(fields->operation, fields->status,
                             fields->present_fields, &expected);
  if (result != HMG4V27_RESPONSE_OK) return result;
  if (fields->present_fields != expected) {
    return HMG4V27_RESPONSE_PRESENCE_INVALID;
  }
  if (hmg4v27_validate_diagnostic_status(fields->diagnostic_code,
                                         fields->status) != HMG4V27_OK) {
    return HMG4V27_RESPONSE_DIAGNOSTIC_INVALID;
  }
  if ((expected & HMG4V27_RESPONSE_HAS_CAPABILITY) != 0) {
    const uint32_t required = fields->status == 6 ? 2u : 1u;
    if (fields->capability_state != required) {
      return HMG4V27_RESPONSE_CAPABILITY_INVALID;
    }
  }
  if ((expected & HMG4V27_RESPONSE_HAS_CURRENT_STATE) != 0 &&
      fields->status != 1 && fields->status != 6 &&
      digest_is_zero(fields->observed_current_set_sha256)) {
    return HMG4V27_RESPONSE_CURRENT_STATE_INVALID;
  }
  if ((expected & HMG4V27_RESPONSE_HAS_TRANSACTION) != 0) {
  ...
  if ((expected & HMG4V27_RESPONSE_HAS_TERMINAL) != 0) {
    ...
    result = validate_custody_binding(
        fields->terminal_receipt_leaf, HMG4V27_CUSTODY_RECEIPT,
...
static int add_tlv_size(size_t *total, size_t value_length) {
  ...
  if (total == NULL ||
      !hmg4v27_checked_add_size(HMG4V27_TLV_HEADER_SIZE, value_length,
                               &field_length) ||
      !hmg4v27_checked_add_size(*total, field_length, total)) {
    return 0;
  }
  return 1;
}
...
hmg4v27_response_result hmg4v27_response_payload_length(
  ...
  size_t total = 0;
  hmg4v27_response_result result;
  if (payload_length == NULL) return HMG4V27_RESPONSE_NULL_ARGUMENT;
  result = validate_fields(fields);
  if (result != HMG4V27_RESPONSE_OK) return result;
  if (!add_tlv_size(&total, 4) || !add_tlv_size(&total, 32)) {
    return HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE;
  }
  if ((fields->present_fields & HMG4V27_RESPONSE_HAS_TRANSACTION) != 0 &&
      (!add_tlv_size(&total, 32) ||
       !add_tlv_size(&total, fields->journal_leaf.length) ||
       !add_tlv_size(&total, 32))) {
    return HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE;
  }
  if ((fields->present_fields & HMG4V27_RESPONSE_HAS_TERMINAL) != 0 &&
      (!add_tlv_size(&total, fields->terminal_receipt_leaf.length) ||
       !add_tlv_size(&total, 32) || !add_tlv_size(&total, 4))) {
    return HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE;
  }
  if ((fields->present_fields & HMG4V27_RESPONSE_HAS_CURRENT_STATE) != 0 &&
      !add_tlv_size(&total, 32)) {
    return HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE;
  }
  if ((fields->present_fields & HMG4V27_RESPONSE_HAS_CAPABILITY) != 0 &&
      !add_tlv_size(&total, 4)) {
    return HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE;
  }
  if ((uint64_t)total > HMG4V27_RESPONSE_MAX_PAYLOAD) {
    return HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE;
  }
...
static int write_bytes(response_writer *writer,
  ...
  if (writer == NULL || (length != 0 && bytes == NULL) ||
      !hmg4v27_range_within(writer->offset, length, writer->capacity)) {
    return 0;
  }
...
static int write_tlv(response_writer *writer,
                     ...
                     const void *value,
                     size_t value_length) {
  uint8_t header[HMG4V27_TLV_HEADER_SIZE];
...
static int write_tlv_u32(response_writer *writer,
                         ...
                         uint32_t value) {
  uint8_t encoded[4];
  hmg4v27_write_u32_be(encoded, value);
  return write_tlv(writer, tag, HMG4V27_TLV_U32, encoded, sizeof(encoded));
}

static int write_tlv_sha(response_writer *writer,
                         uint16_t tag,
                         const uint8_t digest[32]) {
  return write_tlv(writer, tag, HMG4V27_TLV_SHA256, digest, 32);
}
...
hmg4v27_response_result hmg4v27_encode_response_frame(
  ...
  result = hmg4v27_response_payload_length(fields, &payload_length);
  if (result != HMG4V27_RESPONSE_OK) return result;
  if (!hmg4v27_checked_add_size(HMG4V27_RESPONSE_HEADER_SIZE, payload_length,
                               &frame_length)) {
    return HMG4V27_RESPONSE_PAYLOAD_TOO_LARGE;
  }
  if (output_capacity < frame_length) {
    return HMG4V27_RESPONSE_OUTPUT_TOO_SMALL;
  }
  writer.bytes = output;
  writer.capacity = frame_length;
  writer.offset = HMG4V27_RESPONSE_HEADER_SIZE;
  if (!write_tlv_u32(&writer, 0x8001, fields->diagnostic_code) ||
      !write_tlv_sha(&writer, 0x8002, fields->request_payload_sha256)) {
    return HMG4V27_RESPONSE_OUTPUT_TOO_SMALL;
  }
  if ((fields->present_fields & HMG4V27_RESPONSE_HAS_TRANSACTION) != 0 &&
      (!write_tlv(&writer, 0x8003, HMG4V27_TLV_BYTES,
                  fields->transaction_id, 32) ||
       !write_tlv(&writer, 0x8004, HMG4V27_TLV_SAFE_CUSTODY_LEAF,
                  fields->journal_leaf.bytes, fields->journal_leaf.length) ||
       !write_tlv_sha(&writer, 0x8005, fields->journal_sha256))) {
    return HMG4V27_RESPONSE_OUTPUT_TOO_SMALL;
  }
  if ((fields->present_fields & HMG4V27_RESPONSE_HAS_TERMINAL) != 0 &&
      (!write_tlv(&writer, 0x8006, HMG4V27_TLV_SAFE_CUSTODY_LEAF,
  ...
  if (writer.offset != frame_length ||
      hmg4v27_sha256((hmg4v27_span){output + HMG4V27_RESPONSE_HEADER_SIZE,
                                   payload_length},
                     payload_sha256) != HMG4V27_OK) {
    return HMG4V27_RESPONSE_HASH_ENGINE_FAILURE;
  }
  memcpy(output, response_magic, sizeof(response_magic));
  hmg4v27_write_u32_be(output + 8, 2);
  hmg4v27_write_u32_be(output + 12, fields->status);
...
static hmg4v27_response_result read_response_payload(
    uint32_t operation,
    uint32_t status,
  ...
  hmg4v27_tlv_view tlv;
  hmg4v27_tlv_result tlv_result;
  int done = 0;
  uint32_t seen = 0;
  if (fields == NULL) return HMG4V27_RESPONSE_NULL_ARGUMENT;
  memset(fields, 0, sizeof(*fields));
  fields->operation = operation;
  fields->status = status;
  hmg4v27_tlv_cursor_init(&cursor, payload);
  while (!done) {
    tlv_result = hmg4v27_tlv_next_raw(&cursor, &tlv, &done);
    if (tlv_result != HMG4V27_TLV_OK) return HMG4V27_RESPONSE_TLV_INVALID;
    if (done) break;
    if (tlv.tag == 0x8001 && tlv.type == HMG4V27_TLV_U32 &&
        tlv.value.length == 4) {
      fields->diagnostic_code = hmg4v27_read_u32_be(tlv.value.bytes);
      seen |= UINT32_C(1) << 0;
    } else if (tlv.tag == 0x8002 && tlv.type == HMG4V27_TLV_SHA256 &&
               tlv.value.length == 32) {
      memcpy(fields->request_payload_sha256, tlv.value.bytes, 32);
      seen |= UINT32_C(1) << 1;
    } else if (tlv.tag == 0x8003 && tlv.type == HMG4V27_TLV_BYTES &&
               tlv.value.length == 32) {
      memcpy(fields->transaction_id, tlv.value.bytes, 32);
      fields->present_fields |= HMG4V27_RESPONSE_HAS_TRANSACTION;
      seen |= UINT32_C(1) << 2;
    } else if (tlv.tag == 0x8004 &&
               tlv.type == HMG4V27_TLV_SAFE_CUSTODY_LEAF) {
      fields->journal_leaf = tlv.value;
      seen |= UINT32_C(1) << 3;
    } else if (tlv.tag == 0x8005 && tlv.type == HMG4V27_TLV_SHA256 &&
               tlv.value.length == 32) {
      ...
      fields->present_fields |= HMG4V27_RESPONSE_HAS_TERMINAL;
      seen |= UINT32_C(1) << 5;
    } else if (tlv.tag == 0x8007 && tlv.type == HMG4V27_TLV_SHA256 &&
               tlv.value.length == 32) {
      memcpy(fields->terminal_receipt_sha256, tlv.value.bytes, 32);
      seen |= UINT32_C(1) << 6;
    } else if (tlv.tag == 0x8008 && tlv.type == HMG4V27_TLV_U32 &&
               tlv.value.length == 4) {
      fields->terminal_state = hmg4v27_read_u32_be(tlv.value.bytes);
      seen |= UINT32_C(1) << 7;
    } else if (tlv.tag == 0x8009 && tlv.type == HMG4V27_TLV_SHA256 &&
               tlv.value.length == 32) {
      memcpy(fields->observed_current_set_sha256, tlv.value.bytes, 32);
      fields->present_fields |= HMG4V27_RESPONSE_HAS_CURRENT_STATE;
      seen |= UINT32_C(1) << 8;
    } else if (tlv.tag == 0x800a && tlv.type == HMG4V27_TLV_U32 &&
               tlv.value.length == 4) {
      fields->capability_state = hmg4v27_read_u32_be(tlv.value.bytes);
      fields->present_fields |= HMG4V27_RESPONSE_HAS_CAPABILITY;
      seen |= UINT32_C(1) << 9;
    } else {
      return HMG4V27_RESPONSE_TLV_INVALID;
    }
  }
...
hmg4v27_response_result hmg4v27_validate_response_frame(
  ...
  if (complete_frame.length < HMG4V27_RESPONSE_HEADER_SIZE) {
    return HMG4V27_RESPONSE_FRAME_TOO_SHORT;
  }
  if (memcmp(complete_frame.bytes, response_magic, sizeof(response_magic)) != 0) {
    return HMG4V27_RESPONSE_MAGIC_INVALID;
  }
  if (hmg4v27_read_u32_be(complete_frame.bytes + 8) != 2) {
    return HMG4V27_RESPONSE_VERSION_INVALID;
  }
  if (hmg4v27_read_u32_be(complete_frame.bytes + 12) > 6) {
    return HMG4V27_RESPONSE_STATUS_INVALID;
  }
  payload_length_u64 = hmg4v27_read_u64_be(complete_frame.bytes + 16);
  if (payload_length_u64 > HMG4V27_RESPONSE_MAX_PAYLOAD ||
  ...
  if (!hmg4v27_checked_add_size(HMG4V27_RESPONSE_HEADER_SIZE, payload_length,
    ...
    return HMG4V27_RESPONSE_FRAME_LENGTH_INVALID;
  }
  if (hmg4v27_sha256(
          (hmg4v27_span){complete_frame.bytes + HMG4V27_RESPONSE_HEADER_SIZE,
                         payload_length},
          payload_sha256) != HMG4V27_OK) {
    return HMG4V27_RESPONSE_HASH_ENGINE_FAILURE;
  }
  if (memcmp(payload_sha256, complete_frame.bytes + 24, 32) != 0) {
    return HMG4V27_RESPONSE_PAYLOAD_HASH_INVALID;
  }
  memset(view, 0, sizeof(*view));
  view->complete_frame = complete_frame;
  view->payload = (hmg4v27_span){
      complete_frame.bytes + HMG4V27_RESPONSE_HEADER_SIZE, payload_length};
  result = read_response_payload(
      expected_operation, hmg4v27_read_u32_be(complete_frame.bytes + 12),
      view->payload, expected_sequence_zero_record_sha256, &view->fields);
  if (result != HMG4V27_RESPONSE_OK) return result;
  if (hmg4v27_sha256(complete_frame, view->response_frame_sha256) != HMG4V27_OK) {
    return HMG4V27_RESPONSE_HASH_ENGINE_FAILURE;
  }
  return HMG4V27_RESPONSE_OK;
}
