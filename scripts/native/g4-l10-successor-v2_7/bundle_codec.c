...
enum {
  HMG4V27_TYPE_U32 = HMG4V27_TLV_U32,
  HMG4V27_TYPE_U64 = HMG4V27_TLV_U64,
  HMG4V27_TYPE_SHA256 = HMG4V27_TLV_SHA256,
  HMG4V27_TYPE_POLICY_REL_PATH = HMG4V27_TLV_POLICY_REL_PATH
};
...
static int checked_align_up_size(
  ...
  return hmg4v27_checked_add_size(value, increment, result);
}

static hmg4v27_bundle_result sha256_streaming(
    hmg4v27_span input,
    uint8_t digest[32]) {
  ...
  return HMG4V27_BUNDLE_OK;
}

static int bytes_are_zero(hmg4v27_span bytes) {
  size_t index = 0;
...
static int spans_equal(hmg4v27_span left, hmg4v27_span right) {
...
hmg4v27_bundle_result hmg4v27_parse_bundle_header(
  ...
  hmg4v27_bundle_header candidate;
  uint32_t version = 0;
  uint32_t entry_count = 0;
  if (result == NULL || !span_pointer_is_valid(header_bytes)) {
    return HMG4V27_BUNDLE_NULL_ARGUMENT;
  }
  if (header_bytes.length < HMG4V27_BUNDLE_HEADER_SIZE) {
    return HMG4V27_BUNDLE_TRUNCATED_HEADER;
  }
  if (header_bytes.length != HMG4V27_BUNDLE_HEADER_SIZE) {
    return HMG4V27_BUNDLE_HEADER_LENGTH_MISMATCH;
  }
  if (memcmp(header_bytes.bytes, k_bundle_magic, sizeof(k_bundle_magic)) != 0) {
    return HMG4V27_BUNDLE_BAD_MAGIC;
  }
  version = hmg4v27_read_u32_be(header_bytes.bytes + 8);
  if (version != 2) return HMG4V27_BUNDLE_BAD_VERSION;
  entry_count = hmg4v27_read_u32_be(header_bytes.bytes + 12);
  if (entry_count != HMG4V27_BUNDLE_ENTRY_COUNT) {
    return HMG4V27_BUNDLE_BAD_ENTRY_COUNT;
  }
  candidate.table_length = hmg4v27_read_u64_be(header_bytes.bytes + 16);
  candidate.data_region_length = hmg4v27_read_u64_be(header_bytes.bytes + 24);
  if (candidate.table_length > HMG4V27_BUNDLE_MAX_TABLE_LENGTH) {
    return HMG4V27_BUNDLE_TABLE_TOO_LARGE;
  }
  if (candidate.data_region_length > HMG4V27_BUNDLE_MAX_DATA_LENGTH) {
    return HMG4V27_BUNDLE_DATA_TOO_LARGE;
  }
...
static hmg4v27_bundle_result next_expected(
    hmg4v27_tlv_cursor *cursor,
    uint16_t tag,
    uint8_t type,
    hmg4v27_tlv_view *view) {
  int done = 0;
  hmg4v27_tlv_result core_result =
      hmg4v27_tlv_next_raw(cursor, view, &done);
  if (core_result == HMG4V27_TLV_PATH_SYNTAX_INVALID) {
    return HMG4V27_BUNDLE_BAD_PATH;
  }
  if (core_result != HMG4V27_TLV_OK || done) {
    return HMG4V27_BUNDLE_BAD_ENTRY;
  }
...
static hmg4v27_bundle_result parse_bundle_entry(
    hmg4v27_span encoded_struct,
    uint32_t expected_index,
    hmg4v27_bundle_entry_view *result) {
  static const uint8_t k_types[11] = {
      HMG4V27_TYPE_U32,
      HMG4V27_TYPE_POLICY_REL_PATH,
      HMG4V27_TYPE_U64,
      HMG4V27_TYPE_U64,
      HMG4V27_TYPE_SHA256,
      HMG4V27_TYPE_U32,
      ...
      HMG4V27_TYPE_SHA256};
  hmg4v27_tlv_cursor cursor;
  hmg4v27_tlv_view fields[11];
  size_t field_index = 0;
  int done = 0;
  hmg4v27_tlv_view extra;
  hmg4v27_tlv_result core_result;
  if (result == NULL || !span_pointer_is_valid(encoded_struct)) {
    return HMG4V27_BUNDLE_NULL_ARGUMENT;
  }
  hmg4v27_tlv_cursor_init(&cursor, encoded_struct);
  for (field_index = 0; field_index < 11; field_index += 1) {
    hmg4v27_bundle_result status = next_expected(
        &cursor, (uint16_t)(0x0401u + field_index), k_types[field_index],
        &fields[field_index]);
    if (status != HMG4V27_BUNDLE_OK) return status;
  }
  core_result = hmg4v27_tlv_next_raw(&cursor, &extra, &done);
  if (core_result != HMG4V27_TLV_OK || !done) {
    return HMG4V27_BUNDLE_BAD_ENTRY;
  }
  result->index = hmg4v27_read_u32_be(fields[0].value.bytes);
  if (result->index != expected_index) return HMG4V27_BUNDLE_BAD_ENTRY;
  result->path = fields[1].value;
  if (!hmg4v27_policy_rel_path_is_lexically_safe(result->path)) {
    return HMG4V27_BUNDLE_BAD_PATH;
  }
  result->offset = hmg4v27_read_u64_be(fields[2].value.bytes);
  result->size = hmg4v27_read_u64_be(fields[3].value.bytes);
  memcpy(result->sha256, fields[4].value.bytes, sizeof(result->sha256));
  result->installed_mode = hmg4v27_read_u32_be(fields[5].value.bytes);
  result->installed_owner_uid = hmg4v27_read_u32_be(fields[6].value.bytes);
  result->installed_group_gid = hmg4v27_read_u32_be(fields[7].value.bytes);
  result->installed_flags = hmg4v27_read_u32_be(fields[8].value.bytes);
  ...
  if ((result->offset % HMG4V27_BUNDLE_ALIGNMENT) != 0 || result->size == 0 ||
      result->size > HMG4V27_BUNDLE_MAX_BLOB_SIZE ||
      result->offset > HMG4V27_BUNDLE_MAX_DATA_LENGTH ||
      result->size > HMG4V27_BUNDLE_MAX_DATA_LENGTH - result->offset) {
    return HMG4V27_BUNDLE_BAD_RANGE;
  }
  return HMG4V27_BUNDLE_OK;
}

static hmg4v27_bundle_result parse_bundle_table(
    hmg4v27_span table,
  ...
  if (table.length < 4) return HMG4V27_BUNDLE_BAD_TABLE;
  count = hmg4v27_read_u32_be(table.bytes);
  if (count != HMG4V27_BUNDLE_ENTRY_COUNT) {
    return HMG4V27_BUNDLE_BAD_ENTRY_COUNT;
  }
  offset = 4;
  for (index = 0; index < count; index += 1) {
    ...
    hmg4v27_bundle_result status;
    if (!range_within(offset, 4, table.length)) {
      return HMG4V27_BUNDLE_BAD_TABLE;
    }
    member_length = hmg4v27_read_u32_be(table.bytes + offset);
    if (!hmg4v27_checked_add_size(offset, 4, &offset) ||
    ...
    for (prior_index = 0; prior_index < index; prior_index += 1) {
      if (spans_equal(entries[prior_index].path, entries[index].path)) {
        return HMG4V27_BUNDLE_DUPLICATE_PATH;
      }
      if (spans_ascii_lower_equal(entries[prior_index].path,
                                  entries[index].path)) {
        return HMG4V27_BUNDLE_CASE_COLLISION;
      }
    }
    if (!hmg4v27_checked_add_size(offset, (size_t)member_length, &offset)) {
      return HMG4V27_BUNDLE_SIZE_OVERFLOW;
    }
  }
  return offset == table.length ? HMG4V27_BUNDLE_OK
                                : HMG4V27_BUNDLE_BAD_TABLE;
}

static hmg4v27_bundle_result validate_data_region(
    hmg4v27_span data_region,
    const hmg4v27_bundle_entry_view entries[HMG4V27_BUNDLE_ENTRY_COUNT],
  ...
  if (memcmp(actual_data_sha256, expected_data_sha256,
    ...
    return HMG4V27_BUNDLE_DATA_HASH_MISMATCH;
  }
  for (index = 0; index < HMG4V27_BUNDLE_ENTRY_COUNT; index += 1) {
    ...
    if (!range_within(entry_offset, entry_size, data_region.length) ||
        entry_offset < cursor ||
        !hmg4v27_checked_add_size(entry_offset, entry_size, &entry_end)) {
      return HMG4V27_BUNDLE_BAD_RANGE;
    }
    gap.bytes = data_region.bytes + cursor;
    gap.length = entry_offset - cursor;
    if (!bytes_are_zero(gap)) return HMG4V27_BUNDLE_NONZERO_DATA_GAP;
    ...
    if (memcmp(actual_blob_sha256, entries[index].sha256,
      ...
      return HMG4V27_BUNDLE_BLOB_HASH_MISMATCH;
    }
    cursor = entry_end;
  }
  if (cursor > data_region.length) return HMG4V27_BUNDLE_BAD_RANGE;
  {
    hmg4v27_span trailing_gap;
    trailing_gap.bytes = data_region.bytes + cursor;
    trailing_gap.length = data_region.length - cursor;
    if (!bytes_are_zero(trailing_gap)) {
      return HMG4V27_BUNDLE_NONZERO_DATA_GAP;
    }
  }
  return HMG4V27_BUNDLE_OK;
}
...
hmg4v27_bundle_result hmg4v27_validate_bundle(
  ...
  hmg4v27_bundle_view candidate;
  hmg4v27_span header_span;
  ...
  if (bundle_bytes.length < HMG4V27_BUNDLE_HEADER_SIZE) {
    return HMG4V27_BUNDLE_TRUNCATED_HEADER;
  }
  header_span.bytes = bundle_bytes.bytes;
  header_span.length = HMG4V27_BUNDLE_HEADER_SIZE;
  status = hmg4v27_parse_bundle_header(header_span, &header);
  if (status != HMG4V27_BUNDLE_OK) return status;
  ...
  if (!hmg4v27_checked_add_size(HMG4V27_BUNDLE_HEADER_SIZE, table_length,
                              &table_end) ||
      !checked_align_up_size(table_end, (size_t)HMG4V27_BUNDLE_ALIGNMENT,
                             &data_start) ||
      !hmg4v27_checked_add_size(data_start, data_length, &exact_file_length)) {
    return HMG4V27_BUNDLE_SIZE_OVERFLOW;
  }
  if (exact_file_length != bundle_bytes.length) {
    return HMG4V27_BUNDLE_FILE_LENGTH_MISMATCH;
  }
  candidate.header = header;
  candidate.table.bytes = bundle_bytes.bytes + HMG4V27_BUNDLE_HEADER_SIZE;
  ...
  if (!bytes_are_zero(candidate.predata_padding)) {
    return HMG4V27_BUNDLE_NONZERO_PREDATA_PADDING;
  }
  ...
  if (memcmp(actual_table_sha256, header.table_sha256,
    ...
    return HMG4V27_BUNDLE_TABLE_HASH_MISMATCH;
  }
