...
#ifdef HMG4V27_TRANSPORT_TESTING
...
_Static_assert(HMG4V27_INVALID_HEADER_TOKEN_SIZE == 21,
               "invalid-header token size drift");
_Static_assert(HMG4V27_INVALID_HEADER_TOKEN_SIZE <= PIPE_BUF,
...
static const hmg4v27_io_errno_symbols io_symbols = {
    EINTR, EAGAIN, EWOULDBLOCK};
static const hmg4v27_poll_symbols poll_symbols = {
    (uint16_t)POLLIN,
    (uint16_t)POLLOUT,
    (uint16_t)POLLERR,
    (uint16_t)POLLHUP,
...
#ifdef HMG4V27_TRANSPORT_TESTING
static hmg4v27_transport_test_backend test_backend;
...
static int call_monotonic_now_ns(uint64_t *now_ns) {
...
#endif
  {
    ...
    return hmg4v27_timespec_parts_to_ns(
               seconds, (int64_t)sample.tv_nsec, now_ns) ==
           HMG4V27_TRANSPORT_OK;
  }
}
...
static hmg4v27_pipe_result classify_request_transport_failure(
    int fixed_header_valid,
    hmg4v27_pipe_result post_header_result) {
  return fixed_header_valid ? post_header_result
                            : HMG4V27_PIPE_UNFRAMED_INVALID_HEADER;
}

static hmg4v27_pipe_result sample_before_call(
    int fd,
  ...
  if (!status_flags_are_exact(fd) ||
    ...
    return classify_request_transport_failure(
        fixed_header_valid, HMG4V27_PIPE_REQUEST_TRANSPORT_ERROR);
  }
  if (*pre_call_ns >= deadline_ns) {
    return classify_request_transport_failure(
        fixed_header_valid, HMG4V27_PIPE_REQUEST_DEADLINE_EXCEEDED);
  }
  return HMG4V27_PIPE_OK;
}

static hmg4v27_pipe_result sample_after_request_call(
    uint64_t deadline_ns,
    int fixed_header_valid,
...
typedef enum {
  HMG4V27_NEXT_READ_ORDINARY = 0,
  HMG4V27_NEXT_READ_HUP_ONLY = 1
} hmg4v27_next_read_mode;

static hmg4v27_pipe_result poll_request_until_read(
    uint64_t deadline_ns,
    int fixed_header_valid,
    hmg4v27_next_read_mode *next_mode) {
  for (;;) {
    ...
    hmg4v27_poll_decision decision = HMG4V27_POLL_TRANSPORT_ERROR;
    hmg4v27_pipe_result sample_result = sample_before_call(
        STDIN_FILENO, deadline_ns, fixed_header_valid, &pre_call_ns);
    if (sample_result != HMG4V27_PIPE_OK) return sample_result;
    if (hmg4v27_poll_timeout_ms(pre_call_ns, deadline_ns, &timeout_ms) !=
        HMG4V27_TRANSPORT_OK) {
      return classify_request_transport_failure(
          fixed_header_valid, HMG4V27_PIPE_REPRESENTATION_FAILURE);
    }
    returned_events = 0;
    if (!call_poll_one(
            STDIN_FILENO,
    ...
    if (sample_result != HMG4V27_PIPE_OK) return sample_result;
    if (hmg4v27_poll_decide(
            &poll_symbols,
            HMG4V27_POLL_ENDPOINT_REQUEST,
    ...
    if (decision == HMG4V27_POLL_DEADLINE) {
      return classify_request_transport_failure(
          fixed_header_valid, HMG4V27_PIPE_REQUEST_DEADLINE_EXCEEDED);
    }
    if (decision == HMG4V27_POLL_AGAIN) continue;
    if (decision == HMG4V27_POLL_TRANSPORT_ERROR) {
      return classify_request_transport_failure(
          fixed_header_valid, HMG4V27_PIPE_REQUEST_TRANSPORT_ERROR);
    }
    *next_mode = decision == HMG4V27_POLL_REQUEST_EOF_READ
                     ? HMG4V27_NEXT_READ_HUP_ONLY
                     : HMG4V27_NEXT_READ_ORDINARY;
    return HMG4V27_PIPE_OK;
  }
}

static hmg4v27_pipe_result read_request_bytes(
    uint8_t *destination,
    size_t requested_bytes,
    hmg4v27_read_phase phase,
    uint64_t deadline_ns,
    int fixed_header_valid,
    size_t *bytes_read,
    int *eof_confirmed) {
  hmg4v27_next_read_mode next_mode = HMG4V27_NEXT_READ_ORDINARY;
  if (destination == NULL || bytes_read == NULL || eof_confirmed == NULL ||
      requested_bytes == 0) {
    return HMG4V27_PIPE_NULL_ARGUMENT;
  }
  *bytes_read = 0;
  *eof_confirmed = 0;
  for (;;) {
    ...
    hmg4v27_io_decision decision = HMG4V27_IO_TRANSPORT_ERROR;
    hmg4v27_pipe_result sample_result = sample_before_call(
        STDIN_FILENO, deadline_ns, fixed_header_valid, &pre_call_ns);
    (void)pre_call_ns;
    if (sample_result != HMG4V27_PIPE_OK) return sample_result;
    ...
    sample_result = sample_after_request_call(
        deadline_ns, fixed_header_valid, &post_at_or_after_deadline);
    if (sample_result != HMG4V27_PIPE_OK) return sample_result;
    if (hmg4v27_request_read_decide(
            &io_symbols,
            phase,
            post_at_or_after_deadline,
            read_result,
            read_error,
            requested_bytes,
            &decision) != HMG4V27_TRANSPORT_OK) {
      return classify_request_transport_failure(
          fixed_header_valid, HMG4V27_PIPE_REPRESENTATION_FAILURE);
    }
    if (decision == HMG4V27_IO_DEADLINE) {
      return classify_request_transport_failure(
          fixed_header_valid, HMG4V27_PIPE_REQUEST_DEADLINE_EXCEEDED);
    }
    if (decision == HMG4V27_IO_RETRY_DIRECT) continue;
    if (decision == HMG4V27_IO_POLL) {
      if (next_mode == HMG4V27_NEXT_READ_HUP_ONLY) {
        return classify_request_transport_failure(
            fixed_header_valid, HMG4V27_PIPE_REQUEST_TRANSPORT_ERROR);
      }
    ...
    next_mode = HMG4V27_NEXT_READ_ORDINARY;
    if (decision == HMG4V27_IO_ADVANCE) {
      *bytes_read = (size_t)read_result;
      return HMG4V27_PIPE_OK;
    }
    if (decision == HMG4V27_IO_EOF_CONFIRMED) {
      *eof_confirmed = 1;
      return HMG4V27_PIPE_OK;
    }
    if (decision == HMG4V27_IO_INVALID_INCOMPLETE_HEADER) {
      return HMG4V27_PIPE_UNFRAMED_INVALID_HEADER;
    }
    if (decision == HMG4V27_IO_PAYLOAD_TRUNCATED) {
      return HMG4V27_PIPE_REQUEST_PAYLOAD_TRUNCATED;
    }
    if (decision == HMG4V27_IO_TRAILING_OR_SECOND_FRAME) {
      return HMG4V27_PIPE_REQUEST_TRAILING_OR_SECOND_FRAME;
    }
    if (decision == HMG4V27_IO_REPRESENTATION_ERROR) {
      return classify_request_transport_failure(
          fixed_header_valid, HMG4V27_PIPE_REPRESENTATION_FAILURE);
    }
    return classify_request_transport_failure(
...
static int attempt_invalid_header_token(void) {
  const uint8_t *token =
      (const uint8_t *)HMG4V27_INVALID_HEADER_TOKEN;
  uint64_t start_ns = 0;
  uint64_t deadline_ns = 0;
  if (!call_verify_sigpipe_ignore() ||
      !call_monotonic_now_ns(&start_ns) ||
      hmg4v27_deadline_from_start(
          start_ns, HMG4V27_DIAGNOSTIC_DEADLINE_NS, &deadline_ns) !=
          HMG4V27_TRANSPORT_OK) {
    return 0;
  }
  for (;;) {
    ...
    hmg4v27_io_decision decision = HMG4V27_IO_DIAGNOSTIC_ABANDONED;
    ...
    write_result = call_write_bytes(
        STDERR_FILENO, token, HMG4V27_INVALID_HEADER_TOKEN_SIZE, &write_error);
    if (!call_monotonic_now_ns(&post_call_ns)) return 0;
    if (hmg4v27_diagnostic_write_decide(
            &io_symbols,
    ...
    if (decision == HMG4V27_IO_RETRY_DIRECT) continue;
    return decision == HMG4V27_IO_WRITE_COMPLETE;
  }
}

static hmg4v27_pipe_result finish_unframed_result(
    hmg4v27_received_request *request,
    hmg4v27_pipe_result result) {
  if (result == HMG4V27_PIPE_UNFRAMED_INVALID_HEADER) {
    request->invalid_header_token_completed = attempt_invalid_header_token();
  }
  return result;
}

hmg4v27_pipe_result hmg4v27_install_and_verify_sigpipe_ignore(void) {
  return call_install_sigpipe_ignore() && call_verify_sigpipe_ignore()
             ? HMG4V27_PIPE_OK
             : HMG4V27_PIPE_SIGPIPE_BOOTSTRAP_FAILED;
}

void hmg4v27_release_received_request(
    hmg4v27_received_request *request) {
...
hmg4v27_pipe_result hmg4v27_receive_request_fd0(
    hmg4v27_received_request *request) {
  uint8_t header_bytes[HMG4V27_REQUEST_HEADER_SIZE];
  uint8_t eof_probe = 0;
  ...
  hmg4v27_transport_result transport_result = HMG4V27_TRANSPORT_OK;

  if (request == NULL) return HMG4V27_PIPE_NULL_ARGUMENT;
  memset(request, 0, sizeof(*request));
  memset(header_bytes, 0, sizeof(header_bytes));
  memset(payload_digest, 0, sizeof(payload_digest));
  if (!call_verify_sigpipe_ignore()) {
    return HMG4V27_PIPE_SIGPIPE_BOOTSTRAP_FAILED;
  }
  if (!call_monotonic_now_ns(&start_ns) ||
      hmg4v27_deadline_from_start(
          start_ns, HMG4V27_REQUEST_DEADLINE_NS, &deadline_ns) !=
          HMG4V27_TRANSPORT_OK) {
    return finish_unframed_result(
        request, HMG4V27_PIPE_UNFRAMED_INVALID_HEADER);
  }
  while (header_offset < sizeof(header_bytes)) {
    size_t read_count = 0;
    int eof_confirmed = 0;
    hmg4v27_pipe_result read_result = read_request_bytes(
        header_bytes + header_offset,
        sizeof(header_bytes) - header_offset,
        HMG4V27_READ_PHASE_HEADER,
        deadline_ns,
        0,
        &read_count,
        &eof_confirmed);
    (void)eof_confirmed;
    if (read_result != HMG4V27_PIPE_OK) {
      return finish_unframed_result(request, read_result);
    }
    header_offset += read_count;
  }
  transport_result = hmg4v27_parse_request_header(
      (hmg4v27_span){header_bytes, sizeof(header_bytes)}, &request->header);
  if (transport_result != HMG4V27_TRANSPORT_OK) {
    return finish_unframed_result(
        request, HMG4V27_PIPE_UNFRAMED_INVALID_HEADER);
  }
  request->fixed_header_valid = 1;
  if (request->header.payload_length > (uint64_t)SIZE_MAX ||
      !hmg4v27_checked_add_size(
          HMG4V27_REQUEST_HEADER_SIZE,
          (size_t)request->header.payload_length,
          &total_length)) {
    return HMG4V27_PIPE_REPRESENTATION_FAILURE;
  }
  request->owned_frame_bytes = (uint8_t *)call_allocate(total_length);
  if (request->owned_frame_bytes == NULL) {
    return HMG4V27_PIPE_ALLOCATION_FAILURE;
  }
  ...
  while (payload_offset < (size_t)request->header.payload_length) {
    ...
    int eof_confirmed = 0;
    hmg4v27_pipe_result read_result = read_request_bytes(
        request->owned_frame_bytes + HMG4V27_REQUEST_HEADER_SIZE +
            payload_offset,
        (size_t)request->header.payload_length - payload_offset,
        HMG4V27_READ_PHASE_PAYLOAD,
        deadline_ns,
        1,
        &read_count,
        &eof_confirmed);
  ...
  }
  {
    ...
    hmg4v27_pipe_result read_result = read_request_bytes(
        &eof_probe,
        1,
        HMG4V27_READ_PHASE_EOF_PROBE,
        deadline_ns,
        1,
        &read_count,
        &eof_confirmed);
    (void)read_count;
    if (read_result != HMG4V27_PIPE_OK || !eof_confirmed) {
      call_deallocate(request->owned_frame_bytes);
      request->owned_frame_bytes = NULL;
      request->owned_frame_length = 0;
      return read_result == HMG4V27_PIPE_OK
  ...
  if (hmg4v27_sha256(
          (hmg4v27_span){
              request->owned_frame_bytes + HMG4V27_REQUEST_HEADER_SIZE,
              (size_t)request->header.payload_length},
          payload_digest) != HMG4V27_OK) {
    const hmg4v27_request_header retained_header = request->header;
    hmg4v27_release_received_request(request);
    request->header = retained_header;
    request->fixed_header_valid = 1;
    return HMG4V27_PIPE_SHA256_ENGINE_FAILURE;
  }
  if (memcmp(payload_digest, request->header.payload_sha256, 32) != 0) {
    ...
    request->header = retained_header;
    request->fixed_header_valid = 1;
    return HMG4V27_PIPE_REQUEST_PAYLOAD_HASH_MISMATCH;
  }
  transport_result = hmg4v27_validate_buffered_request_frame(
      (hmg4v27_span){
  ...
  if (transport_result != HMG4V27_TRANSPORT_OK) {
    ...
    request->header = retained_header;
    request->fixed_header_valid = 1;
    if (transport_result == HMG4V27_TRANSPORT_PAYLOAD_HASH_INVALID) {
      return HMG4V27_PIPE_REQUEST_PAYLOAD_HASH_MISMATCH;
    }
    return transport_result == HMG4V27_TRANSPORT_REPRESENTATION_INVALID
...
static hmg4v27_pipe_result sample_before_response_call(
    uint64_t deadline_ns,
    uint64_t *pre_call_ns) {
  if (!call_verify_sigpipe_ignore() ||
      !status_flags_are_exact(STDOUT_FILENO) ||
      !call_monotonic_now_ns(pre_call_ns)) {
    return HMG4V27_PIPE_RESPONSE_TRANSPORT_FAILED;
  }
...
static hmg4v27_pipe_result poll_response_until_write(
    uint64_t deadline_ns) {
  for (;;) {
    uint64_t pre_call_ns = 0;
    uint64_t post_call_ns = 0;
    ...
    if (sample_before_response_call(deadline_ns, &pre_call_ns) !=
            HMG4V27_PIPE_OK ||
        hmg4v27_poll_timeout_ms(pre_call_ns, deadline_ns, &timeout_ms) !=
            HMG4V27_TRANSPORT_OK) {
      return HMG4V27_PIPE_RESPONSE_TRANSPORT_FAILED;
    }
    returned_events = 0;
    ...
    if (hmg4v27_poll_decide(
            &poll_symbols,
            HMG4V27_POLL_ENDPOINT_RESPONSE,
            post_call_ns >= deadline_ns,
            poll_return,
            poll_error,
            (uint16_t)returned_events,
    ...
    if (decision == HMG4V27_POLL_AGAIN) continue;
    return decision == HMG4V27_POLL_RESPONSE_RETRY_WRITE
               ? HMG4V27_PIPE_OK
               : HMG4V27_PIPE_RESPONSE_TRANSPORT_FAILED;
  }
}

hmg4v27_pipe_result hmg4v27_emit_response_fd1(
    uint32_t expected_operation,
    hmg4v27_span response_frame,
  ...
  if ((response_frame.length != 0 && response_frame.bytes == NULL) ||
      hmg4v27_validate_response_frame(
          expected_operation,
          response_frame,
          expected_sequence_zero_record_sha256,
          &view) != HMG4V27_RESPONSE_OK) {
    return HMG4V27_PIPE_RESPONSE_FRAME_INVALID;
  }
  if (!call_verify_sigpipe_ignore() ||
      !call_monotonic_now_ns(&start_ns) ||
      hmg4v27_deadline_from_start(
  ...
  while (offset < response_frame.length) {
    ...
    if (decision == HMG4V27_IO_RETRY_DIRECT) continue;
    if (decision == HMG4V27_IO_POLL) {
      if (poll_response_until_write(deadline_ns) != HMG4V27_PIPE_OK) {
        return HMG4V27_PIPE_RESPONSE_TRANSPORT_FAILED;
      }
      continue;
    }
    ...
    if (decision == HMG4V27_IO_WRITE_COMPLETE) {
      return HMG4V27_PIPE_OK;
    }
    return HMG4V27_PIPE_RESPONSE_TRANSPORT_FAILED;
  }
...
uint32_t hmg4v27_pipe_result_diagnostic_code(
    hmg4v27_pipe_result result) {
  switch (result) {
    case HMG4V27_PIPE_REQUEST_PAYLOAD_TRUNCATED:
      return UINT32_C(0x00010001);
    case HMG4V27_PIPE_REQUEST_PAYLOAD_HASH_MISMATCH:
      return UINT32_C(0x00010002);
    case HMG4V27_PIPE_REQUEST_TRANSPORT_ERROR:
      return UINT32_C(0x00010006);
    case HMG4V27_PIPE_REQUEST_TRAILING_OR_SECOND_FRAME:
      return UINT32_C(0x00010008);
    case HMG4V27_PIPE_REQUEST_DEADLINE_EXCEEDED:
      return UINT32_C(0x00010009);
    case HMG4V27_PIPE_REPRESENTATION_FAILURE:
      return UINT32_C(0x00020015);
    case HMG4V27_PIPE_ALLOCATION_FAILURE:
      return UINT32_C(0x00020016);
    case HMG4V27_PIPE_SHA256_ENGINE_FAILURE:
...
int hmg4v27_pipe_result_exit_code(
    hmg4v27_pipe_result result,
    int success_response_exit_code) {
  if (result == HMG4V27_PIPE_OK) return success_response_exit_code;
  if (result == HMG4V27_PIPE_SIGPIPE_BOOTSTRAP_FAILED ||
      result == HMG4V27_PIPE_UNFRAMED_INVALID_HEADER) {
    return HMG4V27_STARTUP_EXIT;
  }
  return result == HMG4V27_PIPE_RESPONSE_FRAME_INVALID ||
                 result == HMG4V27_PIPE_RESPONSE_TRANSPORT_FAILED
             ? HMG4V27_RESPONSE_FAILURE_EXIT
             : success_response_exit_code;
}

const char *hmg4v27_pipe_result_name(hmg4v27_pipe_result result) {
