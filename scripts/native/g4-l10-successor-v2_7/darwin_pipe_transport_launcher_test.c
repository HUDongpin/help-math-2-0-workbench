...
static size_t make_request(uint8_t frame[HMG4V27_REQUEST_HEADER_SIZE]) {
  uint8_t empty_digest[32];
  memset(frame, 0, HMG4V27_REQUEST_HEADER_SIZE);
  memcpy(frame, "HMG4V2\0\0", 8);
  hmg4v27_write_u32_be(frame + 8, 2);
  hmg4v27_write_u32_be(frame + 12, 1);
  hmg4v27_write_u64_be(frame + 16, 0);
  if (hmg4v27_sha256((hmg4v27_span){NULL, 0}, empty_digest) != HMG4V27_OK) {
    return 0;
  }
  memcpy(frame + 24, empty_digest, sizeof(empty_digest));
  return HMG4V27_REQUEST_HEADER_SIZE;
}
...
int main(int argc, char **argv) {
  ...
  pid_t child = 0;
  int status = 0;
  uint8_t request[HMG4V27_REQUEST_HEADER_SIZE];
  ...
  char *const child_env[] = {
      "HMG4V27_IGNORED_ENVIRONMENT=adversarial", NULL};
  ...
  if (mode == LAUNCH_VALID) {
    ...
    if (diagnostic_length != 0 ||
      ...
      return 1;
             ...
             diagnostic_length != HMG4V27_INVALID_HEADER_TOKEN_SIZE ||
             memcmp(diagnostic,
