...
static uint64_t state = UINT64_C(0x170bd54b031f1f6e);
...
int main(void) {
  uint8_t bytes[320]; size_t iteration, i; hmg4v21_authority_envelope envelope;
  hmg4v21_poll_symbols symbols={1,4,8,16,32,4}; hmg4v21_poll_decision decision;
  for (iteration=0; iteration<300000; ++iteration) {
    ...
    (void)hmg4v21_validate_authority_envelope((hmg4v21_span){bytes,length},&envelope);
    (void)hmg4v21_tlv_type_site_is_legal((uint8_t)next_u32(),(uint16_t)next_u32());
    (void)hmg4v21_poll_decide(&symbols,(next_u32()&1)?HMG4V21_POLL_ENDPOINT_REQUEST:HMG4V21_POLL_ENDPOINT_RESPONSE,
