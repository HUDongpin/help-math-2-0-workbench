...
static int authority_component_is_safe(const uint8_t *bytes, size_t length) {
  ...
  if (bytes == NULL || length == 0 ||
      length > HMG4V21_MAX_PATH_COMPONENT_BYTES ||
...
static int safe_relative_path(hmg4v21_span path, size_t maximum_length) {
  ...
  for (index = 0; index <= path.length; ++index) {
    if (index == path.length || path.bytes[index] == (uint8_t)'/') {
      if (component_count == HMG4V21_MAX_PATH_COMPONENTS ||
...
static int parse_custody_nonindexed(hmg4v21_span encoded,
  ...
  result->transaction_id_hex = (hmg4v21_span){encoded.bytes + 3, 64};
  result->managed_index = HMG4V21_NO_INDEX;
  result->digest_hex = (hmg4v21_span){encoded.bytes + digest_offset, 64};
  return 1;
}

static int parse_custody_indexed(hmg4v21_span encoded,
                                 const char *middle, size_t middle_length,
                                 hmg4v21_custody_variant variant,
                                 hmg4v21_custody_leaf *result) {
...
#undef EVIDENCE_RULE
  ...
  if (!span_pointer_is_valid(encoded) || complete_object_sha256 == NULL ||
      role < HMG4V21_EVIDENCE_PLAN || role > HMG4V21_EVIDENCE_REVIEW) {
    return 0;
  }
...
int hmg4v21_parse_fixture_root(hmg4v21_span encoded,
  ...
  if (encoded.length == sizeof(target_prefix) - 1 + 64 &&
    ...
    scope = HMG4V21_FIXTURE_SCOPE_TARGET;
    ...
    scope = HMG4V21_FIXTURE_SCOPE_SYSTEM_LOCK;
  ...
  result->scope = scope;
  result->nonce_hex = (hmg4v21_span){encoded.bytes + prefix_length, 64};
  return 1;
}

int hmg4v21_validate_fixture_root(hmg4v21_span encoded,
                                  hmg4v21_fixture_scope scope,
                                  const uint8_t nonce[32]) {
  hmg4v21_fixture_root parsed;
  return nonce != NULL && hmg4v21_parse_fixture_root(encoded, &parsed) &&
...
int hmg4v21_parse_fixture_attempt(hmg4v21_span encoded,
  ...
  if (encoded.length >= root_length &&
      hmg4v21_parse_fixture_root((hmg4v21_span){encoded.bytes, root_length},
                                 &root)) {
    offset = root_length;
  } else {
    root_length = sizeof("capability-fixtures/system-lock-") - 1 + 64;
    if (encoded.length < root_length ||
        !hmg4v21_parse_fixture_root(
            (hmg4v21_span){encoded.bytes, root_length}, &root)) {
      return 0;
    }
    offset = root_length;
  }
  if (bytes_at_equal(encoded.bytes, encoded.length, offset, "/op-", 4) &&
      ...
      attempt <= 2 &&
      ((root.scope == HMG4V21_FIXTURE_SCOPE_TARGET && operation >= 1 &&
        operation <= 18) ||
       (root.scope == HMG4V21_FIXTURE_SCOPE_SYSTEM_LOCK && operation >= 101 &&
        operation <= 105))) {
    result->kind = HMG4V21_FIXTURE_ATTEMPT_CAPABILITY;
  ...
  if (root.scope == HMG4V21_FIXTURE_SCOPE_TARGET &&
    ...
    result->kind = HMG4V21_FIXTURE_ATTEMPT_DENIAL;
    result->scope = root.scope;
    result->nonce_hex = root.nonce_hex;
    result->operation = operation;
...
int hmg4v21_validate_fixture_claim(hmg4v21_span encoded,
                                   const uint8_t nonce[32]) {
...
static int is_zero_sha256(hmg4v21_span value) {
  size_t index;
...
static hmg4v21_tlv_result validate_basic_value(uint8_t type,
                                               hmg4v21_span value) {
  hmg4v21_custody_leaf custody;
  hmg4v21_fixture_root fixture_root;
  hmg4v21_fixture_attempt fixture_attempt;
  switch (type) {
    case HMG4V21_TLV_U32:
      return value.length == 4 ? HMG4V21_TLV_OK
                               : HMG4V21_TLV_LENGTH_INVALID;
    case HMG4V21_TLV_U64:
      return value.length == 8 ? HMG4V21_TLV_OK
                               : HMG4V21_TLV_LENGTH_INVALID;
    case HMG4V21_TLV_BOOL:
      if (value.length != 1) return HMG4V21_TLV_LENGTH_INVALID;
      return value.bytes[0] <= 1 ? HMG4V21_TLV_OK
                                 : HMG4V21_TLV_BOOL_INVALID;
    case HMG4V21_TLV_SHA256:
      return value.length == 32 ? HMG4V21_TLV_OK
                                : HMG4V21_TLV_LENGTH_INVALID;
    case HMG4V21_TLV_BYTES:
    case HMG4V21_TLV_STRUCT:
      return HMG4V21_TLV_OK;
    case HMG4V21_TLV_POLICY_REL_PATH:
    case HMG4V21_TLV_APPROVED_EVIDENCE_REL_PATH:
      return hmg4v21_policy_rel_path_is_lexically_safe(value)
                 ? HMG4V21_TLV_OK
                 : HMG4V21_TLV_PATH_SYNTAX_INVALID;
    case HMG4V21_TLV_LIST:
      return value.length >= 4 ? HMG4V21_TLV_OK
                               : HMG4V21_TLV_LENGTH_INVALID;
    case HMG4V21_TLV_APPROVED_ABS_ROOT_PATH:
      return hmg4v21_approved_abs_root_is_lexically_safe(value)
                 ? HMG4V21_TLV_OK
                 : HMG4V21_TLV_PATH_SYNTAX_INVALID;
    case HMG4V21_TLV_SAFE_CUSTODY_LEAF:
      return hmg4v21_parse_custody_leaf(value, &custody)
                 ? HMG4V21_TLV_OK
                 : HMG4V21_TLV_PATH_SYNTAX_INVALID;
    case HMG4V21_TLV_OBSERVED_CUSTODY_LEAF:
      return hmg4v21_observed_custody_leaf_is_lexically_safe(value)
                 ? HMG4V21_TLV_OK
                 : HMG4V21_TLV_PATH_SYNTAX_INVALID;
    case HMG4V21_TLV_CAPABILITY_FIXTURE_ROOT_REL_PATH:
      return hmg4v21_parse_fixture_root(value, &fixture_root)
                 ? HMG4V21_TLV_OK
                 : HMG4V21_TLV_PATH_SYNTAX_INVALID;
    case HMG4V21_TLV_CAPABILITY_FIXTURE_REL_PATH:
      return hmg4v21_parse_fixture_attempt(value, &fixture_attempt)
                 ? HMG4V21_TLV_OK
                 : HMG4V21_TLV_PATH_SYNTAX_INVALID;
    case HMG4V21_TLV_CAPABILITY_FIXTURE_CLAIM_REL_PATH:
      if (value.length != sizeof("fixture-reservation-") - 1 + 64 +
        ...
        return HMG4V21_TLV_PATH_SYNTAX_INVALID;
      }
      return HMG4V21_TLV_OK;
    case HMG4V21_TLV_BUILD_REL_PATH:
      return hmg4v21_build_rel_path_is_lexically_safe(value)
                 ? HMG4V21_TLV_OK
                 : HMG4V21_TLV_PATH_SYNTAX_INVALID;
    default:
      return HMG4V21_TLV_TYPE_SITE_FORBIDDEN;
  }
}

void hmg4v21_tlv_cursor_init(hmg4v21_tlv_cursor *cursor,
                             hmg4v21_span encoded) {
...
hmg4v21_tlv_result hmg4v21_tlv_next_raw(hmg4v21_tlv_cursor *cursor,
  ...
  *done = 0;
  if (cursor->offset > cursor->encoded.length ||
      cursor->encoded.length - cursor->offset < HMG4V21_TLV_HEADER_SIZE) {
    return HMG4V21_TLV_TRUNCATED;
  }
  ...
  if (cursor->encoded.bytes[cursor->offset + 3] != 0) {
    return HMG4V21_TLV_RESERVED_NONZERO;
  }
  if (!hmg4v21_tlv_type_site_is_legal(type, tag)) {
    return HMG4V21_TLV_TYPE_SITE_FORBIDDEN;
  }
  if (cursor->has_previous_tag && tag <= cursor->previous_tag) {
    return HMG4V21_TLV_ORDER_INVALID;
  }
  length = hmg4v21_read_u32_be(cursor->encoded.bytes + cursor->offset + 4);
  if (!checked_add(cursor->offset, HMG4V21_TLV_HEADER_SIZE, &value_offset) ||
      !checked_add(value_offset, (size_t)length, &next_offset)) {
    return HMG4V21_TLV_LENGTH_INVALID;
  }
  if (next_offset > cursor->encoded.length) return HMG4V21_TLV_TRUNCATED;
  view->tag = tag;
  view->type = type;
  view->value = (hmg4v21_span){cursor->encoded.bytes + value_offset,
                              (size_t)length};
  validated = validate_basic_value(type, view->value);
  if (validated != HMG4V21_TLV_OK) return validated;
...
static int is_authority_path_type(uint8_t type) {
  return type == HMG4V21_TLV_POLICY_REL_PATH ||
         type == HMG4V21_TLV_APPROVED_ABS_ROOT_PATH ||
         ...
         type == HMG4V21_TLV_CAPABILITY_FIXTURE_REL_PATH ||
         type == HMG4V21_TLV_CAPABILITY_FIXTURE_CLAIM_REL_PATH ||
         type == HMG4V21_TLV_BUILD_REL_PATH;
}

static hmg4v21_tlv_result validate_rule_definition(
    const hmg4v21_tlv_field_rule *rule) {
  const uint32_t known_flags =
      HMG4V21_FIELD_OPTIONAL | HMG4V21_FIELD_LENGTH_RANGE |
      HMG4V21_FIELD_BYTES_BOUND_OVERRIDE | HMG4V21_FIELD_NUMERIC_RANGE |
      HMG4V21_FIELD_LIST_COUNT_RANGE | HMG4V21_FIELD_ALLOW_ZERO_SHA256;
  if (rule == NULL || rule->reserved != 0 ||
      (rule->flags & ~known_flags) != 0 ||
      !hmg4v21_tlv_type_site_is_legal(rule->type, rule->tag)) {
    return HMG4V21_TLV_SCHEMA_INVALID;
  }
  if ((rule->flags & HMG4V21_FIELD_LENGTH_RANGE) != 0 &&
      rule->minimum_length > rule->maximum_length) {
    return HMG4V21_TLV_SCHEMA_INVALID;
  }
  if ((rule->flags & HMG4V21_FIELD_BYTES_BOUND_OVERRIDE) != 0 &&
      (rule->type != HMG4V21_TLV_BYTES ||
       (rule->flags & HMG4V21_FIELD_LENGTH_RANGE) == 0)) {
    return HMG4V21_TLV_SCHEMA_INVALID;
  }
  if ((rule->flags & HMG4V21_FIELD_NUMERIC_RANGE) != 0 &&
      (rule->minimum_numeric_value > rule->maximum_numeric_value ||
       (rule->type != HMG4V21_TLV_U32 && rule->type != HMG4V21_TLV_U64))) {
    return HMG4V21_TLV_SCHEMA_INVALID;
  }
  if (rule->type == HMG4V21_TLV_STRUCT) {
    if (rule->nested_schema == NULL ||
        (rule->flags & HMG4V21_FIELD_LIST_COUNT_RANGE) != 0) {
      return HMG4V21_TLV_SCHEMA_INVALID;
    }
  } else if (rule->type == HMG4V21_TLV_LIST) {
    if (rule->nested_schema == NULL ||
        (rule->flags & HMG4V21_FIELD_LIST_COUNT_RANGE) == 0 ||
  ...
  if ((rule->flags & HMG4V21_FIELD_ALLOW_ZERO_SHA256) != 0 &&
      rule->type != HMG4V21_TLV_SHA256) {
    return HMG4V21_TLV_SCHEMA_INVALID;
  }
  return HMG4V21_TLV_OK;
}

static hmg4v21_tlv_result validate_schema_at_depth(
    hmg4v21_span encoded,
...
static hmg4v21_tlv_result validate_list(
    hmg4v21_span encoded,
    const hmg4v21_tlv_field_rule *rule,
    hmg4v21_path_authorizer path_authorizer,
    void *path_context,
  ...
  uint32_t ordinal;
  size_t offset = 4;
  if (encoded.length < 4) return HMG4V21_TLV_LIST_FRAMING_INVALID;
  count = hmg4v21_read_u32_be(encoded.bytes);
  if (count < rule->minimum_list_count || count > rule->maximum_list_count) {
    return HMG4V21_TLV_LIST_COUNT_INVALID;
  }
  for (ordinal = 0; ordinal < count; ++ordinal) {
    uint32_t member_length;
    size_t member_offset;
    size_t next_offset;
    hmg4v21_tlv_result nested;
    if (offset > encoded.length || encoded.length - offset < 4) {
      return HMG4V21_TLV_LIST_FRAMING_INVALID;
    }
    member_length = hmg4v21_read_u32_be(encoded.bytes + offset);
    ...
    nested = validate_schema_at_depth(
        (hmg4v21_span){encoded.bytes + member_offset, (size_t)member_length},
        rule->nested_schema, path_authorizer, path_context, depth + 1,
        maximum_depth);
    if (nested != HMG4V21_TLV_OK) return HMG4V21_TLV_NESTED_SCHEMA_INVALID;
    offset = next_offset;
  }
  return offset == encoded.length ? HMG4V21_TLV_OK
                                  : HMG4V21_TLV_LIST_FRAMING_INVALID;
}

static hmg4v21_tlv_result validate_field_value(
    const hmg4v21_tlv_field_rule *rule,
    hmg4v21_tlv_view view,
    hmg4v21_path_authorizer path_authorizer,
    void *path_context,
    ...
    uint32_t maximum_depth) {
  uint64_t numeric;
  hmg4v21_tlv_result contextual;
  if (view.type != rule->type) return HMG4V21_TLV_FIELD_TYPE_MISMATCH;
  if (view.type == HMG4V21_TLV_BYTES &&
      (rule->flags & HMG4V21_FIELD_BYTES_BOUND_OVERRIDE) == 0 &&
      view.value.length > HMG4V21_DEFAULT_BYTES_MAX) {
    return HMG4V21_TLV_LENGTH_INVALID;
  }
  if ((rule->flags & HMG4V21_FIELD_LENGTH_RANGE) != 0 &&
  ...
  if (view.type == HMG4V21_TLV_SHA256 &&
      (rule->flags & HMG4V21_FIELD_ALLOW_ZERO_SHA256) == 0 &&
      is_zero_sha256(view.value)) {
    return HMG4V21_TLV_SHA256_ZERO_FORBIDDEN;
  }
  if ((rule->flags & HMG4V21_FIELD_NUMERIC_RANGE) != 0) {
    numeric = view.type == HMG4V21_TLV_U32
                  ? (uint64_t)hmg4v21_read_u32_be(view.value.bytes)
                  : hmg4v21_read_u64_be(view.value.bytes);
    if (numeric < rule->minimum_numeric_value ||
        numeric > rule->maximum_numeric_value) {
      return HMG4V21_TLV_NUMERIC_RANGE_INVALID;
    }
  }
  if (is_authority_path_type(view.type)) {
    if (path_authorizer == NULL) return HMG4V21_TLV_PATH_CONTEXT_REQUIRED;
    contextual = path_authorizer(path_context, view.tag, view.type, view.value);
    if (contextual != HMG4V21_TLV_OK) {
      return contextual == HMG4V21_TLV_PATH_CONTEXT_REQUIRED
                 ? contextual
                 : HMG4V21_TLV_PATH_CONTEXT_REJECTED;
    }
  }
  if (view.type == HMG4V21_TLV_STRUCT) {
    contextual = validate_schema_at_depth(view.value, rule->nested_schema,
                                          path_authorizer, path_context,
                                          depth + 1, maximum_depth);
    return contextual == HMG4V21_TLV_OK
               ? HMG4V21_TLV_OK
               : HMG4V21_TLV_NESTED_SCHEMA_INVALID;
  }
  if (view.type == HMG4V21_TLV_LIST) {
...
static hmg4v21_tlv_result validate_schema_at_depth(
  ...
  if (depth >= maximum_depth) return HMG4V21_TLV_MAXIMUM_DEPTH_EXCEEDED;
  for (index = 0; index < schema->field_count; ++index) {
    hmg4v21_tlv_result definition = validate_rule_definition(&schema->fields[index]);
    if (definition != HMG4V21_TLV_OK ||
        (index != 0 && schema->fields[index - 1].tag >= schema->fields[index].tag)) {
      return HMG4V21_TLV_SCHEMA_INVALID;
    }
  }
  hmg4v21_tlv_cursor_init(&cursor, encoded);
  for (;;) {
    hmg4v21_tlv_result next = hmg4v21_tlv_next_raw(&cursor, &view, &done);
    if (next != HMG4V21_TLV_OK) return next;
    if (done) break;
    while (rule_index < schema->field_count &&
           schema->fields[rule_index].tag < view.tag) {
      if ((schema->fields[rule_index].flags & HMG4V21_FIELD_OPTIONAL) == 0) {
        return HMG4V21_TLV_REQUIRED_FIELD_MISSING;
      }
      ++rule_index;
    }
    if (rule_index == schema->field_count ||
        schema->fields[rule_index].tag != view.tag) {
      return HMG4V21_TLV_UNKNOWN_FIELD;
    }
...
hmg4v21_tlv_result hmg4v21_validate_tlv_schema(
    ...
    hmg4v21_path_authorizer path_authorizer,
    void *path_context) {
  if (schema == NULL) return HMG4V21_TLV_NULL_ARGUMENT;
