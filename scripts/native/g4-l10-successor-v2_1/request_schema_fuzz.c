...
static size_t append_u32(uint8_t *output, size_t offset, uint16_t tag,
  ...
  uint8_t bytes[4];
  put_u32(bytes, value);
  return append_tlv(output, offset, tag, HMG4V21_TLV_U32, bytes, sizeof(bytes));
}
...
static size_t append_u64(uint8_t *output, size_t offset, uint16_t tag,
  ...
  uint8_t bytes[8];
  put_u64(bytes, value);
  return append_tlv(output, offset, tag, HMG4V21_TLV_U64, bytes, sizeof(bytes));
}
...
static size_t append_hash(uint8_t *output, size_t offset, uint16_t tag,
  ...
  return append_tlv(output, offset, tag, HMG4V21_TLV_SHA256,
                    hash, sizeof(hash));
}

static size_t build_probe(uint8_t output[1024], hmg4v21_span root_path) {
  uint8_t root[512];
  uint8_t fsid[16];
  ...
  root_length = append_tlv(root, root_length, 0x0207, HMG4V21_TLV_BYTES,
                           fsid, sizeof(fsid));
  root_length = append_hash(root, root_length, 0x0208, 0x42);
  root_length = append_tlv(root, root_length, 0x0209,
                           HMG4V21_TLV_APPROVED_ABS_ROOT_PATH,
                           root_path.bytes, root_path.length);
  length = append_tlv(output, length, 0x0001, HMG4V21_TLV_SHA256,
                      hmg4v21_successor_sha256, 32);
  ...
  length = append_tlv(output, length, 0x0007, HMG4V21_TLV_STRUCT,
                      root, root_length);
...
int main(void) {
  ...
  size_t iteration;
  size_t index;
  authority.approved_root_path = (hmg4v21_span){
  ...
  for (index = 0; index < HMG4V21_MANAGED_ENTRY_COUNT; ++index) {
    ...
    paths[index] = (hmg4v21_span){(const uint8_t *)path_bytes[index],
                                  (size_t)written};
  }
  ...
  for (iteration = 0; iteration < 1024; ++iteration) {
    ...
    (void)hmg4v21_validate_request_payload(
        operation, (hmg4v21_span){candidate, length}, &authority, &summary);
    (void)hmg4v21_request_result_name(
        (hmg4v21_request_result)(next_u32() % 24));
  }
