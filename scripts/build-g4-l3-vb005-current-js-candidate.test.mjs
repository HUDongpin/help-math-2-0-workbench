...
import {
  generateG4L3Vb005CurrentJsCandidate,
  parseArguments,
  validateG4L3Vb005CurrentJsCandidate,
  validateVb005HotspotEvidence,
} from "./build-g4-l3-vb005-current-js-candidate.mjs";
...
test("VB005 candidate CLI keeps the write/tool surface explicit", () => {
...
test("VB005 checked-in report is fingerprinted and semantically fail-closed", async () => {
  const report = JSON.parse(await readFile(REPORT, "utf8"));
  assert.equal(validateG4L3Vb005CurrentJsCandidate(report), report);
  ...
  assert.throws(() => validateG4L3Vb005CurrentJsCandidate(promoted),
  ...
  assert.throws(() => validateG4L3Vb005CurrentJsCandidate(accepted),
    /acceptance fields must all remain false/);
});

test("VB005 report records all three exact hit-only glossary placements", async () => {
  const report = JSON.parse(await readFile(REPORT, "utf8"));
  assert.equal(validateVb005HotspotEvidence(report.hotspotEvidence),
  ...
  assert.deepEqual(report.hotspotEvidence.hotspots.map((hotspot) => ({
    ...
    {characterId: 11, keyAttribute: "Negative number", first: 1, last: 180, depth: 5, shape: 10, visible: false, enabled: false},
    {characterId: 12, keyAttribute: "Less than", first: 1, last: 180, depth: 7, shape: 10, visible: false, enabled: false},
    {characterId: 13, keyAttribute: "Zero", first: 1, last: 180, depth: 9, shape: 10, visible: false, enabled: false},
  ]);
});

test("VB005 browser evidence executes all 180 source-static frames without parity claims", async () => {
  const report = JSON.parse(await readFile(REPORT, "utf8"));
  const evidence = report.candidateRenderability;
  assert.equal(evidence.executedFrameCount, 180);
  assert.equal(evidence.pngEncodedFrameCount, 180);
...
test("VB005 manifest binds runtime, tools, source, scope, and disabled behavior", async () => {
  ...
  assert.equal(manifest.runtime.boundedScope.frameDomain, "sprite-53");
  assert.deepEqual(manifest.runtime.boundedScope.frames, {first: 1, lastInclusive: 180});
...
test("VB005 checked-in outputs reproduce from fresh extraction and 180-frame browser execution", async () => {
  const result = await generateG4L3Vb005CurrentJsCandidate({
    check: true,
    python: "/opt/anaconda3/bin/python3",
  });
  assert.equal(result.animationId, "course-g04-l03-vb-005");
  assert.equal(result.candidateRenderability.executedFrameCount, 180);
