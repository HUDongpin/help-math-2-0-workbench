...
const ANIMATION_ID = 'course-g04-l10-vb-011';
const CALIBRATION_ID = 'g4-l10-candidate-to-product-v4';
const FREEZE = Object.freeze({
  path: 'catalog/product-bridge-calibrations/g4-l10-candidate-to-product-v4.json',
  sha256: '65a8a4a4aa6aec8638979cf606d21a477b6a9fcce65e9c47090a1fa6e599ff93',
});
const SOURCE_SWF =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/VB/L10VB11.swf';
const SOURCE_SWF_SHA256 =
  'dd12bb87cffa76948020b1cfc34163f67fa4062bd286ea571bf4b08473709ba0';
const SWFMILL_XML =
  'migrations/course-g04-l10-vb-011/audit/machine/swfmill.xml.gz';
const SWFMILL_XML_SHA256 =
  '6dc5dd086a6d0a376d17681cc5cf384c85481866151b13b22c80abd2146bfc47';
const AUDIO_RUNTIME_AUDIT =
  'migrations/course-g04-l10-vb-011/audit/audio-runtime-evidence.json';
const AUDIO_RUNTIME_AUDIT_SHA256 =
  '0d84fe7f85aed5bd7d89709097ba9ed60a76ba94a3dc1747bdf34578e584a919';
const SPANISH_SOURCE =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/SA/L10VB11.mp3';
const PUBLIC_DIRECTORY =
  'apps/web/candidate-assets/flash-assets/2026-08-22-page-only-candidates-v1/courses/course-g04-l10-vb-011/audio';
...
export const EXPECTED_VB011_AUDIO = Object.freeze({
  embedded: Object.freeze({
    sourceTimelineId: 'sprite-31',
    characterId: 31,
    headFrame: 1,
    firstFrame: 2,
    lastFrame: 153,
...
function validateEmbedded(stream, audioAudit) {
  const expected = EXPECTED_VB011_AUDIO.embedded;
  invariant(stream?.head, 'sprite-31 has no SoundStreamHead');
  invariant(
    stream.head.compression === '2' &&
      stream.head.soundRate === '2' &&
      stream.head.soundStereo === '0',
    'sprite-31 is not the frozen mono 22050 Hz MP3 SoundStream',
  );
  invariant(
    ...
    'sprite-31 extracted MP3 bytes differ from the frozen result',
  );
  ...
  invariant(
    ...
    'audio-runtime evidence no longer binds sprite-31 frames 2..153',
  );
  invariant(
    stream.mp3[0] === 0xff && (stream.mp3[1] & 0xe0) === 0xe0,
    'sprite-31 extracted bytes have no MP3 sync word',
  );
}

function outputRecords(scriptSha256, embeddedProbe, spanishProbe) {
  const embedded = EXPECTED_VB011_AUDIO.embedded;
  const spanish = EXPECTED_VB011_AUDIO.spanish;
  const assets = [
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-vb-011/audio/embedded-stream-0001.mp3',
    ...
    },
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-vb-011/audio/spanish-host-narration.mp3',
      bytes: spanish.bytes,
  ...
  const common = {
    ...
    freeze: FREEZE,
    generatedBy: {
      path: 'scripts/materialize-g4-l10-vb011-private-audio.mjs',
      sha256: scriptSha256,
  ...
  const manifest = jsonBytes({
    ...common,
    artifactType: 'g4-l10-vb011-private-product-audio-manifest',
    status: 'byte-exact-private-current-js-audio-candidate',
  });
  const receipt = jsonBytes({
    ...common,
    artifactType: 'g4-l10-vb011-private-product-audio-receipt',
...
export async function materializeG4L10Vb011PrivateAudio(options = {}) {
  ...
  const spanishBytes = await readBound(
    root,
    SPANISH_SOURCE,
    EXPECTED_VB011_AUDIO.spanish.sha256,
    EXPECTED_VB011_AUDIO.spanish.bytes,
  );
  const streams = await extractMp3SoundStreams(
    path.join(root, SWFMILL_XML),
    [EXPECTED_VB011_AUDIO.embedded.characterId],
  );
  const stream = streams.get(EXPECTED_VB011_AUDIO.embedded.characterId);
  validateEmbedded(stream, JSON.parse(audioAuditBytes));
  const embeddedProbe = probeMp3(
    stream.mp3,
    EXPECTED_VB011_AUDIO.embedded,
    'VB011 embedded SoundStream',
  );
  const spanishProbe = probeMp3(
    spanishBytes,
    EXPECTED_VB011_AUDIO.spanish,
    'VB011 Spanish host track',
  );
  ...
  if (options.check) {
    invariant(
      stale.length === 0,
      `VB011 private audio outputs are stale:\n${stale.join('\n')}`,
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const result = await materializeG4L10Vb011PrivateAudio(
