...
const ANIMATION_ID = 'course-g04-l10-ts-006';
const CALIBRATION_ID = 'g4-l10-candidate-to-product-v2';
const FREEZE = Object.freeze({
  path: 'catalog/product-bridge-calibrations/g4-l10-candidate-to-product-v2.json',
  sha256: 'bf557eca119de797158e34d9e4a458bd0868eb9146b1710232ee7702e699485a',
});
const SOURCE_SWF =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/TS/L10TS06.swf';
const SOURCE_SWF_SHA256 =
  'b0ad832f7d755e2f94dddc53e3267414c5d8430ed0e8c28d498cc5ec3c05160e';
const SWFMILL_XML =
  'migrations/course-g04-l10-ts-006/audit/machine/swfmill.xml.gz';
const SWFMILL_XML_SHA256 =
  '59d1e03806fed5c2d5f7ae34a0ae7a483a060d0bb845a668917aa2ee3f9bb3be';
const AUDIO_RUNTIME_AUDIT =
  'migrations/course-g04-l10-ts-006/audit/audio-runtime-evidence.json';
const AUDIO_RUNTIME_AUDIT_SHA256 =
  'b96e4ff6112fb37bc3022fa40a15fd0dc3b719eb250284ff0e19f254acc9e37c';
const SPANISH_SOURCE =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/SA/L10TS06.mp3';
const PUBLIC_DIRECTORY =
  'apps/web/candidate-assets/flash-assets/2026-08-22-page-only-candidates-v1/courses/course-g04-l10-ts-006/audio';
...
export const EXPECTED_TS006_AUDIO = Object.freeze({
  embedded: Object.freeze({
    sourceTimelineId: 'sprite-13',
    characterId: 13,
    firstFrame: 1,
    lastFrame: 245,
    endFrame: 246,
    blockCount: 245,
    totalDecodedSamples: 449856,
    sampleRateHz: 22050,
    channels: 1,
    bytes: 101530,
    sha256:
...
function probeMp3(bytes, expected, label) {
  ...
  invariant(
    ...
    `${label} media shape differs from the frozen MP3 disposition`,
  );
...
function validateEmbedded(stream, audioAudit) {
  const expected = EXPECTED_TS006_AUDIO.embedded;
  invariant(stream?.head, 'sprite-13 has no SoundStreamHead');
  invariant(
    stream.head.compression === '2' &&
      stream.head.soundRate === '2' &&
      stream.head.soundStereo === '0',
    'sprite-13 is not the frozen mono 22050 Hz MP3 SoundStream',
  );
  invariant(
    ...
    'sprite-13 extracted MP3 bytes differ from the frozen result',
  );
  ...
  invariant(
    ...
    'audio-runtime evidence no longer binds sprite-13 frames 1..245',
  );
  invariant(
    stream.mp3[0] === 0xff && (stream.mp3[1] & 0xe0) === 0xe0,
    'sprite-13 extracted bytes have no MP3 sync word',
  );
}

function outputRecords(scriptSha256, embeddedProbe, spanishProbe) {
  const embedded = EXPECTED_TS006_AUDIO.embedded;
  const spanish = EXPECTED_TS006_AUDIO.spanish;
  const assets = [
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-ts-006/audio/embedded-stream-0001.mp3',
      ...
      routeAuthority:
        'Legacy host route plus embedded/default-page convention only; no listening acceptance.',
    },
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-ts-006/audio/spanish-host-narration.mp3',
      bytes: spanish.bytes,
      sha256: spanish.sha256,
  ...
  const common = {
    ...
    freeze: FREEZE,
    generatedBy: {
      path: 'scripts/materialize-g4-l10-ts006-private-audio.mjs',
      sha256: scriptSha256,
  ...
  const manifest = jsonBytes({
    ...common,
    artifactType: 'g4-l10-ts006-private-product-audio-manifest',
    status: 'byte-exact-private-current-js-audio-candidate',
  });
  const receipt = jsonBytes({
    ...common,
    artifactType: 'g4-l10-ts006-private-product-audio-receipt',
...
export async function materializeG4L10Ts006PrivateAudio(options = {}) {
  ...
  const spanishBytes = await readBound(
    root,
    SPANISH_SOURCE,
    EXPECTED_TS006_AUDIO.spanish.sha256,
    EXPECTED_TS006_AUDIO.spanish.bytes,
  );
  const streams = await extractMp3SoundStreams(
    path.join(root, SWFMILL_XML),
    [EXPECTED_TS006_AUDIO.embedded.characterId],
  );
  const stream = streams.get(EXPECTED_TS006_AUDIO.embedded.characterId);
  validateEmbedded(stream, JSON.parse(audioAuditBytes));
  const embeddedProbe = probeMp3(
    stream.mp3,
    EXPECTED_TS006_AUDIO.embedded,
    'TS006 embedded SoundStream',
  );
  const spanishProbe = probeMp3(
    spanishBytes,
    EXPECTED_TS006_AUDIO.spanish,
    'TS006 Spanish host track',
  );
  ...
  for (const output of outputs) {
    ...
    else if (!(await readFile(absolute)).equals(output.bytes)) {
  ...
  if (options.check) {
    invariant(
      stale.length === 0,
      `TS006 private audio outputs are stale:\n${stale.join('\n')}`,
    );
  } else {
    for (const output of outputs) {
      await publishMissingExact(root, output.path, output.bytes);
    }
  }
  return Object.freeze({
    animationId: ANIMATION_ID,
    action: options.check ? 'verified' : 'materialized',
    outputCount: outputs.length,
    totalAudioBytes: stream.mp3.length + spanishBytes.length,
    fullEofDecodeCount: 2,
    outputs: outputs.map(({path: outputPath}) => portable(outputPath)),
    strictAcceptanceEffect: 'none',
  });
}
