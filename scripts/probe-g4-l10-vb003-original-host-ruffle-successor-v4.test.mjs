...
import {
  ...
  SUCCESSOR_V4_OUTPUT_RELATIVE,
  analyzeDomInputEvents,
  buildFailClosedAuthority,
  buildInputAttemptPlans,
  traceReferencesExpectedPath,
} from "./probe-g4-l10-vb003-original-host-ruffle-successor-v4.mjs";

const ORIGIN = "http://127.0.0.1:43123";
const RESULT_PATH = `${SUCCESSOR_V4_OUTPUT_RELATIVE}/diagnostic.json`;
const V2_PATH =
...
const SCRIPT_PATH =
  "scripts/probe-g4-l10-vb003-original-host-ruffle-successor-v4.mjs";

function sha256(value) {
  return createHash("sha256").update(value).digest("hex");
}

test("v4 orders two fresh-context attempts without mixing causal axes", () => {
...
test("checked-in v4 preserves its pre-Ruffle observer-ordering failure without rewriting v3", async () => {
  const [resultBytes, v2Bytes, v3Bytes, scriptBytes] = await Promise.all([
  ...
  for (const attempt of result.observation.attempts) {
    ...
    assert.equal(transition.input.domSequence.observerTarget, "window");
  ...
  const [resultMetadata, outputMetadata] = await Promise.all([
    stat(RESULT_PATH),
    stat(SUCCESSOR_V4_OUTPUT_RELATIVE),
  ]);
