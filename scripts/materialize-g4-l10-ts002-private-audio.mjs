...
const ANIMATION_ID = 'course-g04-l10-ts-002';
const CALIBRATION_ID = 'g4-l10-candidate-to-product-v8';
const FREEZE = Object.freeze({
  path: 'catalog/product-bridge-calibrations/g4-l10-candidate-to-product-v8.json',
  sha256: '02109ac0f91e18a55674a985c0fdd54158a1b2210b9810434f7627d56f74302e',
});
const SOURCE_SWF =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/TS/L10TS02.swf';
const SOURCE_SWF_SHA256 =
  '852ef6a6f24e0666fe4d14d3bce63d0170f01c255c5f61e147db559895db032f';
const SWFMILL_XML =
  'migrations/course-g04-l10-ts-002/audit/machine/swfmill.xml.gz';
const SWFMILL_XML_SHA256 =
  '91c28507fe20c0a997733c86007897673bf498aa9d3ad8d796e993d82ce46483';
const AUDIO_RUNTIME_AUDIT =
  'migrations/course-g04-l10-ts-002/audit/audio-runtime-evidence.json';
const AUDIO_RUNTIME_AUDIT_SHA256 =
  'a4613f980b19e7bd913c6b41f20af7de5abdfd5a2255a5285c059d6c1b2b6931';
const SPANISH_SOURCE =
  'source-assets/flash/HELP MATH_ORIGINAL FILES/HELP_COURSES/ELMGR4/L10/SA/L10TS02.mp3';
const PUBLIC_DIRECTORY =
  'apps/web/candidate-assets/flash-assets/2026-08-22-page-only-candidates-v1/courses/course-g04-l10-ts-002/audio';
...
export const EXPECTED_TS002_AUDIO = Object.freeze({
  embedded: Object.freeze({
    sourceTimelineId: 'sprite-29',
    characterId: 29,
    headFrame: 1,
    firstFrame: 1,
    lastFrame: 324,
...
function validateEmbedded(stream, audioAudit) {
  const expected = EXPECTED_TS002_AUDIO.embedded;
  invariant(stream?.head, 'sprite-29 has no SoundStreamHead');
  invariant(
    stream.head.compression === '2' &&
      stream.head.soundRate === '2' &&
      stream.head.soundStereo === '0',
    'sprite-29 is not the frozen mono 22050 Hz MP3 SoundStream',
  );
  invariant(
    ...
    'sprite-29 extracted MP3 bytes differ from the V8 freeze',
  );
  ...
  invariant(
    ...
    'audio-runtime evidence no longer binds sprite-29 frames 1..324',
  );
  invariant(
    stream.mp3[0] === 0xff && (stream.mp3[1] & 0xe0) === 0xe0,
    'sprite-29 extracted bytes have no MP3 sync word',
  );
}

function outputRecords(scriptSha256, embeddedProbe, spanishProbe) {
  const embedded = EXPECTED_TS002_AUDIO.embedded;
  const spanish = EXPECTED_TS002_AUDIO.spanish;
  const assets = [
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-ts-002/audio/embedded-stream-0001.mp3',
    ...
    },
    {
      ...
      publicPath:
        '/flash-assets/courses/course-g04-l10-ts-002/audio/spanish-host-narration.mp3',
      bytes: spanish.bytes,
  ...
  const common = {
    ...
    freeze: FREEZE,
    generatedBy: {
      path: 'scripts/materialize-g4-l10-ts002-private-audio.mjs',
      sha256: scriptSha256,
  ...
  const manifest = jsonBytes({
    ...common,
    artifactType: 'g4-l10-ts002-private-product-audio-manifest',
    status: 'byte-exact-private-current-js-audio-candidate',
  });
  const receipt = jsonBytes({
    ...common,
    artifactType: 'g4-l10-ts002-private-product-audio-receipt',
...
export async function materializeG4L10Ts002PrivateAudio(options = {}) {
  ...
  const spanishBytes = await readBound(
    root,
    SPANISH_SOURCE,
    EXPECTED_TS002_AUDIO.spanish.sha256,
    EXPECTED_TS002_AUDIO.spanish.bytes,
  );
  const streams = await extractMp3SoundStreams(
    path.join(root, SWFMILL_XML),
    [EXPECTED_TS002_AUDIO.embedded.characterId],
  );
  const stream = streams.get(EXPECTED_TS002_AUDIO.embedded.characterId);
  validateEmbedded(stream, JSON.parse(audioAuditBytes));
  const embeddedProbe = probeMp3(
    stream.mp3,
    EXPECTED_TS002_AUDIO.embedded,
    'TS002 embedded SoundStream',
  );
  const spanishProbe = probeMp3(
    spanishBytes,
    EXPECTED_TS002_AUDIO.spanish,
    'TS002 Spanish host track',
  );
  ...
  if (options.check) {
    invariant(
      stale.length === 0,
      `TS002 private audio outputs are stale:\n${stale.join('\n')}`,
    );
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const result = await materializeG4L10Ts002PrivateAudio(
