...
const PREVIOUS_IMPLEMENTATION_MANIFEST =
  "output/playwright/g4-l3-ts006-exact-pid-comparison-v11/en-diagnostic/capture-manifest.json";
const DIAGNOSTIC_ENTRY_STATE =
  "output/playwright/g4-l3-ts006-exact-pid-comparison-v12/diagnostic-entry-state.json";
const IMPLEMENTATION_DIRECTORY =
  "output/playwright/g4-l3-ts006-exact-pid-comparison-v12/en-diagnostic";
const IMPLEMENTATION_MANIFEST =
  `${IMPLEMENTATION_DIRECTORY}/capture-manifest.json`;
const DIFF_DIRECTORY =
  "output/playwright/g4-l3-ts006-exact-pid-comparison-v12/diffs";
const REPORT_JSON =
  "reports/g4-l3-ts006-exact-pid-implementation-comparison-v12.json";
const REPORT_MARKDOWN =
  "reports/g4-l3-ts006-exact-pid-implementation-comparison-v12.md";
...
const EXPECTED_PREVIOUS_REPORT_SHA256 =
  "9cd215bbef4046d4ed73cd94da95686c5d5174070278f379d05f0126ac2ea6b2";
...
const EXPECTED_ENTRY_STATE_SHA256 =
  "5d06cf556684c9cb238031416df1e17fe389df2930946e492b08bc9811f1da7b";
...
export const EXACT_PID_V12_PROGRESS_ANCHORS = Object.freeze([
...
export function validateExactPidV12Inputs({
  analysis,
  source,
  previousReport,
  entryState,
  ...
  invariant(
    ...
    "v11 comparison report identity or boundary drifted",
  );
  invariant(
    entryState?.stateId
      === "course-g04-l03-ts-006-exact-pid-v12-color-convergence-context"
    ...
    "v12 diagnostic entry-state identity, anchors, or authority drifted",
  );
  const increment = entryState.diagnosticCandidateIncrement;
  invariant(
    increment?.status
      === "acceptance-neutral-color-convergence-not-authoritative"
      ...
      && increment.transferFunction === "C_out=C_in^(1/1.2)"
      && increment.browserImplementation?.primitive
        === "svg-feComponentTransfer"
      && increment.browserImplementation.colorInterpolationFilters === "sRGB"
      && JSON.stringify(increment.browserImplementation.channels)
        === JSON.stringify(["red", "green", "blue"])
      && increment.browserImplementation.type === "gamma"
      && increment.browserImplementation.amplitude === 1
      && increment.browserImplementation.exponent === 5 / 6
      && increment.browserImplementation.offset === 0
      && increment.sourceStaticPathAffected === false
      && increment.wholeFrameOrRegionAssetUsed === false
      && increment.originalRuntimeColorPipelineEstablished === false
      && increment.visualParityEstablished === false
      && increment.strictAcceptanceEffect === "none",
    "v12 bounded color-convergence increment drifted",
  );
  invariant(
    implementation?.schemaVersion === IMPLEMENTATION_CAPTURE_SCHEMA_VERSION
      ...
      && implementation.requirementId === "diagnostic:ts006:exact-pid-v12:en"
      && implementation.traceId
        === "diagnostic:exact-pid-v12:tentative-piecewise-color:en:seed-0"
    ...
    "v12 implementation capture identity drifted",
  );
  ...
  invariant(
    ...
    "v12 implementation frames or rendered capture identities are incomplete",
  );
  invariant(
    ...
    "v12 implementation capture contains browser or network errors",
  );
  return assertFixedRegionContract(EXACT_PID_V10_REGIONS);
}

export function validateV12CapturePathColorDiagnostics({
  windowIdCapture,
...
export function validateV12ImplementationClosureDelta({
  ...
  invariant(
    added.length === 0,
    "v12 closure must not add implementation artifacts",
  );
  invariant(removed.length === 0, "v12 closure must not remove implementation assets");
  invariant(
    ...
    "v12 closure may change only the frozen TS006 renderer/timeline and the separately rebound current-JS manifest",
  );
  invariant(
    current.get(CURRENT_JS_ASSET_MANIFEST)?.sha256
      === EXPECTED_CURRENT_JS_ASSET_MANIFEST_SHA256,
    "v12 current-JS manifest rebind hash drifted",
  );
  invariant(
    [...added, ...changed].every(
      (item) =>
    ...
    "v12 closure introduced a prohibited full-frame or region-strip artifact",
  );
  return {
    added,
    removed,
...
export function assertExactV12DiffArtifactSet(actualNames, diffArtifacts) {
  invariant(
...
function markdown(report) {
  ...
  const summary = Object.entries(report.summary.regions).map(
    ([id, value]) =>
      ...
      + `v11-to-v12 mean reduction `
      + `${report.v11Delta.regions[id].absoluteReduction.toFixed(6)} `
      + `(${report.v11Delta.regions[id].relativeReductionPercent.toFixed(2)}%)`,
  ).join("\n");
  return `# G4 L3 TS006 exact-PID implementation comparison v12\n\n`
...
export async function buildG4L3Ts006ExactPidImplementationComparisonV12() {
  const [
  ...
  invariant(
    ...
    "v11 comparison report hash drifted",
  );
  invariant(
  ...
  invariant(
    ...
    "v12 diagnostic entry-state hash drifted",
  );
  invariant(
  ...
  invariant(
    ...
    "v12 capture does not bind the diagnostic entry-state digest",
  );
  invariant(
  ...
  const closureDelta = validateV12ImplementationClosureDelta({
    previousImplementation,
    implementation,
  });
  ...
  invariant(
    ...
    `v12 implementation closure is stale: ${closureFreshnessErrors.join("; ")}`,
  );
  ...
  invariant(
    ...
    "v12 color diagnostic frame lookup drifted",
  );
  const [
  ...
  const colorSamples = Object.fromEntries(
    Object.entries(COLOR_SAMPLE_POINTS).map(([sampleId, point]) => [
      sampleId,
      {
        ...
        v12ImplementationCapture: rgbHexAt(implementationColorPng, point),
      },
    ]),
  );
  invariant(
    colorSamples.body.windowIdCapture === "#b8d8f7"
      ...
      && colorSamples.body.v12ImplementationCapture === "#c2def8"
      ...
      && colorSamples.footer.v12ImplementationCapture === "#1e67ce",
    "v12 capture-path color diagnostic sample drifted",
  );
  ...
  for (const pair of EXACT_PID_V10_KEYFRAME_PAIRS) {
    ...
    const progressWidthPixels = EXACT_PID_V12_PROGRESS_ANCHORS.find(
  ...
  const v11Delta = {
    ...
    perFrame: comparisons.map((item) => {
      ...
      invariant(previous, "v11 comparison pair lookup drifted");
      return {
  ...
  const report = {
    schemaVersion: 1,
    reportType: "g4-l3-ts006-exact-pid-implementation-comparison-v12",
    ...
    boundedIncrement: {
      ...
      transferFunction:
        entryState.diagnosticCandidateIncrement.transferFunction,
      browserImplementation:
        entryState.diagnosticCandidateIncrement.browserImplementation,
      sourceStaticPathAffected: false,
      originalRuntimeColorPipelineEstablished: false,
      visualParityEstablished: false,
      progressAnchorsInheritedUnchangedFromV11:
...
export async function writeG4L3Ts006ExactPidImplementationComparisonV12({
  check = false,
} = {}) {
  const {
    report,
    ...
    diffArtifacts,
  } = await buildG4L3Ts006ExactPidImplementationComparisonV12();
  ...
  writeG4L3Ts006ExactPidImplementationComparisonV12(
    ...
    .then((report) => {
      console.log(JSON.stringify({
        ...
        v11Delta: report.v11Delta.regions,
      }, null, 2));
    })
    .catch((error) => {
