...
async function privateRoot(label) {
  const root = await mkdtemp(path.join(tmpdir(), `hmg4v27-${label}-`));
...
test("v2.7 core binds the exact frozen contract, Gate-A, and source preimages", async () => {
  for (const [file, expected] of expectedHashes) {
    assert.equal(sha256(await readFile(file)), expected, path.basename(file));
  }
});

test("v2.7 core exposes no ambient filesystem, process, network, or allocator authority", async () => {
...
test("strict v2.7 registry, framing, direction, and transport matrix passes", {
...
test("v2.7 unit and 300,000-case fuzz pass ASan and UBSan", {
  skip: process.platform !== "darwin",
}, async () => {
...
test("v2.7 core object has exact symbols and reproducible bytes", {
  skip: process.platform !== "darwin",
}, async () => {
