...
import {
  PAGE_CONFIGS,
  buildPageArtifacts,
} from "./build-g5-l5-vb012-ts007-behavior-aware-canvases.mjs";
import {
...
test("VB012 and TS007 behavior-aware artifacts are deterministic and current", async () => {
  const currentnessCheck = spawnSync(
    process.execPath,
    [
      path.join(ROOT, "scripts", "manage-current-js-asset-profiles.mjs"),
      "--check",
    ],
    {
      cwd: ROOT,
  ...
  assert.equal(currentnessCheck.error, undefined);
  assert.equal(currentnessCheck.signal, null);
  assert.equal(currentnessCheck.status, 0, currentnessCheck.stderr);
  assert.equal(currentnessCheck.stderr, "");
  const currentnessSummary = JSON.parse(currentnessCheck.stdout);
  assert.equal(currentnessSummary.operation, "check");

  const builtPages = [];
  for (const config of PAGE_CONFIGS) {
    builtPages.push(await buildPageArtifacts(config));
  }
  const summary = {
    mode: currentnessSummary.operation,
    calibratedPageCount: builtPages.length,
    otherG5L5PagesStarted: builtPages.reduce((count, built) => {
      const manifestArtifact = built.artifacts.find(
        (artifact) => artifact.path === built.paths.manifest,
      );
      assert.ok(manifestArtifact);
      const manifest = JSON.parse(manifestArtifact.bytes.toString("utf8"));
      return count
        + manifest.candidateScope.otherG5L5PagesStartedByThisTransaction;
    }, 0),
    pages: builtPages.map(({summary: pageSummary}) => pageSummary),
  };
