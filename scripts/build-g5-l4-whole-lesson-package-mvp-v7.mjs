...
export const DEFAULT_PORT = 3233;
...
export const SMOKE_SCREENSHOT_ROOT = path.join(
  WORKSPACE_ROOT,
  'output/playwright/g5-l4-whole-lesson-package-mvp-v7',
);
...
export async function assertPathAbsent(target, label = target) {
  try {
    await lstat(target);
  } catch (error) {
  ...
  throw new Error(`Immutable target already exists: ${label}`);
}
...
export function readmeText() {
  return `# G5 L4 Number Lines — Whole-Lesson CEO Preview MVP v7
...
Use \`node start.mjs --port ${DEFAULT_PORT + 1}\` if port ${DEFAULT_PORT} is busy. Press
...
async function atomicInstall(stagedPackageRoot) {
  await assertPathAbsent(PACKAGE_ROOT, path.relative(WORKSPACE_ROOT, PACKAGE_ROOT));
...
async function archivePackage() {
  await assertPathAbsent(ARCHIVE_PATH, path.relative(WORKSPACE_ROOT, ARCHIVE_PATH));
  await assertPathAbsent(
    ARCHIVE_SHA_PATH,
    path.relative(WORKSPACE_ROOT, ARCHIVE_SHA_PATH),
  );
...
async function buildPackage() {
  ...
  try {
    ...
    const manifest = {
      ...
      title: 'G5 L4 Number Lines — Whole-Lesson CEO Preview MVP v7',
      target: {
...
async function smokePackage() {
  await assertPathAbsent(
    SMOKE_REPORT_PATH,
    path.relative(WORKSPACE_ROOT, SMOKE_REPORT_PATH),
  );
  await assertPathAbsent(
    SMOKE_SCREENSHOT_ROOT,
    path.relative(WORKSPACE_ROOT, SMOKE_SCREENSHOT_ROOT),
  );
  const archive = await readOuterArchiveBinding();
  ...
  try {
    ...
    try {
      ...
      await mkdir(SMOKE_SCREENSHOT_ROOT, {recursive: false});
  ...
  await writeFile(SMOKE_REPORT_PATH, stableJson(report), {
    encoding: 'utf8',
    flag: 'wx',
    mode: 0o444,
  });
