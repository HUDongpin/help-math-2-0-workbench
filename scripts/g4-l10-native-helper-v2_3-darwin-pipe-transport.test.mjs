...
async function privateRoot(label) {
  const root = await mkdtemp(path.join(tmpdir(), `hmg4v23-darwin-pipe-${label}-`));
...
test("production transport exposes no caller-injected backend, dispatcher, path, or mutation API", async () => {
  ...
  assert.match(productionSource, /#ifdef HMG4V23_TRANSPORT_TESTING/u);
...
test("strict injected transport boundary vectors pass", {
  ...
  await run("/usr/bin/clang", [
    "-DHMG4V23_TRANSPORT_TESTING=1", ...strictFlags,
...
test("injected transport vectors pass ASan and UBSan", {
  ...
  await run("/usr/bin/clang", [
    "-DHMG4V23_TRANSPORT_TESTING=1", ...sanitizerFlags,
    ...sharedCodecSources, source, unit, "-o", executable,
  ]);
  const result = await run(executable, [], {
    env: {
...
test("Clang static analyzer accepts production and test-bound transport builds", {
  ...
  for (const [label, prefix] of [
    ["production", []],
    ["test", ["-DHMG4V23_TRANSPORT_TESTING=1"]],
  ]) {
...
test("production transport object has a closed symbol surface and reproducible bytes", {
  ...
  assert.deepEqual(defined, [
    "_hmg4v23_emit_response_fd1",
    "_hmg4v23_install_and_verify_sigpipe_ignore",
    "_hmg4v23_pipe_result_diagnostic_code",
    "_hmg4v23_pipe_result_exit_code",
    "_hmg4v23_pipe_result_name",
    "_hmg4v23_receive_request_fd0",
    "_hmg4v23_release_received_request",
  ].sort());
  ...
  assert.deepEqual(undefinedResult.stdout.trim().split("\n").sort(), [
    ...
    "_fcntl", "_free", "_hmg4v23_checked_add_size",
    "_hmg4v23_deadline_from_start", "_hmg4v23_diagnostic_write_decide",
    "_hmg4v23_parse_request_header", "_hmg4v23_poll_decide",
    "_hmg4v23_poll_timeout_ms", "_hmg4v23_request_read_decide",
    "_hmg4v23_response_write_decide", "_hmg4v23_sha256",
    "_hmg4v23_timespec_parts_to_ns", "_hmg4v23_validate_buffered_request_frame",
    "_hmg4v23_validate_response_frame", "_malloc", "_poll", "_read",
    "_sigaction", "_write",
  ].sort());
