...
export const SUCCESSOR_V3_OUTPUT_RELATIVE =
  "output/playwright/g4-l10-vb003-original-host-ruffle-successor-v3";
const RESULT_RELATIVE = `${SUCCESSOR_V3_OUTPUT_RELATIVE}/diagnostic.json`;
...
function invariant(condition, message) {
  if (!condition) {
    throw new Error(`G4 L10 VB003 Ruffle successor v3: ${message}`);
  }
}
...
export function buildInputAttemptPlans() {
  return [
    {
      attempt: 1,
      id: "fresh-context-staged-pointer-release-v2-network-policy",
    ...
    {
      attempt: 2,
      id: "fresh-context-staged-pointer-release-with-exact-xml",
      transport: "page.mouse move/down/up",
      freshContext: true,
...
function launchSources() {
  ...
  const launch = Buffer.from([
    ...
    '    player.id = "player";',
    '    player.setAttribute("aria-label", "L10 original-host Ruffle successor v3 forensic diagnostic");',
  ...
  const html = Buffer.from([
    "<!doctype html>",
    '<html lang="en"><head><meta charset="utf-8"><title>L10 contained Ruffle successor v3</title>',
...
async function bindInputs() {
  ...
  const [staticReport, v2Result] = await Promise.all([
...
async function createOutput(inputs) {
  const outputRoot = path.join(PROJECT_ROOT, SUCCESSOR_V3_OUTPUT_RELATIVE);
  invariant(
    !(await pathExists(outputRoot)),
    `${SUCCESSOR_V3_OUTPUT_RELATIVE} already exists; immutable output will not be overwritten`,
  );
...
export async function runSuccessorV3Probe() {
  ...
  const [staticAfter, v2After] = await Promise.all([
    readBinding(STATIC_REPORT_RELATIVE),
    readBinding(V2_RESULT_RELATIVE),
  ]);
  ...
  invariant(
    ...
    "an immutable predecessor or canonical XML changed during v3",
  );
  ...
  const result = {
    ...
    probe: {
      ...
      browserAutomation:
        "real Chromium via repository-pinned Playwright; same staged trusted pointer transport in two bounded fresh-context network-policy variants at most",
    },
    lineage: {
      staticAntecedent: {
    ...
    hypotheses: {
      ...
      domEventLayer:
        "trusted capture/bubble pointer and mouse phases are recorded on the exact Ruffle shadow canvas",
      avm1EffectLayer:
